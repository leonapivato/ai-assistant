...
- Status: Partially superseded by ADR-0265 (one scope, and it is a count. §15 item 19's enumeration of what §13's rule requires before a consequential capability is wired — "five conditions and not three, the fourth assigned to none of those three lanes and the fifth assigned to A8 but outside the reconciliation guarantee the gate names" — becomes six. The sixth is a containment for a wrongly minted intended action: a deterministic or user-authorised ruling that a proposed act is a second act the user asked for and not the act this goal already performed. It is assigned to none of the three lanes the gate names and is reached by none of their guarantees, because the duplicate it admits is a correctly claimed, correctly authorised and correctly verified dispatch of an effect the goal already holds — nothing downstream of the mint can see that it was one act and not two, so a reader holding only item 19 wires an integration after five and is wrong. That one count, and nothing else in this ADR: §13's rule itself binds verbatim and is relied on, its statement that that decision adds two prerequisites stays true of that decision, and the two it adds are untouched; §7's obligation that an effect is performed at most once across every plan of a goal binds entire and is what the new identity serves; §7's refusal of an identity derived from capability and parameters is honoured rather than lifted; §12's booking of whether an attempt's executions are projected into the planner's input is untouched and no projection is taken; and §§1-6, §§8-12, §14 and §§16-17 stand entire) and ADR-0259 (two scopes. First, the identity §7's and §12's at-most-once obligation is stated over. §7 rules that an effect is "performed at most once across every plan of that goal" and §12 states the acceptance requirement over "a plan whose step would perform the same effect"; the superseding decision lands an identity in two parts — the intended action a step is an attempt at, which the effect row is scoped to, and the authorised call within it, identified by the bound tool's id, the digest of the supplied arguments and the binding's account, transport endpoint and canonical destination set. The guarantee is therefore narrower than "the same effect" in one direction, because two calls meaning one thing while spelling it differently carry two keys, so under one intended action the second is answered as a completed act carrying different arguments and reaches a modify-before-replace investigation rather than a reuse, and deliberately narrower in another, because two intended actions carrying one key are two acts and both happen. A reader holding only §7 reads its obligation more widely than it now holds. Every other clause of §7 and §12 binds entire: §7's keeps-everything-it-recorded rule, its extension of the not-driven rule to a plan superseded after driving, its sweep and that sweep's stated residual, its one-plan-per-walk rule, its stale-`targets_revision` refusal and its no-licence-to-repeat prohibition; §12's every other entry, both acceptance requirements' other halves — the modifying-and-fresh demonstration, the paired `INDETERMINATE` case, and the resolved-but-unapplied answer entire — and its firing conditions. §§1-6, §§8-10 and §§13-17 are untouched by that scope, and the deferrals §12 books are fired rather than replaced. Second, §11's `core/types.py` clause, in its "Nothing else" closure over `StepTransition` alone: that type gains `satisfied_by_execution` and `satisfied_by_step`, both `DurableIdentifier | None` defaulting to None, and `satisfied_by_key`, an `EffectKey | None` defaulting to None, all three required together and only on a transition to `SUCCEEDED`, which `commit_transition` refuses on any other target status and refuses carrying some but not all, because they are the only stated route by which a step satisfied from an effect its goal already completed carries the holder's identifiers into the committed `StepExecution` and lets the store verify the key it is satisfied under — a reader holding only §11 builds a transition that cannot carry that satisfaction at all. That closure, and nothing else in §11: `attempt_id` and its validator, the no-new-model, no-new-enumeration and no-new-constant rule, the no-widening list of `StepExecution`, `ExecutionState`, `ActionPlan`, `PlanStep`, `GoalAttempt` and `Goal`, the three-member and four-strengthening classification of what this decision changed, which the superseding one adds two strengthenings to rather than restates, the triad obligation, and the `PROTOCOL_VERSION` and `PlanExport.schema_version` clauses all bind entire) and ADR-0261 (one scope: the class §3's attempt conjunct refuses with in its *state limb* alone. §3's *"Both refuse with a `PlanningError` that is **not** a `StaleExecutionError`"* becomes, for a refusal on that limb — the attempt being terminal or paused — **a refusal with `ClaimRefused`** — a new `PlanningError` subclass raised by **that limb** and by ADR-0249 §8's revision conjunct and by nothing else in the corpus, carrying a message and no structured state. It is still a `PlanningError` and still not a `StaleExecutionError`, so §3's own reasoning for that choice is exercised rather than contradicted; what fails the test is that a reader holding only §3 builds a store whose claim refusals a driver cannot tell from an unknown execution's or a malformed transition's, which is the distinction ADR-0261 §7's composing turn is decided on. The conjunct's other three limbs — the attempt exists, it carries this execution, it is the only attempt of the goal naming it — and the successor conjunct keep that class exactly, each being a defect of the walk rather than a state a user act produced, and ADR-0261 §7 leaves every one of them to propagate. Every other clause of §3 binds entire and is what that decision reasons from — its four attempt limbs, its successor conjunct, its read-inside-the-same-indivisible-step rule, its exclusive ownership of an execution, its refusal of a caller-supplied revision and its "no separate read of the attempt on which a decision is taken" — and so do §2's stop list, §6's authoritative record of an uncertainty, §7's preservation rules, §11's `TurnOutcome.step` clause and §§12-13's bookings, a booking discharged not being a clause made false) and ADR-0267 (one scope, and it is a count. §15 item 19's enumeration of what §13's rule requires before a consequential capability is wired — *"**five** conditions and not three"*, which the record above makes six — becomes **seven**. The seventh is a freshness prerequisite and it binds one kind of capability: one whose acts are authorised through a `MONEY` ceiling is wired only where the provider offers an enforceable **hold** on the quoted amount, or a **conditional execution** that validates the quoted amount atomically with the act and fails the act where it does not hold. It is assigned to none of the lanes the gate names and is reached by none of the six, because the act it admits is correctly claimed, correctly authorised against a recorded number and correctly verified afterwards: the quote was true when it was read and false when the charge was made, and the over-bound charge has already happened. A capability no `MONEY` ceiling ever authorises is unaffected and the other six reach it unchanged. That one count, and nothing else in this ADR: §13's rule binds verbatim and is relied on, its own *"this decision adds **two** prerequisites"* stays true of that decision, §15's every other item, its thin/full division and its owed-by-the-lane-that-lands-the-clause rule are untouched, and §§1-14 and §§16-17 stand entire)
...
- **Partially superseded: 2026-09-13 by ADR-0259 — the identity §7's and §12's at-most-once
  ...
  *"a plan whose step would perform **the same effect**"*, and neither names the identity that
  ...
  keys, so under **one** intended action the second is answered as a completed act carrying
  different arguments — not dispatched, and not reused either — and what the unrecognised
  equivalence costs is a modify-before-replace investigation of an act that needed no modifying.
  And it does not refuse a repeat the user asked for: two intended actions carrying one key
  ...
  **Every other clause of §7 and §12 binds entire and is relied on**: §7's
  ...
  answer entire — and its firing conditions. **§§1-6, §§8-10 and §§13-17 are untouched by that
  scope**, and the deferrals §12 books to A8 are **fired rather than replaced**, which earns no
  record under ADR-0082 §1.
- **Partially superseded: 2026-09-13 by ADR-0259 — §11's `core/types.py` clause, in its
  *"Nothing else"* closure over `StepTransition` alone. Nothing else in §11.** That clause adds
  *"**One field**: `StepTransition.attempt_id`"* and closes the type. It now gains three more:
  **`satisfied_by_execution`** and **`satisfied_by_step`**, both `DurableIdentifier | None`
  defaulting to `None`, and **`satisfied_by_key`**, an `EffectKey | None` defaulting to `None`,
  **all three required together and only on a `→ SUCCEEDED` transition** — `commit_transition`
  refuses any of them on a transition to any other status, and refuses one carrying some but not
  all. They exist because the stage that satisfies a step from an effect the goal
  already completed is the only holder of the two identifiers, and `commit_transition` writes the
  `StepExecution` from a `StepTransition` and nothing else: without them the pair reaches the
  committed row by no stated route. **A reader holding only §11 builds a transition that cannot
  carry that satisfaction at all**, which is why the record is written.

  **Every other clause of §11 binds entire and is relied on**: `attempt_id` and its validator, the
  no-new-model, no-new-enumeration and no-new-constant rule, the no-widening list, the
  three-member and four-strengthening classification of what **this** decision changed, which that
  one adds two strengthenings to rather than restates — `commit_transition`'s persistence of the
  pair and its claim condition verifying it — the triad obligation, and the `PROTOCOL_VERSION` and
  `PlanExport.schema_version` clauses, which
  that decision moves by their own read-off-the-tree instruction rather than by lifting them.
- **Partially superseded: 2026-09-15 by ADR-0267 — §15 item 19's condition count alone, six to
  seven. Nothing else in this ADR.**
  The seventh condition is the freshness prerequisite the quote decision could not discharge in
  code: a quote is read before the act and the charge is made after it, and nothing local proves
  the number is still true — an unenforced expiry, elapsed-time proximity and a local
  compare-and-swap are each a check in the wrong place, a store transaction observing stored values
  where what must be observed is the provider's current price at the instant of the act. So a
  capability whose acts are authorised through a `MONEY` ceiling is wired only where the provider
  offers a hold or a conditional execution that validates the quoted amount atomically with the
  act. §13's rule is relied on verbatim; what grows is the gate's total, not that rule.
