...
- **Partially superseded: 2026-09-13 by ADR-0259 — the identity §7's and §12's at-most-once
  ...
  *"a plan whose step would perform **the same effect**"*, and neither names the identity that
  ...
  ADR-0259 §1 lands one: the **authorised call**, identified by the bound tool's id, the digest of
  the **supplied** arguments and the binding's account, transport endpoint and canonical
  destination set. It therefore recognises a repeat of the same authorised call and **not** two
  calls that mean the same thing while spelling it differently — `alice@Example.com` in one plan
  and `alice@example.com` in the next carry two keys and the second is dispatched. **A reader
