...
- **Partially superseded: 2026-09-15 by ADR-0266 — §15 item 19's count, and nothing else in this
  ADR.** That item closes, in terms, the enumeration of what §13's rule requires before a
  consequential capability is wired; ADR-0265 §6 made it six, and this decision makes it
  **seven**. The seventh binds any capability whose authorisation can be reached through route (d)
  with a coverage member that decision mints, and it is one thing implemented and demonstrated: **a
  mechanical test of the polarity of the clause a recorded span was taken from**, or a span
  contract that makes a span unable to carry a polarity its surrounding text does not.

  **The residual it contains is stated exactly rather than gestured at.** A model chooses the
  interpretation element's span; that decision admits a span only where it is a **whole clause**
  of the act's own recorded utterance, which fixes the polarity of the clause the span *is* and
  refuses every truncation of one. Nothing there establishes the polarity a **neighbouring**
  clause supplies, and no enumeration of refusal words reaches the next spelling of *avoid* — so
  *"avoid these prices — under 100 euros"* mints an exclusive `maximum` of `100`, and route (d)
  can `ALLOW` a charge inside an authority the user's own words refuse. ADR-0249 §7 owns the
  question, its span check being a containment test and never a check of what the model meant.

  **The gate's existing guarantees do not reach it, which is why the count grows rather than the
  residual being left to them.** Verification reports the completed charge and cancellation
  compensates the attempt, and **neither makes the pre-execution permission decision valid** —
  §13's own test, applied here the other way round from the duplicate-dispatch residual the sixth
  condition closes. **§13's rule itself binds verbatim**, its own two prerequisites are unchanged,
  the milestone clause and the wiring-one-is-what-this-rule-binds sentence stand, and every other
  clause of §15 and every other section of this ADR stand entire.
## Context
