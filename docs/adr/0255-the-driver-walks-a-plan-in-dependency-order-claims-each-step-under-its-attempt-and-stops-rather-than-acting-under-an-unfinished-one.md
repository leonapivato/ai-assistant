...
- **Partially superseded: 2026-09-13 by ADR-0259 — §11's `core/types.py` clause, in its
  ...
  *"**One field**: `StepTransition.attempt_id`"* and closes the type. It now gains two more:
  **`satisfied_by_execution`** and **`satisfied_by_step`**, both `DurableIdentifier | None`
  defaulting to `None`, **non-`None` together and only on a `→ SUCCEEDED` transition** —
  `commit_transition` refuses either on a transition to any other status, and refuses one carrying
  exactly one of them. They exist because the stage that satisfies a step from an effect the goal
