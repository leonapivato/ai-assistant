...
- Status: Partially superseded by ADR-0256 (five limbs, all about one field, `Authorization.expires_at`. **§12's ladder rung 3 alone**: where the recorded act states no instant and the goal carries no `deadline`, `expires_at` is now the recorded act's own retention horizon — `proposed_at` advanced by the window the deployment keeps a captured turn for — and a row **is** written, where that window is finite. **§12's stated cost of that rung**, *"a goal carrying no `deadline` whose user stated no horizon has no route (d) at all, and every call of it asks"*, which is over-wide as it stands: it is true only where the deployment is set to keep turns forever, and there the ratified rung stands verbatim as the ladder's last. **§12's path-(ii) transcription clause, §1's path-(ii) *"transcribed unchanged"* list and §1's path-(i) *"the only path that may … set `expires_at`"***, each in the `expires_at` limb alone and in the **narrowing direction alone**: a correcting act whose own span states an instant strictly after the new row's `proposed_at` and strictly before the superseded row's `expires_at` carries that instant, and in every other case the expiry is transcribed unchanged — nothing lengthens one on any path but (i), so *"No sequence of corrections outlives the confirmation that began it"* holds a fortiori. **§9 clause (ii)'s bar limb *"move `expires_at`"***, in that same narrowing direction alone, on that clause's own stated principle that *"The interpretation narrows what the act covers and can never widen it"*. And **§19's entry naming *"What the user is shown where §12's ladder yields no instant"***, **narrowed in application and not replaced**: the case it books now arises only where the deployment keeps turns forever, and the decision it names — asking *"until when?"* at the act — is still booked and still not taken. Every other clause of §12 binds entire and several are what the replacement rests on: no `Settings` field, no deployment-wide expiry for an authorization, no ceiling over rung 1 or rung 2, the expiry taken once at the write and never recomputed, an answer arriving at or after it establishing nothing, the recipient-grant and configured-provider exclusions, the visible-at-the-act rule and the no-deletion rule. §§1–11 and §§13–22 stand as they are, but for the limbs of §1 and §9 named above.)
- Date: 2026-09-12
