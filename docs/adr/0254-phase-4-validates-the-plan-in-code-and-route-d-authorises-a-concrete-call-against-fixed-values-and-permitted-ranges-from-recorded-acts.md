...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1's proposal-completeness condition, §2's
  ...
  `maximum` inequality alone — its `minimum` conjunct is unmoved — §6's lineage-discharge clause
  in the narrowing direction, §9 clause (ii)'s test of *"bear on"*, §9 clause (iii)'s
  *"an argument no member names"* limb, and §8's and §10's resolution enumeration in its closure
  at three. Nothing else in this ADR.** No
  ...
  **how a member's shape is chosen** — §10's three resolutions turn a span into a value and name
  no argument, §9 clause (ii) states the property and not the procedure, §8's basis names no
  ...
  **The superseding decision answers it by removing the question from the record and then moving
  where the bound is proved.** A `CoverageMember` carries a **`kind`** and no `argument`, so it
  states what the user's words fixed or bounded and nothing about which slot carries it. The
  owner's ruling of 2026-09-14 then rules that a price is *"usually a consequence of the chosen
  params, not an argument"*: so the **investigation** records the quote for the goal's intended
  action, over that step's own user-facing arguments, and the **authorisation** phase proves the
  bound against that quote before execution. A declaration **may** declare an argument at a kind,
  and then that comparison is taken **as well**, a `MONEY` member needing the quote in every case
  because a `max_price` filter and a transfer amount are not the charge. *"Bears on"* is
  established by **kind agreement** between the constraint and the thing it is proved against,
  ...
  **What that decision does *not* settle, said here because this section's reader will look for
  it.** It states the four facts its comparison reads off a quote — the intended action it was
  taken for, the arguments it was quoted over, an amount with its currency, and the output it was
  read from — and states **no carrier**: no type, no field of any model, no store member, no
  Protocol and no rule about which component mints one. Those are booked to a later decision, so
  between that ADR's lanes and this one a row carrying a `MONEY` member covers nothing and every
  such act asks — the fail-closed direction. What **is** settled here is what the record holds and
  what the proof is taken against, which is what §20's Lane 2 stopped for want of.
  ...
  **`maximum_exclusive`**, so *"under 100"* stops being recorded as a bound a call at exactly 100
  ...
  a **fourth** member, `STATED_BOUND`, reading a **ceiling** off the span by a closed table — no
  floor at all, so a truncation of the user's own words can only ever restrict — and no reading is
  taken at all unless the span is **equal to the act's own recorded utterance**, which is what makes
  the polarity total: no substring, clause or neighbouring text is left for a rule to understand.
  **That is the supersession §19 books by name.**

  **And §6's lineage discharge is narrowed rather than read.** That clause discharges ADR-0181
  §5's floor for a request a live row covers *"in full"*, because where every user-facing argument
  is covered by the user's own fixed values and permitted ranges, outside content *"cannot have
  steered anything the user did not bound"*. A digest proves the call is the one **quoted**, not
  one the user bounded, so the discharge is now available **only where every user-facing argument
  is covered by a member on the argument route** — a tainted request covered through the quote
  keeps its `CONFIRM`, which is this section's own *"Partial coverage still asks"* reaching one
  further case. Every other limb binds entire and ADR-0181 §5 itself is untouched.

  **And §9 clause (iii) is narrowed in one limb, which the decision could not leave standing.**
  That clause makes a request reach no route (d), and no standing route at all on §6's bar, where
  it introduces *"an argument no member names"*. A member names no argument once the shape above
  lands, so **every** argument of **every** request would satisfy that limb and route (d) would be
  unreachable altogether — the mirror of §1's inert proposal. The limb is restated as *an argument
  the row does not cover under condition 6*: its force is unchanged, and only its test moves.
  Every other limb binds entire, the material-ambiguity rule with ADR-0250 §6's three conditions
  among them.

  **Every other clause of the nine sections binds entire.** §1 is untouched but for its third
  proposal condition: a member needs **no argument at write time**, so the row written before the
  question is put, the coverage never edited afterwards and all three write paths stand exactly as
  ratified, and the completeness condition is restated over the new test rather than dropped.
  §3's condition 3, comparing the request's `tool` against the row's **by value**, is relied on
  and is why a kind-typed member is still not portable across tools. §13 is relied on entire and
  is what the new comparison is taken under — at
  `ActionPolicy.decide`, on the concrete request, at every dispatch, with no cached verdict — and
  its *"Coverage and sufficiency are two tests and neither clears the other"* stays true: a
  standing quote satisfies no condition of a step. §9's clause (i) and every limb of clause (iii)
  but the one narrowed above, §2's two-shapes
  validator and `BoundKind`, §3's conditions 1-5 and its classification, §4's totality and its
  no-schema clause, §8's basis and §10's three ratified resolutions are each relied on rather
  than moved.
## Context
