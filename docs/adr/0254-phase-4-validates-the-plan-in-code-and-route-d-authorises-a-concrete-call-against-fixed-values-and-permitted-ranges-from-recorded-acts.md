...
- **Partially superseded: 2026-09-12 by ADR-0255 — §14's where-phase-4-leaves-an-attempt
  ...
  third case: a check **of a step** whose operands the same plan will produce before that step is
  dispatched — a step earlier in `steps` not yet disposed of, or an interpretation of that plan
  not yet performed, a `record`-backed one included — is **deferred** rather than failed, so it
  neither blocks the advance to `AttemptPhase.EXECUTE` nor triggers a replan, and the attempt
