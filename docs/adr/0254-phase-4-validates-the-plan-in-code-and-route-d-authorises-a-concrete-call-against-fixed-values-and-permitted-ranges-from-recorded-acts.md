...
- **Partially superseded: 2026-09-15 by ADR-0267 — §1's `Authorization` field list, §11's
  ...
  **§1.** The row gains `quoted`, an `ActionQuote | None` defaulting to `None`: the governing quote
  ...
  `orchestration` code that builds the row and never edited afterwards. It is on the row for §1's
  own reason for writing the row before the question — what the user is shown is a rendering of a
