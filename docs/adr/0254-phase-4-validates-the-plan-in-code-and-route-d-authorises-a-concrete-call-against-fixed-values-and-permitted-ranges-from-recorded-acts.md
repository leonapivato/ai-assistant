...
- **Partially superseded: 2026-09-15 by ADR-0270 — one scope, in §16's `core`-surface roster.
  Nothing else in this ADR.** §16 states what `core/types.py` gains, and it has been counted up
  twice since: ADR-0266 §9 left it at **fourteen** types and **seven** fields, ADR-0267 §9 at
  **eighteen** and **twelve**. ADR-0270 §2 adds **`CoverageAnswer`** — a **nineteenth** type, a
  frozen model carrying `met` and the governing `ActionQuote` that answer was proved over — and
  **no further field**. A reader holding only §16 implements a roster test that fails on that
  decision's own surface, which is the ground ADR-0267 §9 took the same scope on. **§16's
  `core/protocols.py` roster is untouched**: it enumerates what this ADR's own lanes add there and
  names `ActionPolicy` in none of it, the widening of that Protocol being *"a widening §16 does not
  close"* as ADR-0267 §9 already read it. **Its `PermissionDecision` clause is untouched**, that
  record gaining nothing there; **its `core/errors.py` roster is untouched**, no error class being
  minted; and its *"`core/config.py` gains nothing at all"*, its wire clause — nothing of that
  decision crossing a frame — its lane attribution and its transcribe-the-ruling-whole rule bind
  entire. **And every other clause of this ADR binds entire**, §1's completeness condition, §13's
  recheck-at-`decide` with its no-cached-verdict rule and §15's writer clauses conspicuously so:
  they are what that decision is stated over and what it leaves standing word for word.
