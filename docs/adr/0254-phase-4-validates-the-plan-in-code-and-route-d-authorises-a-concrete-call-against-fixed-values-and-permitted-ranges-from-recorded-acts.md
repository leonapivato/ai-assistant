...
- **Partially superseded: 2026-09-13 by ADR-0266 — §2's member shape, §3's comparison and
  field count, §4's `MONEY` currency conjunct, and §8's and §10's resolution enumeration in its
  closure at three. Nothing else in this ADR.** No clause of this decision states **how a
  recorded span is associated with an argument key**, nor **how a member's shape is chosen** —
  §10's three resolutions turn a span into a value and name no argument, §9 clause (ii) states
  the property and not the procedure, §8's basis names no argument, §1's path (iii) points at an
  element of the goal's `constraints` which carries no typed value and no argument key, and §9's
  no-model clause forecloses the planner. **The superseding decision answers it by removing the
  question from the record**: a `CoverageMember` carries a **`kind`** and no `argument`, so it
  states what the user's words fixed or bounded and nothing about which slot carries it, and the
  argument it meets is decided **at the comparison**, against the declaration — an argument
  covered where its `ToolDefinition.bounded_arguments` declares it at a `BoundKind`, the row
  carries exactly one member of that kind, and it satisfies that member. *"Bears on"* is then
  established by **kind agreement** and never by numeric fit, which is what §9 clause (ii)
  requires and what no rule selecting an argument from a request could give: a constraint about
  a hotel's star rating has no `MONEY` reading, mints no money member and therefore meets no
  price argument.
  ...
  **Three consequences of that shape reach this section's own words.** `maximum` stops being
  ...
  fact about a declaration and not about an act, so §4's conjunct is taken at the declaration's
  own currency key. And `ResolutionRule` gains a **fourth** member, `STATED_BOUND`, reading a
  bound off the span by a closed table of direction forms with a negation guard — **the
  supersession §19 books by name**, fired here because a direction is in the user's words or it
  is nowhere.

  **Every other clause of the five sections binds entire.** §1 is untouched in every limb and
  is what makes this change possible: a member needs **no argument at write time**, so the row
  written before the question is put, the coverage never edited afterwards and all three write
  paths stand exactly as ratified. §9 is untouched and is what the change exists to satisfy.
  §2's two-shapes validator and `BoundKind`, §3's conditions 1-5 and its classification, §4's
  totality and its no-schema clause, §8's basis and §10's three ratified resolutions are each
  relied on rather than moved.
