...
- **Partially superseded: 2026-09-15 by ADR-0266 — §2's member shape, §3's condition 6 and
  field count, §4's `MONEY` currency conjunct, §9 clause (ii)'s test of *"bear on"*, and §8's
  and §10's resolution enumeration in its closure at three. Nothing else in this ADR.** No
  ...
  **Three consequences of that shape reach this section's own words.** `maximum` stops being
  ...
  fact about a declaration or about a quote and not about an act. And `ResolutionRule` gains a
  ...
  **Every other clause of the six sections binds entire.** §1 is untouched in every limb and is
  what makes this change possible: a member needs **no argument at write time**, so the row
  written before the question is put, the coverage never edited afterwards and all three write
  paths stand exactly as ratified. §3's condition 3, comparing the request's `tool` against the
