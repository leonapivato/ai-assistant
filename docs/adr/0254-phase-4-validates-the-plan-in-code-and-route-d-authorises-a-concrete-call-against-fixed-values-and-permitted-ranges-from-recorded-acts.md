...
- **Partially superseded: 2026-09-12 by ADR-0255 — §17's carried-and-discharged sentence, in the
  ...
  **discharged by A8, A9 and A10**"* gains a **fourth** condition: that decision's §13 forbids
  wiring a consequential capability until the **evidence-to-claim window** its §1 states — a
  concurrent turn's ADR-0252 §8 refresh superseding a `STANDING` row between a step's evaluation
  and its committed claim — is **closed**, and that window is assigned to none of A8, A9 and A10.
  A reader holding only §17 would read those three guarantees landing as releasing a deployment
  §13 still refuses. **§17's rule itself is untouched and is quoted verbatim** — the verification,
