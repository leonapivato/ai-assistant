...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1 in four limbs, §2's member shape and two
  ...
  §16's roster in three limbs, and eight of §20's landed arms. Twelve scopes, and nothing else in
  this ADR.** No clause of this
  ...
  **§20's arm enumeration is superseded as a class rather than as a list**, because a partial list is
  what two review rounds each found wrong. The scope is every arm of the seventy-one whose
  statement reads the contract that decision changes, and **five mechanisms decide which**: a
  member *naming an argument* and condition 6 over key sets (arms 8, 15, 20, 47, 53, 54, 59, 68); a
  bound's `currency_argument` and its cross-member refusal (arms 10, 18); a row carrying two
  members of one **kind** or a `fixed` value the new kind validation refuses (arms 1, 7, 14, 18,
  48, 64); a `MONEY` member treated as **met without a quote**, and `ResolutionRule`'s closure at
  three (arms 31, 34, 44, 45, 46, 49, 57, 60, 66, 67, 70); and a **path-(iii) opening act minted
  from a stated bound** (arms 64, 65). **Each is restated over the new shape by the lane that owns
  it and none is dropped.** **The cleared arms are recorded with their ground**: arm 5 stands
  verbatim, `maximum_exclusive` defaulting to `False`; arm 21's basis refusals and arm 35's roster
  assertions are negative and stay true; arms 6, 11 and 12 test §4's readings, unmoved; and `TERMS`
  and `PERIOD` members stay constructible, §2 admitting a kind and the new reading minting none
