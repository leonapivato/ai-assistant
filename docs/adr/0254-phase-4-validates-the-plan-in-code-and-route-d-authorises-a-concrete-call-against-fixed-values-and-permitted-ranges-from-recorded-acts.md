...
- Status: Partially superseded by ADR-0256 (six limbs, all about one field, `Authorization.expires_at`. **§12's ladder rung 3 entire**, its *"no row at all"* and its *"Nothing is invented, nothing is defaulted and nothing falls back to a configuration"* included: where the recorded act states no instant and the goal carries no `deadline` **or carries one at or before `proposed_at`**, `expires_at` is now `proposed_at` advanced by the deployment's turn-retention window and a row **is** written, where that window is finite; and with that rung, **§1's fourth proposal condition** — *"§12's ladder yields an `expires_at` … There is no deployment figure to fall back on and none is invented"*, which §1 makes govern path (iii) too — and **§20's arm 69 in its two rung-3 cases alone**, every other case of that arm — rung 1's, rung 2's, its path-(ii) *"make it Sunday"* case, its edited-deadline case, its `Settings` roster test and its recipient-grant and configured-provider limbs — standing verbatim. **§12's stated cost of that rung**, *"a goal carrying no `deadline` whose user stated no horizon has no route (d) at all, and every call of it asks"*, which is over-wide as it stands: it is true only where the deployment is set to keep turns forever, and there the ratified rung stands verbatim as the ladder's last. **§12's no-deployment-wide-expiry clause, in the single respect that the third rung reads one deployment value, the turn-retention setting**, together with **§12's justification clause in its limb *"and in neither case by a deployment"***; every other limb of both stands — no `Settings` **field** is added, the expiry is still per `Authorization` and shown when it is granted, and no figure bounds rung 1 or rung 2. **The path-(ii) transcription rule in all six places this ADR states it** — §12's path-(ii) transcription clause, §1's path-(ii) *"transcribed unchanged"* list, §1's path-(i) *"the only path that may … set `expires_at`"*, §1's *"what path (ii) may change"* clause in its limb *"or move `expires_at`"*, §1's path-(ii) liveness clause in its reason limb *"because a correction transcribes the predecessor's `expires_at`"* — that limb alone, the live-row requirement itself untouched — and §5's *"`expires_at` is transcribed from the row it supersedes"* — **together with §20's arm 17 in its `expires_at` field alone and §20's arm 25 in its *"carry one `expires_at`"* limb alone**, each in the `expires_at` limb and in the **narrowing direction alone**: a correcting act whose own span states an instant strictly after the new row's `proposed_at` and strictly before the superseded row's `expires_at` carries that instant, and in every other case the expiry is transcribed unchanged — nothing lengthens one on any path but (i), so *"No sequence of corrections outlives the confirmation that began it"* holds a fortiori. **§9 clause (ii)'s bar limb *"move `expires_at`"*, and §20's arm 20 which tests it***, in that same narrowing direction alone — arm 20 standing entire on its own terms, its subject being a widening — on that clause's own stated principle that *"The interpretation narrows what the act covers and can never widen it"*. **§9 clause (iii) is not superseded in any limb**: an instant a correction's span does not settle to one value is not taken and the user is asked, exactly as ratified, and clause (iii) binds entire for every coverage member and for a consequence outside scope. And **§19's entry naming *"What the user is shown where §12's ladder yields no instant"***, **narrowed in application and not replaced**: the case it books now arises only where the deployment keeps turns forever, and the decision it names — asking *"until when?"* at the act — is still booked and still not taken. Every other clause of §12 binds entire and several are what the replacement rests on: the expiry taken once at the write and never recomputed, an answer arriving at or after it establishing nothing, the recipient-grant and configured-provider exclusions, the visible-at-the-act rule and the no-deletion rule. §§1–11 and §§13–22 stand as they are, but for the limbs of §1, §5, §9, §19 and §20 named above — those five sections and §12 are the whole of what the superseding decision reaches, and its §7 is the inventory.) and ADR-0255 (two scopes. The first is §14's **where-phase-4-leaves-an-attempt** enumeration, in its two-case shape alone: "**Every check passed** — the attempt's `phase` advances to `AttemptPhase.EXECUTE`" and "**A deterministic check failed on the plan** — the attempt stays `RUNNING` and the plan is replanned within the attempt" gain a **third** case. A check **of a step** at least one of whose operands this same plan will produce before that step is dispatched — a step earlier in `steps` that has not been disposed of, or an interpretation of this plan that has not been performed, a `record`-backed one included — and **every one of whose operands already available at phase 4 is satisfied**, is **deferred**, not failed — a known failure dominating a deferral, so a check with an operand available now and failing now stays a failed check whatever else it waits on: it neither blocks the advance to `EXECUTE` nor triggers a replan, and it is decided at that step's own dispatch, at the moment of dispatch, in the words the sufficiency decision fixes. The attempt advances to `EXECUTE` where every check either **passed or was deferred**. Without it every plan carrying a `depends_on` replans forever, because a dependent step's dependency and condition are unsatisfied at phase 4 for every such plan by construction, and the dependent execution the plan-shape decision exists to make expressible never dispatches. That scope, and §17's below, and nothing else in this ADR: §14's **four checks and their order** bind verbatim, and so do its no-fifth-check rule, its no-capability-vocabulary-check clause, its no-spend-ceiling clause, its `MONEY`-bound clause, its advisory-in-one-direction rule for an intent-match assessment, its **uncovered-arguments limb** — the `CONFIRM` park and the single writer of `AWAITING_AUTHORIZATION`, which the superseding decision routes nothing away from and which is neither a deferral nor a failure — its **failed-check limb for every check that is not deferred**, its no-phase-moves-backwards clause and its supplying-an-authorization-opens-no-attempt clause. §13's recheck-at-`decide` and its no-cached-verdict rule, §19's reservation of the re-entry mechanism and of the outside-a-turn driver to A7, and §§1-12, §15, §16, §18 and §§20-22 all stand entire. **The second scope is §17's carried-and-discharged sentence, in the discharge half alone**: "it is carried by A5, A6 and A7 and **discharged by A8, A9 and A10**" gains two further conditions, because the superseding decision forbids wiring a consequential capability until two residuals its own sections state are closed. The first is the evidence-to-claim window its §1 states — a concurrent turn's evidence refresh superseding a row between a step's evaluation and its committed claim — assigned to none of those three lanes. The second is its §3's resolved-but-unapplied answer, durably recoverable: a confirmation this ADR's §4 resolves at step 5 whose claim that decision's §3 then refuses at step 6, which its §12 assigns to A8 but which is no part of the reconciliation guarantee the gate names, so A8's landing does not by itself discharge it. A reader holding only §17 would read A8's, A9's and A10's guarantees landing as releasing a deployment that decision still refuses. §17's rule itself is untouched and quoted verbatim — the verification, uncertain-outcome and cancellation guarantees, the milestone clause and the "wiring one is what this rule binds" sentence — as is the carry half naming A5, A6 and A7, and §17's second normative clause that this decision's own lane dispatches nothing and wires nothing) and ADR-0266 (twelve scopes, found by one sweep of this document for every clause naming a `CoverageMember`'s `argument`, a `ValueBound`'s `currency_argument` or a member *naming* an argument, for every clause treating a span as **establishing** rather than proposing, for §16's `core`-surface roster, and for §20's landed arms. **§1, in four limbs.** Its **proposal-completeness** condition, that the coverage a proposed row would carry is *"complete for this request"* because *"every user-facing argument of the request … is named by a member"*, is restated as *the row the proposal would write satisfies **condition 6** for this request* — condition 6 alone, because a proposal is written `PROPOSED` and §3's condition 1 asks for a live row; a member names no argument once §2 below lands, so the ratified wording would be satisfiable only by its second limb and no row carrying a non-empty coverage could ever be proposed. Its purpose, disposition and cost are unchanged. **Path (ii)'s trigger**, which fires on a later turn whose span *"names an argument"* a live row already carries a member for, fires on a span minting a member of a **kind** a live row carries one of. **Path (iii)'s trigger and its per-declaration member selection** — it fires on a span stating a value *"over an argument of the request being built"* and rules that *"A row carries members for the arguments of its **own** declaration that the act's words bear on"* — go with the mint reading the goal and **no declaration at all**, so members are not selected per declaration, two rows of one act carry the same members, and this section's own *"up to fifty pounds for the train and a hundred for the hotel"* illustration mints nothing, the one-member-per-kind refusal reaching both money constraints; no new selection rule replaces the limb, and the superseding decision books the reading that would need one. **And path (iii)'s direct establishment, for a member that decision mints and for no other**: a reading of the user's words the user has not yet seen is **proposed** and settled by the answer, never written `ESTABLISHED` with no question put, so the fourth resolution below reaches an opening act in no case — which adds no refusal to §10's three ratified resolutions and leaves path (iii) exactly as inert as it is today. **§2's member shape**: a `CoverageMember` stops carrying an `argument` and carries a **`kind`** instead, so a member records *what the user stated* and never *which slot it fills*; with it go §2's depth-one clause — restated on the new declaration field — its no-two-members-name-one-argument rule, which becomes **one member per kind**, and its `currency_argument` field on a `MONEY` bound, while the bound gains **`maximum_exclusive`** so that a ceiling the user stated strictly stops being recorded as an inclusive one, and a `fixed` value is validated against the member's kind. **Two further limbs of §2's `MONEY` currency clause go with that field**: its cross-member refusal, that a `fixed` member naming the bound's own `currency_argument` must equal its `currency`, is not constructible once no member names an argument; and its ***"A call whose declaration carries no currency argument takes no `MONEY` bound"*** rule is **reversed** by the owner's ruling of 2026-09-14, such a call taking one and proving it against the quote, while its no-inference limb is kept and restated one field over. **`maximum` stays required and `minimum` stays optional**, exactly as ratified: that decision's reading mints only ceilings. A reader holding only §2 authors a member that claims to know which argument it fills, records *"under 100"* as a bound a call at exactly 100 satisfies, and refuses a money bound at every declaration naming no currency argument. **§3's condition 6 and its field count**: the set equality over argument **keys** and the per-argument rule reading a member *"naming it"* are replaced by **two routes** — a member is met against the **quote taken for the step's intended action**, over the very arguments that were quoted, or, where the declaration declares an argument at the member's kind, against that argument; a `MONEY` member needing the quote, and the declared argument **as well** where one exists, a filter or a transfer amount never standing in for a charge; both of condition 6's directions are kept, an unmet member and an uncoverable declared argument each leaving the request uncovered; the validator refusing a member that names a system-supplied argument goes with the field it read, its rule preserved by a refusal on the new declaration field instead; and `ToolDefinition` gains **one** further field, `bounded_arguments`, so this section's *"`ToolDefinition` gains one field"* is over-narrow by one. **Where a quote is carried and what records one is decided nowhere in that ADR**: it states the four facts its comparison reads off one — the intended action it was taken for, the arguments it was quoted over, an amount with its currency, and the output it was read from — and books the carrier and the producer to a later decision, so until that lands a `MONEY` member is met by nothing and the act asks. A reader holding only §3 builds a comparison in which a price must be an argument of the call and refuses every booking whose price is a consequence of the site and the dates. **§4's `MONEY` reading, in two limbs**: the currency conjunct is read off the **quote** on the evidence route and off the **declaration** on the argument route, never off the bound, its force unchanged; and its *"less than or equal to `maximum`"* is read strictly where the bound's own `maximum_exclusive` is set, the `minimum` conjunct unmoved. **§5's widening clause, in two limbs**: at **equal** `maximum` the exclusivity flag is the whole of the difference, so **setting it narrows and clearing it widens**, and a clearing at an equal `maximum` is a widening path (ii) refuses — a lane comparing `maximum` alone would write it and establish a wider authority with no confirmation; and its *"naming an argument no member covers"* limb is restated as *an argument the row would not cover under condition 6 once the correction is applied*, its force kept and only its test moved. Every other limb of §5 binds entire. **§6's lineage-discharge clause, in the narrowing direction alone**: that clause discharges that lineage floor for a request a live row covers *"in full"*, on the stated ground that where every user-facing argument is covered by *"the user's own fixed values and permitted ranges"*, outside content *"cannot have steered anything the user did not bound"* — and the superseding decision's third conjunct lets an **undeclared** argument be covered instead by the digest of the arguments a quote was taken over, which proves the call is the one quoted rather than one the user bounded. So the discharge is available **only where every user-facing argument is covered by a member on the argument route**; a tainted request otherwise keeps the `CONFIRM` the table reached, which is this section's own *"Partial coverage still asks"* reached by one further case. Every other limb of §6 binds entire and **§6's bar takes no scope of its own**, being stated over *"§3's condition 6 and that condition alone"* so that its test moves with condition 6. **§9 clause (ii)**: *"an argument the act's own words bear on"* is given its mechanical test — agreement between the constraint's kind and the kind of the thing it is proved against, and never numeric fit, never a model's nomination, never a schema. **§9 clause (iii), in its *"an argument no member names"* limb alone and both times this ADR states it**: a member names no argument once §2 above lands, so every argument of every request would be one no member names and route (d) would be unreachable altogether. The limb is restated as **an argument the row does not cover under the superseding decision's condition 6**, keeping its whole force and moving only its test; every other limb binds entire. **§8's and §10's resolution enumeration, in the closure at three alone**: a fourth `ResolutionRule`, **`STATED_BOUND`**, reads a **ceiling** off the element's span by a closed table of four forms — *"under"*, *"below"*, *"at most"* and *"up to"*, in one language, with no negated form and no floor among them — and mints a member that is **proposed and rendered to the user beside that span**, never established from the span alone. §19 books the fourth rule by name and the superseding decision fires it; its one input is the span the basis already names. **§11's `CoverageView`**, whose **required** `argument` field would have to be transcribed from nothing once a member carries none, so that no projection would be constructible for any member at all and the confirmation §1 puts before every path-(i) row would not render: it carries **`kind`** in its place, its other three fields, its two-shape rule and its **required `span`** unchanged — and that span, *"the user's own words the member rests on"*, is what the new reading rests on, since the user's assent to the rendering is where a stated bound now gets its authority. `AuthorizationProjection` and `AuthorizationView` gain no member and lose none. **And §11's listing, in its per-member phrasing**, which the field list forces and no other clause of §11 absorbs: the surfaces clause obliges the listing to render *"each coverage member as *this argument is fixed at that value* or *this argument is bounded by that limit* together with the span the user said"*, wording that names a key no member carries, so a reader holding it either invents an argument or renders nothing. The listing renders each member **by its `kind`** — *this kind of value is fixed at that value*, or *this kind of value is bounded by that limit* — the span clause and every other item that sentence lists untouched, the goal's statement, the declaration's `VisibleIdentifier` and description, the expiry, the live-or-lapsed flag, the `PROPOSED` exclusion and the renders-no-internal-value bar among them, and the revocation surface untouched. Every other clause of §11 binds entire, and every remaining sentence of §11 worded over an argument is unmarked reason beside a mark, takes no scope of its own, and reads over the kind. **§16's roster, in three limbs**: `core/types.py` gains a **fourteenth** type, `BoundedArgument`, and a **sixth and seventh** field, `ToolDefinition.bounded_arguments` and `ActionRequest.intended_action`; and *"`PermissionDecision` gains no field"* gains one, `intended_action`, transcribed by `from_request` and compared by a sixth conjunct of `authorises`, without which a decision recorded for one act authorises a later request differing only in the act it is an attempt at — which is the one value the coverage was proved through, and so is precisely what §16's own ground for declining `ActionRequest.goal`, that the trail has no use for a value *"it cannot compare against the arguments"*, distinguishes. **§20's arm enumeration, scoped by a decidable test and not by a list**, because three review rounds each found a list incomplete: the scope is every arm of the seventy-one an implementation of the new contract cannot satisfy, and **six mechanisms decide it**, applied arm by arm by the lane that owns the arm — a member *naming an argument* or condition 6 over key sets; a bound's `currency_argument` and the cross-member refusal that a `fixed` member naming it equals the bound's `currency`; a row carrying two members of one **kind**, or a `fixed` value the new kind validation refuses; a `MONEY` member treated as **met without a quote**, or `ResolutionRule`'s closure at three; a **path-(iii) opening act carrying a member the new reading mints**; and an act that would need that reading — one stating a **money bound** — whose own recorded words its four-form table does not match, so the act mints nothing at all; that last mechanism reaching **no** act a ratified resolution mints, so arm 14's *"actually, make it Sunday"* and arm 47's *"add insurance"* are untouched. **The disposition is stated per mechanism rather than asserted of all**: for five of the six the property each arm demonstrates is unchanged and only the value or the act it is stated over moves — the lane restates the act in a form the table carries, or supplies the quote — and **no arm is dropped**, the **covered** limbs of the quote mechanism's arms riding with the decision that lands the carrier because no lane of the superseding decision can make a `MONEY` member met; for the path-(iii) one it is a **supersession outright**, arm 64's *"a row is written `ESTABLISHED` … with no `CONFIRM` put at all"* and arm 65's *"never asked to repeat the bound"* not surviving for a member that reading mints, while the whole structure of both stands for an opening act minted by §10's three ratified resolutions, which are untouched. **The arms the sweep cleared are recorded with their ground**: arm 5 stands verbatim, `maximum_exclusive` defaulting to `False`; arms 21 and 35 state negative assertions that stay true; and arms 6, 11 and 12 test §4's readings, unmoved — `TERMS` and `PERIOD` members staying **constructible**, §2 admitting a kind and the new reading minting none being two different things. §20's lane cut, its wire clause and its dependency clause bind entire. **Every other clause of all twelve sections binds entire**, several of them load-bearing there: §1's three write paths, its write-before-the-question rule and its other three proposal conditions, untouched because a member needs **no argument at write time**; §2's two-shapes-and-no-third validator, its `BoundKind` vocabulary and its permissive-direction asymmetry; §3's conditions 1-5 — condition 3's by-value declaration comparison conspicuously so, which is why a member is not portable across tools — its user-facing classification and empty default, its no-omission-reads-as-consent rule, its seam/policy split, its canonical encoding and its clause that *"a value a tool produced can satisfy a bound and can never supply one"*, which is what makes the quote route safe; §4's totality, its three readings in every other limb and its no-schema clause; §8's basis fields, its span check and its two-zones clause; §10's three ratified resolutions, untouched wherever that section already fires them, and its no-member-where-a-resolution-cannot-be-taken sentence; §11's rendered-from-the-proposed-row rule and its restart recovery; §13's recheck-at-`decide`, its no-cached-verdict rule and its coverage-and-sufficiency-are-two-tests sentence, under which the new comparison is taken; §15's writer clauses; and §16's `PROTOCOL_VERSION` clause with its no-shim rule. §7, §12, §14, §§17-19, §21 and §22 stand as they are) and ADR-0267 (three scopes. **§1's `Authorization` field list**: the row gains `quoted`, an `ActionQuote | None` defaulting to `None`, carrying the governing quote for the request's intended action **in the goal read the row was built from** — the figure the user's answer was taken over, a fact about that one read and not about the instant of persistence. It is written on §1's **path (i) alone** and is **absent on every path-(ii) and path-(iii) row**, both of which write `ESTABLISHED` directly, put no question and so show no figure, and a path-(ii) correction does not transcribe it; §1's transcription list is unchanged. Without it §11's *"A restart between the question and the answer recovers the row and renders the same projection"* is unsatisfiable for the one value a `MONEY` ceiling rests on. It is provenance: no comparison of any decision reads it, it sits **outside** `_AUTHORIZATION_SUBJECT` so `subject_digest` and §7's recompute parity are unmoved, and a stored row decodes with it absent. **§11's `AuthorizationProjection` field list**: that model is declared as carrying *"exactly `coverage` … and `expires_at`"* and gains `quote`, a `QuoteView | None` required with no default and **transcribed from the row's `quoted`** — without which §11's own *"A confirmation that establishes a bound without naming it is not a confirmation of that bound"* is unmet for the one bound whose proof is a price, and §11's concrete list already requires the question to name *"the price as a figure or as a bounded limit with its currency"*. §11's restart clause takes no scope and binds entire, the quote being on the row. **And §16's `core`-surface roster, in two limbs**: `core/types.py` gains a fifteenth, sixteenth, seventeenth and eighteenth type — `ActionQuote`, `QuotedOutput`, `ActionQuoteMinting` and `QuoteView` — and an eighth through twelfth field: `ToolDefinition.quoted_output`, `Goal.quotes`, `Goal.quotes_elided`, `Authorization.quoted` and `AuthorizationProjection.quote`. §16's `PermissionDecision` clause is untouched, that record gaining nothing here, and so is its `core/errors.py` roster, the new seam minting no error class. Every other clause of §§1, 11 and 16 binds entire — §1's three write paths, its row-before-the-question rule, its never-edited coverage and its conditional supersession; §11's rendered-from-the-proposed-row and transcription rules, its `CoverageView`, its possibly-empty coverage, its renders-no-internal-value bar, its listing, its revocation surface and `TurnOutcome.authorizations`; and §16's lane attribution and transcribe-the-ruling-whole rule. Nothing else in this ADR) and ADR-0268 (**five scopes.** **§1's `AuthorizationDisposition` clause**, in its closure and its retired enumeration: *"closed at exactly **six** members"* becomes **seven**, gaining **`GOAL_CLOSED`** — valued by lower-cased member name, meaning *this row was ended by the ending a closure of its goal takes over the rows standing when it is taken*, and **retired**, no edge leaving it — and *"`DECLINED`, `EXPIRED`, `REVOKED` and `SUPERSEDED` are retired: no edge leaves them"* enumerates four where there are now five. §1's own *"The vocabulary is added to and never renamed"* is the licence and is unmoved. **§1's transition graph, in its edge count, and §1's `PROPOSED`-row clause, in the settlers it enumerates.** *"stated whole, and there are exactly **five** edges"* becomes **seven**, the new ones being `PROPOSED → GOAL_CLOSED` and `ESTABLISHED → GOAL_CLOSED`, both written by `end_for_goal` — by a closing act of the goal, or by a reopen taking the ending a closure this store was never told of never took; its two other assertions stand verbatim — `PROPOSED` and `ESTABLISHED` stay the two members an edge leaves, and no other edge exists — and `settle`'s own clause stands but for the count, `AuthorizationSettlement` unmoved at four members and `WOULD_DUPLICATE` unreachable on either new edge. And §1's clause that an expired proposal is settled *"by the **first operation that reads it**, and there are exactly **two** — a `live_for` read, and the answer that names it"* gains a **third** settler in `end_for_goal`, which settles it `GOAL_CLOSED` rather than `EXPIRED`; **that clause's negative limb is untouched and is what makes the addition admissible** — *"**No sweep, no timer, no reclaim and no start-up scan** settles it"* stays true word for word, the act taking that ending being none of the four but a user act's own write — a closing act, or the reopen — and its *"a `PROPOSED` row neither operation reads again stays `PROPOSED`"* is narrowed to a goal that does not close. §1's field list, its three write paths, its one-`ESTABLISHED`-row uniqueness rule, its conditional supersession and its liveness predicate are untouched. **§1's settlement clause, in its single-operation framing alone.** *"the settlement, and it is the store's single operation"* and *"**There is no other mutation and no `update`**"* are read as a closure over what mutates this store, and it gains two members — `end_for_goal`, which settles rows and raises the closure record, and `clear_closure`, which lifts that record's fence. **What that clause rules still binds verbatim**: `settle` moves one row along one edge under compare-and-swap and refuses every move that is not an edge; a settlement still moves **one field and its instant and nothing else**, and a row's coverage, basis, account, destinations and expiry are **never edited** — true of both new members, `end_for_goal` writing `disposition` and `settled_at` alone and `clear_closure` writing no row at all; and **there is still no `update`**. A reader holding only §1 authors a store whose one mutating operation is `settle`, and has nowhere to put an ending. **§16's store roster, in the signature count, and §16's `InvalidAuthorizationError` clause, in its enumeration of refused writes.** The **eight** signatures stated *"in full, because a roster of names is not a contract"* become **ten**, gaining `end_for_goal(goal, /, *, at, goal_version) -> int` — which in one indivisible step settles every row of a goal standing `PROPOSED` or `ESTABLISHED` to `GOAL_CLOSED`, fences the goal in this store at that version, and answers how many rows it moved, reading no clock, evaluating no liveness and doing none of it where the store's record already stands higher — and `clear_closure(goal, /, *, goal_version) -> bool`, which lifts that fence where the record stands at or below the version passed, removes no record and settles nothing. And the list of writes `record` refuses gains one further entry, a row whose `goal` the store holds fenced, refused in the same indivisible step as the write; **that class is reused and no class is minted**, `core/errors.py` and `AuthorizationError` untouched, and `settle` gains no conjunct, a settlement naming a row the ending settled answering `NOT_AT_SOURCE` on the ratified graph. `GoalAuthorizations` and `AuthorizationResolution` gain **nothing**, so §16's three-faces construction stands entire, as do its detached-snapshot rule, its clock disciplines, its `standing` clause, its `settle`-outcomes clause, its data-rights, `export` and `clear` clauses and its `PROTOCOL_VERSION` clause — which this decision does not fire, no `Authorization` crossing whole and neither new member being promoted. §16's `core/types.py`, `core/errors.py` and `PermissionDecision` rosters take **no scope**: no type, no field and no error class is added. **And §20, in two limbs**: Lane 1's *"The **eight** store signatures"*, which goes with §16's roster and becomes ten; and **arms 37 and 55**, each in its edge count and its retired-disposition enumeration, which now run over seven edges and five retired dispositions, **and arm 37 in one further limb**, its *"a row read after its `expires_at` while still `PROPOSED` is settled `EXPIRED` by a `live_for` read and by the answer that names it, **and by no other operation**"*, whose last limb is false of the closing act. Every other assertion of both stands verbatim, their subject being `settle`; every other arm is untouched, arm 4's revocation-between-`live_for`-and-`record` case conspicuously so; and §20's lane cut and its wire clause bind entire. **§§2-15 and §§17-22 stand as they are, but for the limbs of §1, §16 and §20 named above**, and several are what the superseding decision rests on: §7's route-(d) invariant needs no conjunct, its own ground — *"every other disposition is retired and none of them is live"* — being true of the new member; §11's listing, announcement, projection and revocation surfaces gain nothing, and a row ended under the new member simply leaves the listing as a revoked one does; §12's expiry-taken-once and no-deletion rules bind entire and **no instant is moved**; §13's recheck at `decide` and its stated residual are unnarrowed; §15's writer clauses reach both new members unchanged; and §19's entries are each untouched)
- Date: 2026-09-12
- **Partially superseded: 2026-09-12 by ADR-0255 — §14's where-phase-4-leaves-an-attempt
  enumeration, in its two-case shape alone. Nothing else in this ADR.** That enumeration gains a
  third case: a check **of a step** at least one of whose operands the same plan will produce
  before that step is
  dispatched — a step earlier in `steps` not yet disposed of, or an interpretation of that plan
  not yet performed, a `record`-backed one included — and **every one of whose operands already
  available at phase 4 is satisfied**, is **deferred** rather than failed, so it
  neither blocks the advance to `AttemptPhase.EXECUTE` nor triggers a replan. **A known failure
  dominates a deferral**: a check with an operand available now and failing now stays a failed
  check whatever else it waits on, and §14's replan limb binds it unchanged. **The attempt
  advances where every check either passed or was deferred.** Without it every plan carrying a
  `depends_on` replans forever, because a dependent step's dependency and condition are
  unsatisfied at phase 4 for every such plan by construction. **§14's four checks and their order,
  its no-fifth-check rule, its uncovered-arguments limb and `CONFIRM` park, and its failed-check
  limb for every check that is not deferred each bind verbatim**, and §13's recheck-at-`decide`
  with its no-cached-verdict rule is relied on as the reason the same predicates are evaluated
  again at each dispatch.
- **Partially superseded: 2026-09-12 by ADR-0255 — §17's carried-and-discharged sentence, in the
  discharge half alone. Nothing else in this ADR.** *"it is carried by A5, A6 and A7 and
  **discharged by A8, A9 and A10**"* gains **two further** conditions, both stated by that
  decision's §13. The first is the **evidence-to-claim window** its §1 states — a concurrent
  turn's ADR-0252 §8 refresh superseding a `STANDING` row between a step's evaluation and its
  committed claim — which must be **closed**, and which is assigned to none of A8, A9 and A10.
  The second is its §3's **resolved-but-unapplied answer**, which must be **durably
  recoverable**: a confirmation §4 of *this* ADR's own protocol resolves at step 5, whose claim
  that decision's §3 then refuses at step 6, leaving the step `AWAITING_APPROVAL` with the
  approval spent. That one its §12 assigns to **A8**, but it is no part of the **reconciliation**
  guarantee the gate names, so A8's landing does not by itself discharge it. A reader holding only
  §17 would read those three guarantees landing as releasing a deployment §13 still refuses, on
  either condition. **§17's rule itself is untouched and is quoted verbatim** — the verification,
  uncertain-outcome and cancellation guarantees, the milestone clause and the *"wiring one is what
  this rule binds"* sentence — as is the **carry** half naming A5, A6 and A7 and §17's second
  normative clause that this decision's own lane dispatches nothing and wires nothing.
- **Partially superseded: 2026-09-15 by ADR-0266 — §1 in four limbs, §2's member shape and two
  further limbs of its `MONEY` currency clause, §3's condition 6 and field count, §4's `MONEY`
  currency conjunct and its two `maximum` inequality alone — its `minimum` conjunct is unmoved —
  §5's widening clause in two limbs, §6's lineage-discharge clause in the narrowing direction, §9
  clause (ii)'s test of *"bear on"*, §9 clause (iii)'s *"an argument no member names"* limb, §8's
  and §10's resolution enumeration in its closure at three, §11's `CoverageView` field list,
  §16's roster in three limbs, and §20's arm enumeration. Twelve scopes, and nothing else in this
  ADR.** No clause of this
  decision states **how a recorded span is associated with an argument key**, nor **how a
  member's shape is chosen** — §10's three resolutions turn a span into a value and name no
  argument, §9 clause (ii) states the property and not the procedure, §8's basis names no
  argument, §1's path (iii) points at an element of the goal's `constraints` which carries no
  typed value and no argument key, and §9's no-model clause forecloses the planner.

  **The superseding decision answers it by removing the question from the record, moving where
  the bound is proved, and settling the reading with the user rather than with a rule.** A
  `CoverageMember` carries a **`kind`** and no `argument`, so it states what the user's words
  fixed or bounded and nothing about which slot carries it. The owner's ruling of 2026-09-14 then
  rules that a price is *"usually a consequence of the chosen params, not an argument"*: so the
  **investigation** records the quote for the goal's intended action, over that step's own
  user-facing arguments, and the **authorisation** phase proves the bound against that quote
  before execution. A declaration **may** declare an argument at a kind, and then that comparison
  is taken **as well**, a `MONEY` member needing the quote in every case because a `max_price`
  filter and a transfer amount are not the charge. *"Bears on"* is established by **kind
  agreement** between the constraint and the thing it is proved against, and never by numeric
  fit: a constraint about a hotel's star rating has no `MONEY` reading, so it is proved against
  no price.

  **And what a span mints is a `PROPOSED` member, not an authority.** A fourth `ResolutionRule`,
  `STATED_BOUND`, reads a ceiling off the element's span by a closed table of four forms, and the
  row carrying it is written `PROPOSED` and **rendered to the user by §11's projection — the
  kind, the ceiling, its exclusivity, the horizon and `CoverageView`'s own `span` — before the
  question is put**, the answer establishing it or declining it on §1's own edges. **The
  authority is the assent to the rendered text and never the span**, so a reading that took the
  polarity of *"avoid booking hotels under 100 euros"* wrongly is a ceiling the user is shown and
  refuses rather than one the record asserts; no rule of that decision inspects the text around a
  span, and none needs to. **Because §1 proposes a row only where its coverage would be complete
  — restated over condition 6, whose first conjunct is that every member is met, and a `MONEY`
  member is met only against a quote — the phases are ordered: the investigation quotes, then the
  authorisation phase proposes and asks, then execution.** The owner's worked case is one
  question: *"Book Riverside if it is dry Saturday, up to 150 euros"* → quote `120` → confirm the
  ceiling once → book; *"make it Sunday"* → re-quote `135`, inside the ceiling, **no new prompt**.

  **§1's third proposal condition** is restated over condition 6 for that reason, and **path
  (ii)'s and path (iii)'s triggers** are restated over the kind, path (iii) losing its
  per-declaration member selection because the mint reads the goal and no declaration — so this
  ADR's own *"up to fifty pounds for the train and a hundred for the hotel"* illustration mints
  nothing, two `MONEY` constraints of one goal minting neither. **Path (iii) takes no
  `STATED_BOUND` member at all**, writing `ESTABLISHED` with no question put, which adds no
  refusal to §10's three ratified resolutions and leaves that path exactly as inert as it is
  today.

  **§2** loses `argument` and its depth-one clause — restated on the new `BoundedArgument` —
  turns its no-two-members rule into one member per kind, loses `currency_argument` and gains
  `maximum_exclusive` on a `MONEY` bound. **Two further limbs of its `MONEY` currency clause go
  with that field**: the cross-member refusal that a `fixed` member naming the bound's
  `currency_argument` equals its `currency` is not constructible once no member names an
  argument, and *"A call whose declaration carries no currency argument takes no `MONEY` bound"*
  is **reversed** — such a call takes one and proves it against the quote, refusing it being the
  failure the owner's ruling names. The no-inference limb is kept one field over.

  **§3's condition 6** becomes two routes and three conjuncts, keeping both of its directions;
  the validator refusing a member that names a system-supplied argument goes with the field it
  read and its rule is preserved by a refusal on `bounded_arguments`; and `ToolDefinition` gains
  one further field, so *"gains one field"* is over-narrow by one. **§4's currency conjunct** is
  read off the quote or the declaration, and its `maximum` inequality is read strictly where
  `maximum_exclusive` is set. **§5's widening clause** gains the ordering that ADR-0266 §3 makes
  necessary — at equal `maximum`, setting the flag narrows and **clearing it widens**, so a
  clearing is a widening path (ii) refuses — and its *"naming an argument no member covers"* limb
  is restated over condition 6. **§6's lineage discharge** is available only where every
  user-facing argument is covered on the **argument** route, a digest proving the call is the one
  quoted rather than one the user bounded; §6's bar takes no scope of its own, being stated over
  condition 6 by name.

  **§9 clause (ii)** is given its mechanical test. **§9 clause (iii)'s *"an argument no member
  names"* limb** is restated as *an argument the row does not cover under condition 6*, both
  times it is stated, since a member names no argument and route (d) would otherwise be
  unreachable altogether — the mirror of §1's inert proposal. **§8's and §10's closure at three**
  admits the fourth rule §19 books by name. **§11's `CoverageView`** carries `kind` in place of
  its required `argument`, which could otherwise be transcribed from nothing, leaving no
  projection constructible for any member and the path-(i) confirmation unrenderable; its
  required `span` is unchanged and is what the new authority rests on. **§16's roster** gains a
  fourteenth type and a sixth and seventh field, and its *"`PermissionDecision` gains no field"*
  gains `intended_action` — transcribed by `from_request`, compared by a sixth conjunct of
  `authorises`, and distinguished from the `ActionRequest.goal` that clause declines precisely
  because it **is** a value the trail can compare against the arguments: it selects the quote the
  coverage was proved through.

  **§20's arm enumeration is superseded by a decidable test rather than by a list**, because three
  review rounds each found a list incomplete. The scope is every arm of the seventy-one an
  implementation of the new contract cannot satisfy, and **six mechanisms decide it**, applied arm
  by arm by the lane that owns the arm: a member *naming an argument* or condition 6 over key sets;
  a bound's `currency_argument` and its cross-member refusal; two members of one **kind** or a
  `fixed` value the new kind validation refuses; a `MONEY` member treated as **met without a
  quote**, or `ResolutionRule`'s closure at three; a **path-(iii) opening act carrying a member the new reading mints**; and an act stating a **money
  bound** whose own recorded words that reading's four-form table does not match — which reaches
  **no** act a ratified resolution mints, arm 14's Sunday correction and arm 47's added term among
  them. **For five of the six the property each arm demonstrates is unchanged and only its value or its act
  moves, and no arm is dropped** — the **covered** limbs of the quote mechanism's arms riding with
  the decision that lands the carrier. **For the path-(iii) one it is a supersession
  outright**: arm 64's *"a row is written `ESTABLISHED` … with no `CONFIRM` put at all"* and arm
  65's *"never asked to repeat the bound"* do not survive for a member that reading mints, while
  the whole structure of both stands for an opening act minted by §10's three ratified resolutions.
  **The cleared arms are recorded with their ground**: arm 5 stands verbatim; arms 21 and 35 state
  negative assertions that stay true; arms 6, 11 and 12 test §4's readings, unmoved; and `TERMS`
  and `PERIOD` members stay constructible.

  **Every other clause of the twelve sections binds entire.** §1's three write paths themselves,
  its row written before the question is put, its never-edited coverage, its conditional
  supersession and its other three proposal conditions stand exactly as ratified. §3's condition
  3, comparing the request's `tool` against the row's **by value**, is relied on and is why a
  kind-typed member is still not portable across tools. §13 is relied on entire and is what the
  new comparison is taken under — at `ActionPolicy.decide`, on the concrete request, at every
  dispatch, with no cached verdict — and its *"Coverage and sufficiency are two tests and neither
  clears the other"* stays true: a standing quote satisfies no condition of a step. §9's clause
  (i) and every limb of clause (iii) but the one narrowed above, §2's two-shapes validator and
  `BoundKind`, §3's conditions 1-5 and its classification, §4's totality and its no-schema
  clause, §5's other limbs, §8's basis, §10's three ratified resolutions, §11's every other
  clause, §15's writer clauses and §16's `PROTOCOL_VERSION` rule are each relied on rather than
  moved, and §20's lane cut, its wire clause and its dependency clause bind entire. §7, §12, §14, §§17-19,
  §21 and §22 are not reached at all.
- **Partially superseded: 2026-09-15 by ADR-0267 — §1's `Authorization` field list, §11's
  `AuthorizationProjection` field list, and §16's `core`-surface roster in two limbs. Nothing else
  in this ADR.**

  **§1.** The row gains `quoted`, an `ActionQuote | None` defaulting to `None`: the governing quote
  for the request's intended action **in the goal read the row was built from** — a fact about that
  one read and not about the instant of persistence, so a quote appended between the read and the
  write leaves the field carrying the one the read returned — written once by the same
  `orchestration` code that builds the row and never edited afterwards. **It is written on path (i)
  alone**: paths (ii) and (iii) write `ESTABLISHED` directly and put no question, so there is no
  rendering anyone was shown and no figure an answer was taken over, and a correction does not
  transcribe its predecessor's. It is on the row for §1's own reason for writing the row before the
  question — what the user is shown is a rendering of a
  durable record — and the figure is part of what they answered. It is **provenance**: the recheck
  at dispatch and the evidence route each read the **current** governing quote through the new
  seam, never this field, and it sits outside `_AUTHORIZATION_SUBJECT` so §7's recompute parity is
  untouched.

  **§11.** The projection gains `quote`, a `QuoteView | None` required with no default, carrying
  that quote's amount, currency and read instant and **transcribed from the row** — so the
  rendered-from-the-proposed-row rule, the transcription rule and the restart clause each bind
  exactly as written, a recovery rendering the same projection value for value.
