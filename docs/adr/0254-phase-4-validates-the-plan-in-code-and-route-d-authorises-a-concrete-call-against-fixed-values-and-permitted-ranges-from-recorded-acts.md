...
- **Partially superseded: 2026-09-15 by ADR-0267 — §11's `AuthorizationProjection` field list
  and one limb of its restart clause, and §16's `core`-surface roster in two limbs. Nothing else
  in this ADR.**

  **§11.** The projection gains `quote`, a `QuoteView | None` required with no default, carrying
  the amount, the currency and the read instant of the governing quote the proposed row's `MONEY`
  member was met through, and **absent** where the row carries no `MONEY` member or where the
  member was met on the argument route alone. It is **transcribed** from the goal's own record by
  the engine and re-selected by no surface, on this section's own transcription rule. And because
  the quote lives on the `Goal` rather than on the row, a restart after a refresh landed renders
  the **refreshed** figure while the coverage, the bounds and the `expires_at` are unchanged —
  which §13's recheck makes the only honest direction, since the ruling at dispatch is taken
  against that same later quote.

  **§16.** The roster grows by four types and four fields. Every other clause of both sections
