...
- **Partially superseded: 2026-09-13 by ADR-0266 — §3's `ToolDefinition` field count, and
  §3's `system_supplied` member type with the three examples beside it. Nothing else in this
  ADR.** No clause of this decision states **how a recorded span is associated with an argument
  key** of a declaration, nor **how a member's shape is chosen** — §10's three resolutions turn a
  span into a value and select no span and name no argument, §9 clause (ii) states the property
  and not the procedure, §8's basis names no argument, §1's path (iii) points at an element of
  the goal's `constraints` which carries no typed value and no argument key, and §9's no-model
  clause forecloses the planner. So `ToolDefinition` gains a **second** field,
  `bounded_arguments`, naming per key the `BoundKind` of range it takes and, on `MONEY`, the key
  carrying its currency: that is the one declared source the association rule has for a **range**,
  and §4's *"no lane reads a `parameters_schema` keyword to establish an association this
  decision requires a row to name"* is what makes a schema keyword unavailable for it. **And
  `system_supplied`'s members become `SystemSuppliedArgument`s carrying a `SystemSuppliedKind`**,
  because the fill half of §3 cannot be written over bare key names — the idempotency key, the
  client reference and the locale this section names as examples are indistinguishable to a
  filler. The **idempotency key ceases to be one of those examples**: ADR-0029 §5 derives it onto
  `ToolCall.idempotency_key` from the ruling's own id, which does not exist at the moment this
  section requires the fill to happen, and a tool whose upstream wants it in the payload maps it
  inside the integration as that contract already requires.

  **Every other clause of §3 binds entire**, and the superseding decision rests on most of them:
  the classification and its user-facing default, the empty-tuple default and its argument, the
  validator refusing a coverage member for a system-supplied argument, the refusal of a plan step
  whose own arguments name such a key, the fill-before-the-fit-test clause, the
  `parameters_digest` clause and the monotonicity limb, and the no-schema-keyword clause. **§§1,
  2, 4–22 stand as they are** and several are read rather than moved: §1's three write paths, its
  write-before-the-question rule and its never-edited coverage; §2's two shapes, its depth-one
  rule and its `currency_argument` clause; §4's readings; §8's basis; §9's three clauses and its
  discard clause; §10's three resolutions and its *"A resolution the loop cannot take is not
  taken, and no member is minted"*; and §15's writer clauses.
