...
- Status: Partially superseded by ADR-0256 (seven limbs, all about one field, `Authorization.expires_at`. **§12's ladder rung 3 entire**, its *"no row at all"* and its *"Nothing is invented, nothing is defaulted and nothing falls back to a configuration"* included: where the recorded act states no instant and the goal carries no `deadline`, `expires_at` is now the **earliest instant among the recorded turns the row's coverage rests on** advanced by the deployment's turn-retention window and a row **is** written, where that window is finite and that sum is strictly after `proposed_at`. **§12's stated cost of that rung**, *"a goal carrying no `deadline` whose user stated no horizon has no route (d) at all, and every call of it asks"*, which is over-wide as it stands: it is true only where the deployment is set to keep turns forever, and there the ratified rung stands verbatim as the ladder's last. **§12's no-deployment-wide-expiry clause, in the single respect that the third rung reads one deployment value, the turn-retention setting**, together with **§12's justification clause in its limb *"and in neither case by a deployment"***; every other limb of both stands — no `Settings` **field** is added, the expiry is still per `Authorization` and shown when it is granted, and no figure bounds rung 1 or rung 2. **§12's path-(ii) transcription clause, §1's path-(ii) *"transcribed unchanged"* list and §1's path-(i) *"the only path that may … set `expires_at`"***, each in the `expires_at` limb alone and in the **narrowing direction alone**: a correcting act whose own span states an instant strictly after the new row's `proposed_at` and strictly before the superseded row's `expires_at` carries that instant, and in every other case the expiry is transcribed unchanged — nothing lengthens one on any path but (i), so *"No sequence of corrections outlives the confirmation that began it"* holds a fortiori. **§9 clause (ii)'s bar limb *"move `expires_at`"***, in that same narrowing direction alone, on that clause's own stated principle that *"The interpretation narrows what the act covers and can never widen it"*. **§9 clause (iii)'s *"not taken"* half, only as §12 reads that clause onto an `expires_at` and only on a path-(ii) correction**, where an instant the correction's span does not settle to one value is now resolved to the earliest admissible reading rather than discarded; clause (iii)'s asking limb is **not** superseded and loses no sentence, it being the consequence of the discarding and there being none in that scope, and its asking for every coverage member, its consequence-outside-scope limb and the *"not taken"* half everywhere else all stand entire. And **§19's entry naming *"What the user is shown where §12's ladder yields no instant"***, **narrowed in application and not replaced**: the case it books now arises only where the deployment keeps turns forever, and the decision it names — asking *"until when?"* at the act — is still booked and still not taken. Every other clause of §12 binds entire and several are what the replacement rests on: the expiry taken once at the write and never recomputed, an answer arriving at or after it establishing nothing, the recipient-grant and configured-provider exclusions, the visible-at-the-act rule and the no-deletion rule. §§1–11 and §§13–22 stand as they are, but for the limbs of §1 and §9 named above.)
- Date: 2026-09-12
