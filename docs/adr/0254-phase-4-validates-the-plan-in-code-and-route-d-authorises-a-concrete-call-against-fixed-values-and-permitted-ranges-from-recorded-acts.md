...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1 in four limbs, §2's member shape and two
  ...
  **§20's arm enumeration is superseded by a decidable test rather than by a list**, because three
  ...
  quote**, or `ResolutionRule`'s closure at three; a **path-(iii) opening act carrying a member the
  new reading mints**; and an act whose **own recorded words that reading's four-form table does not
  match**. **For five of the six the property each arm demonstrates is unchanged and only its value
  or its act moves, and no arm is dropped.** **For the path-(iii) one it is a supersession
