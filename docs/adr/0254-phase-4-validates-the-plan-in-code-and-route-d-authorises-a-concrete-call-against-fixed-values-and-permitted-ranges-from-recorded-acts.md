...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1's proposal-completeness condition, §2's
  ...
  **The superseding decision answers it by removing the question from the record and then moving
  ...
  and then that comparison is taken **as well**; each endpoint names exactly one route, a `MONEY`
  **maximum** needing the quote and a `MONEY` **minimum** the declared argument, because a
  `max_price` filter and a transfer amount are not the charge. *"Bears on"* is
  ...
  **Three consequences of that shape reach this section's own words.** `maximum` stops being
  required, because a stated **floor** — *"at least 150"* — mints a `minimum` and no `maximum`,
  and the always-a-maximum reading would authorise below the floor the user set. A `MONEY`
  bound stops carrying `currency_argument`, because the key carrying an amount's currency is a
  fact about a declaration or about a quote and not about an act; and each endpoint gains an
  **exclusivity flag**, so *"under 100"* stops being recorded as a bound a call at exactly 100
  satisfies and §4's two inequalities are read strictly where it is set. And `ResolutionRule` gains a
  **fourth** member, `STATED_BOUND`, reading a bound off the span by a closed table of direction
  forms whose negation sits adjacent to its comparator — **the supersession §19 books by name**,
  fired there because a direction is in the user's words or it is nowhere.
  ...
  **Every other clause of the seven sections binds entire.** §1 is untouched but for its third
  ...
  ratified, and the completeness condition is restated over the new test rather than dropped. §3's condition 3, comparing the request's `tool` against the
  row's **by value**, is relied on and is why a kind-typed member is still not portable across
  tools. §13 is relied on entire and is what the new comparison is taken under — at
