...
- **Partially superseded: 2026-09-13 by ADR-0264 — §6's first clause, in its second
  ...
  **The sentence that moves.** §6's first clause reads *"On a turn in which **at least one
  ...
  given nothing, and the assembled prompt is byte-identical to what it is today."* ADR-0264
  §6 gives the composing stage one fixed fragment on **every pass that composes a reply other
  than a routed one**, so the second sentence is false of every one of the "every other"
  turns that composes unrouted, and not of some of them. A routed pass keeps the
  byte-identity, which is what leaves ADR-0197 §6's two-input closure standing. Two of those turns carry no `SearchNotServiced` member at all and are what make the
  falsification unarguable: a search that reached the provider and was answered records no
  disposition — `SearchRefusal.NO_RESULT` maps to none (ADR-0231 §13), which §6's third clause
  states in terms — and a turn that asked for no search records nothing either, which is the
  turn issue #2365 observed claiming a lookup that never happened.
