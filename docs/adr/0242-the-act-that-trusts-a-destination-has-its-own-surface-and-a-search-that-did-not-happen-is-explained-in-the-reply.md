...
- **Partially superseded: 2026-09-13 by ADR-0264 — §6's first clause, in its second
  ...
  **The sentence that moves.** §6's first clause reads *"On a turn in which **at least one
  ...
  §6 gives the composing stage one fixed fragment on **every pass that composes a reply**, so
  the second sentence is false of every one of the "every other" turns and not of some of
  them. Two of those turns carry no `SearchNotServiced` member at all and are what make the
