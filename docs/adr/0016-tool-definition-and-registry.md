...
- Status: Accepted, partially superseded by ADR-0018 and ADR-0254 (two scopes, both in §1. **Its `ToolDefinition` model declaration, in the field list**: the model gains `system_supplied`, a possibly-empty, duplicate-free `tuple[EncodableText, ...]` defaulting to the empty tuple, naming the keys of `parameters` the **system** fills — an idempotency key, a client reference, a locale — so that a goal-scoped authorization record needs coverage for the **user-facing** arguments alone and an implementation choice never becomes a question put to the user. A reader holding only §1 authors a definition that never carries one and does not conform. And **its required-field clause, in the application to that one field alone**: *"Every field that a permission decision depends on is required"* is unconditional, and a permission decision does depend on this one, since it moves the coverage comparison and the argument-authority bar — so the empty-tuple default is an exception to that clause and is recorded as one rather than argued away. **The grounds are the clause's own reason, which does not reach this default**: it forbids a default because *"A default is a claim"* and the empty tuple for `reads`/`writes` is *"exactly the false statement a forgetful integration author would ship"*, whereas here the empty tuple makes the **opposite** claim — every argument is user-facing and every argument needs coverage — so a declaration that says nothing asks more often rather than less, and a value the model chose is refused rather than confirmed. Making the field required was the alternative and is declined, because it would oblige every declaration and every fixture to write an empty tuple for a fact empty on almost all of them with no return in the direction the clause protects. **The exception is this one field on this argument and no lane reads it as licence to default a second safety field.** Every other clause of §1 binds entire: every other field a permission decision depends on stays required, `frozen=True` and its audit-record argument stand, `description`'s non-blank refusal stands, and no lane reads this as licence to default a safety field. §§2-7 are untouched, and §4's `parameters_schema` declaration is relied on rather than moved — the classification is a field **beside** the schema and never a keyword inside it, because a dialect that ignores keywords it does not know would drop one silently) and ADR-0266 (one scope, in §1, and it is the scope already recorded here reaching one further field. **The `ToolDefinition` model declaration, and the required-field clause in the application to `bounded_arguments` alone.** The model gains `bounded_arguments`, a possibly-empty, duplicate-free tuple defaulting to the empty tuple, each member naming one key of `parameters` at depth one, the kind of value that key takes — an amount, a period or a named term — and, on an amount alone, the key carrying its currency. A goal-scoped authorization's coverage members now record **what the user stated** and carry no argument key at all, so what a member is proved against is decided at the **comparison**. The primary route is the **quote** the investigation recorded for the goal's intended action, which needs no declaration at all — a booking tool takes a site, dates and a party size, and the price is a consequence of those rather than an argument. This declaration carries the **second** route: where it declares an argument at a kind, that argument is compared against the member of that kind as well, which is how a `max_price` filter or a transfer amount is held to the user's bound. Without the field that route does not exist for any declaration, and a reader holding only §1 authors a definition no member can ever be held against. **And the required-field clause takes a second recorded exception**: *"Every field that a permission decision depends on is required"* is unconditional, and a permission decision does depend on this one, since it is the whole of the argument route's association between a recorded act and a call's arguments. **The grounds are stated exactly, because the obvious one is false.** The empty tuple does **not** make the opposite claim in every direction: the declared-argument comparison is an **additional** safeguard over a bound the quote already proves, so a declaration that declares nothing still has its user's ceiling proved — against the quote for the intended action — and only forgoes the second check. What it costs is real and is recorded rather than argued away: an amount-bearing argument nobody declared is compared against no member, so a quote that measures a component rather than the whole charge is caught by nothing. That is an integration-declaration defect of the class a wrong `risk_level` already is, and declaring the argument is the safeguard against it. **The exception is this one further field on this one argument**, and no lane reads the two records together as licence to default a third safety field. **Where a quote is carried and what records one is decided nowhere in that ADR** — it states the four facts its comparison reads and books the carrier — so no field of this declaration is owed for it here. Every other clause of §1 binds entire: every other field a permission decision depends on stays required with no default, `frozen=True` and its audit-record argument stand, and `description`'s non-blank refusal stands. §§2-7 are untouched, and §4's `parameters_schema` declaration is relied on rather than moved — the new declaration is a field **beside** that schema and never a keyword inside it, for the hazard already recorded here) and ADR-0267 (one scope, in §1, and it is the scope twice recorded above reaching one further field: the `ToolDefinition` model declaration and the required-field clause applied to `quoted_output`, a `QuotedOutput | None` defaulting to `None` and naming, at depth one, the output keys a price and its currency are read at. A reader holding only §1 authors a definition that names no such output, so no act under it is ever quoted and every `MONEY` ceiling that would authorise one asks. **The exception is taken on §1's own test applied afresh** and not on the two records beside it: absent makes the **opposite** claim to the one §1 refuses, a declaration naming no quoted output producing no quote at all, so the default costs a question and can never authorise a call — the one direction §1 exists to protect. Requiring the field would oblige every declaration and fixture in the tree to write `None` for a fact absent on almost all of them and would refuse every definition written before it. The exception is this one further field, and every other clause of §1 binds entire) and ADR-0262 (one scope, in §1, and it is the scope three records already take here reaching one further field. **The `ToolDefinition` model declaration, and the required-field clause in the application to `postconditions` alone.** The model gains `postconditions`, a possibly-empty `tuple[StepVerification, ...]` defaulting to the empty tuple, declared by whoever registers the tool and stating what a **successful** invocation establishes about its own output — a reservation id is present, a `status` equals `confirmed`. That decision's verification against a goal's criteria holds an act's stored output against the **whole** tuple the pinned definition carries — every member must hold, none is selected and none is excused — and authors no predicate of its own, so a reader holding only §1 authors a definition against which nothing can be held and every criterion resting on that tool is unestablished for want of an operand. **And the required-field clause takes a fourth recorded exception, on its own fail-closed ground and not on any recorded here already**: *"Every field that a permission decision depends on is required"* is unconditional, and this field **is** one such — a goal-scoped authorization compares the request's declaration with the row's **by value**, so a `postconditions` edit moves a coverage answer exactly as a severity or schema edit does, and a call built under the edited declaration asks. What makes the empty tuple an exception is that it can only **refuse** to establish: a declaration that declares nothing verifies nothing, no criterion resting on that tool is ever met, and such a goal never reaches `GoalStatus.ACHIEVED` at that decision's rung 2. **The exception is this one further field on this one argument**, and no lane reads the four records together as licence to default a safety field. Every other clause of §1 binds entire: every other field a permission decision depends on stays required with no default, `frozen=True` and its audit-record argument stand, and `description`'s non-blank refusal stands. §2's ordering of the declarations is read by that decision's strength ladder and moved by nothing, and §§3-7 are untouched — §4's `parameters_schema` declaration included, the new declaration being a field **beside** that schema and never a keyword inside it, for the hazard already recorded here) and ADR-0271 (one scope, in §1, and it is the scope four records already take there reaching one further field: the `ToolDefinition` model declaration and the required-field clause, in the application to `charged_output` alone — a `ChargedOutput | None` defaulting to `None`, naming at depth one the key of an invocation's output carrying the whole amount it charged and the key carrying that amount's ISO-4217 code, which is the operand that decision's post-hoc comparison of a charge against the quote its dispatch was proved against reads; a reader holding only §1 authors a definition whose acts report no charge, so no call under such a declaration is ever satisfying, no `MONEY` criterion every call of which runs under one is ever met, and no goal resting on such a criterion reaches ACHIEVED. The default is a fifth recorded exception to the required-field clause, taken because absent makes the opposite claim to the one §1 refuses. The last dated note below states it in full)
- Date: 2026-07-19
...
- Partially superseded: 2026-07-19 by ADR-0018 — five clauses, found by the first
  ...
  snapshot; what a registry stores must be valid and detached; and the spent-id
...
- **Partially superseded: 2026-09-12 by ADR-0254 — §1's `ToolDefinition` model
  declaration, in the field list alone. Nothing else in this ADR.** ADR-0254 §3 gives a
  goal-scoped authorization record coverage members over a call's arguments, and its §6
  refuses every standing route where a live record names an argument the request carries.
  Without a way to tell an argument the **user** chooses from one the **caller** supplies,
  a declaration carrying an idempotency key, a client reference or a locale would be one
  whose calls always ask — or, worse, one whose user is asked to approve an idempotency
  key. So `ToolDefinition` gains **`system_supplied: tuple[EncodableText, ...]`**,
  ...
  **And a system-supplied argument is the system's to fill.** ADR-0254 §3 rules that
  ...
  arguments name one is **refused before the ruling** — the step does not dispatch and no
  ...
  **Everything else stands entire**: every **other** field a permission decision depends
  ...
  beside that schema rather than a keyword inside it — ADR-0145 §5's own hazard, that
  draft 2020-12 *"ignores keywords it does not know"*.

- **Partially superseded: 2026-09-15 by ADR-0266 — §1's `ToolDefinition` model declaration,
  in the field list alone, and the required-field clause in the application to that one further
  field. Nothing else in this ADR.** The decision above gave a goal-scoped authorization record
  coverage members over a call's arguments; ADR-0266 decides what such a member **records** and
  what it **meets**. A member carries a **kind** and **no argument key** — it states what the
  user's own words fixed or bounded, and nothing about which slot carries it — so what it is
  proved against is decided at the **comparison**. Its **primary** route needs no declaration:
  the bound is proved against the quote the investigation recorded for the goal's intended
  action, over the very arguments the call will carry. This declaration carries the **second**
  route, which applies **as well** wherever it exists: an argument is covered where its
  declaration declares it at the member's kind, the record carries exactly one member of that
  kind, and the argument satisfies it. That is how a `max_price` filter or a transfer amount is
  held to the user's bound — a filter is not a charge, so it never stands in for the quote — and
  it is why the declaration must carry the fact: nothing else in this system says which argument
  of a tool takes an amount, which takes a period, and which key carries an amount's currency. So
  `ToolDefinition` gains **`bounded_arguments`**, a possibly-empty, duplicate-free tuple of
  `BoundedArgument` defaulting to the empty tuple, each naming a key of `parameters` at depth
  one, the `BoundKind` it takes and, on `MONEY` alone, its `currency_argument`. It names no key
  of that declaration's own `system_supplied`, refused at construction — which is what preserves
  ...
  **This declaration is owed nothing for the primary route, and that is deliberate.** ADR-0266
  states the four facts its comparison reads off a quote — the intended action it was taken for,
  the arguments it was quoted over, an amount with its currency, and the output it was read from —
  and states **no carrier** for them: where a quote lives and what records one are booked to a
  later decision. So no field of `ToolDefinition` is owed here for the quote, and a lane reading
  this record authors exactly one new field.
  ...
  **The second scope: the required-field clause, for this one further field, recorded as an
  ...
  the whole of the argument route's association between a recorded act and a call's arguments.
  **The grounds are stated exactly, because the obvious one is false.** The empty tuple does not
  make the opposite claim in every direction: the declared-argument comparison is an
  **additional** safeguard over a bound the **quote** already proves, so a declaration declaring
  nothing still has its user's ceiling proved and only forgoes the second check. **What that costs
  is recorded rather than argued away**: an amount-bearing argument nobody declared is compared
  against no member, so a quote measuring a component rather than the whole charge — a €1 fee for
  a €200 transfer — is caught by nothing. That is an integration-declaration defect of the class
  a wrong `risk_level` already is, and declaring the argument is the safeguard against it. The
  exception is taken because requiring the field would oblige every declaration in the tree and
  every fixture to write `()` for a fact empty on almost all of them, and because a
  `ToolDefinition` is stored inside a `PermissionDecision`, so a required field would refuse every
  decision written before it. **The exception is this one further field, on this argument** — no
  lane reads this record and the 2026-09-12 one together as licence to default a third safety
  field.

  **Everything else stands entire**: every other field a permission decision depends on stays
  required with no default, `frozen=True` and the audit-record argument behind it stand,
  `description`'s non-blank refusal stands, and §§2-7 are untouched. §4's `parameters_schema`
  declaration is relied on rather than moved, and the classification is a field **beside** that
  schema rather than a keyword inside it, for the hazard the 2026-09-12 record already names.
- **Partially superseded: 2026-09-15 by ADR-0267 — §1's `ToolDefinition` model declaration and
  its required-field clause, in the application to one further field. Nothing else in this ADR.**
  The model gains `quoted_output`, a `QuotedOutput | None` defaulting to `None`, naming at depth
  one the key of a step's output carrying the **whole charge** an act will make and the key
  carrying that amount's ISO-4217 code. It is the whole of what a declaration says about where a
  price may be read from, and the field lives on the declaration rather than on anything a turn
  produces precisely so that no model chooses which number a price is. The default is an exception
...
- **Partially superseded: 2026-09-15 by ADR-0262 — §1's `ToolDefinition` model declaration and its
  ...
  **whole** tuple the pinned definition carries, every member having to hold, and authors no
  ...
  taken on **its own fail-closed ground**, and **not** on the field being outside a permission
  decision's reach: a goal-scoped authorization compares the request's declaration with the row's
  **by value**, so this field moves a coverage answer exactly as a severity or schema edit does.
  The empty tuple is an exception because it makes the **opposite** claim to the one §1 refuses —
  which is `system_supplied`'s ground and `quoted_output`'s, while `bounded_arguments`' record
  rests on a different one it states for itself, and neither is restated here — a declaration that declares nothing verifies nothing, no criterion
...
- **Partially superseded: 2026-09-15 by ADR-0271 — §1's `ToolDefinition` model declaration and its
  required-field clause, in the application to one further field. Nothing else in this ADR.** The
  model gains `charged_output`, a `ChargedOutput | None` defaulting to `None`, naming at depth one
  the key of an invocation's output carrying **the whole amount it charged** and the key carrying
  that amount's ISO-4217 code. It is the whole of what a declaration says about where an act reports
  what it charged, and it is **a second field beside `quoted_output` rather than that field read
  again**: ADR-0267 §3 defines `quoted_output` over *"the whole charge the act **will** make"*, a
  prospective price from which that decision's mint appends a quote, so reading a charge there would
  make the acting step's own appended reading the operand of the very comparison it is supposed to
  be checked by. A reader holding only §1 therefore authors a definition whose acts report no charge,
  so **no call under such a declaration is ever satisfying, no `MONEY` criterion every call of which
  runs under one is ever `met`, and no goal resting on such a criterion reaches `ACHIEVED`**: the charge test is never taken, which leaves such a criterion `unestablished` unless
...
- **ADR-0004 §2 still reads as forbidding all tool egress**, and this ADR does
  ...
  precondition on the invocation ADR. [Note (2026-07-20): **ADR-0017 supersedes that clause**, so the bullet above
  no longer states the live rule — tool egress is permitted in principle. The
  preconditions it names survive unchanged: the seam, per-call gating and the
  recipient rules are ADR-0017 §3's conditions on designating the boundary, and
  `tools/` transmits nothing until they hold in code and a later ADR ratifies
  that they do. Issue #52.]
