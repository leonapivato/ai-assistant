...
- **Partially superseded: 2026-09-15 by ADR-0271 — §1's `ToolDefinition` model declaration and its
  ...
  and every `MONEY` criterion resting on that tool is `unestablished` for want of an operand. The
  ...
  no charged output reports none and establishes nothing — which is `quoted_output`'s ground and
  `postconditions`'. Every other clause of §1 binds entire, its `frozen=True` rule and its
