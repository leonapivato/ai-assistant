...
- **Partially superseded: 2026-09-13 by ADR-0266 — §1's `ToolDefinition` model declaration,
  ...
  user's own words fixed or bounded, and nothing about which slot carries it — so the argument
  it meets is decided at the **comparison**: an argument is covered where its declaration
  declares it at that kind, the record carries exactly one member of the kind, and the argument
  satisfies it. That is what makes ADR-0254 §9 clause (ii)'s *"a value for an argument the act's
  own words bear on"* true by construction rather than by assertion, and it is why the
  declaration must carry the fact: nothing else in this system says which argument of a tool
  takes an amount, which takes a period, and which key carries an amount's currency. So
  ...
  **opposite** claim to the one §1 refuses: no argument takes a declared kind, so no member
  meets anything, nothing is covered and the call asks. Requiring the field would oblige every
