...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1's `ToolDefinition` model declaration,
  ...
  **The grounds are the clause's own reason, which does not reach this default.** The empty tuple
  here makes the **opposite** claim to the one §1 refuses: no argument takes a declared kind, so
  the argument route holds nothing and the only route left is the stricter one, the quote.
  Requiring the field would oblige every declaration in the tree and every fixture to write `()`
  for a fact empty on almost all of them, with no return in the direction §1 protects. **The
  exception is this one further field, on this argument** — no lane reads this record and the
  2026-09-12 one together as licence to default a third safety field.
