...
- **Partially superseded: 2026-09-13 by ADR-0266 — §1's `ToolDefinition` model declaration,
  ...
  coverage members over a call's arguments; ADR-0266 states how one is **minted** — how a
  recorded span of the user's own words is associated with an argument key, and how the member's
  shape is chosen. For an exact value the association is a comparison against the request's own
  value, which needs nothing declared. For a **range** it cannot be: a span reading as a figure
  says nothing about which of a declaration's arguments is the amount, ADR-0254 §2 makes the
  currency key *"the whole of the association between an amount and the currency it is
  denominated in"* and forbids inferring it *"from a field name, a type, a schema keyword or a
  neighbouring argument"*, and ADR-0254 §4 forbids reading a `parameters_schema` keyword to
  establish an association a row must name. So `ToolDefinition` gains **`bounded_arguments`**,
  a possibly-empty, duplicate-free tuple of `BoundedArgument` defaulting to the empty tuple,
  each naming a key of `parameters` at depth one, the `BoundKind` it takes and, on `MONEY` alone,
  its `currency_argument`.
  ...
  **The second scope: the required-field clause, for this one further field, recorded as an
  ...
  on is required"*, unconditionally, and a permission decision does depend on this one — it
  decides whether a coverage member may be a range at all. **The grounds are the clause's own
  reason, which does not reach this default.** The empty tuple here makes the **opposite** claim
  to the one §1 refuses: no argument takes a range, so every member is an exact value, a
  declaration that says nothing asks more often rather than less, and no wider authority can ever
  rest on silence. Requiring the field would oblige every declaration in the tree and every
  fixture to write `()` for a fact empty on almost all of them, with no return in the direction
  §1 protects. **The exception is this one further field, on this argument** — no lane reads this
  record and the 2026-09-12 one together as licence to default a third safety field.
