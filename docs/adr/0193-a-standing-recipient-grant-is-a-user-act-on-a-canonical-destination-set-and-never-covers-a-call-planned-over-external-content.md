...
- **Partially superseded: 2026-09-11 by ADR-0247 — two scopes, and nothing else in this
  ...
  **§6's eighth-check clause, and the scope of its invariant, in one limb.** The invariant
  is stated over a non-resolving `ALLOW` carrying an `egress_binding` and an
  `authorised_by`, and every such row is validated against the grant store by these eight
  checks. That scope is narrowed by one conjunct — **and whose binding's `closed_loop` is
  `False`** — because a route-(c) row names no grant for the store to hold. **A replacement
  invariant is stated beside it rather than a gap left**: the trail refuses a route-(c)
  `ALLOW` whose `authorised_by` is not equal to its own binding's `BoundAccount.reference`,
  decidable from the decision alone with no store read, which is `_check_authorisation`'s
  own reason — *"the pointer is a string a policy could invent"* — met one route over.
