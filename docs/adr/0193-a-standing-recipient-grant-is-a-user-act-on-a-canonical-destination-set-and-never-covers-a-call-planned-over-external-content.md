...
- **Partially superseded: 2026-09-11 by ADR-0247 — two scopes, and nothing else in this
  ...
  **§6's pairing clause, and the scope of its eight-check invariant, in one limb.** The
  ...
  the trail refuses a route-(c) `ALLOW` whose `authorised_by` is not equal to its own
  binding's `BoundAccount.reference`, decidable from the decision alone with no store read,
  which is `_check_authorisation`'s own reason — *"the pointer is a string a policy could
  invent"* — met one route over.
