...
- Status: Partially superseded by ADR-0235 (§1's "one class rather than several" limb alone: `RecipientGrantStore.record` gains two subclasses of `InvalidRecipientGrantError` — `RecipientGrantCeilingError` for the outstanding-count ceiling and `DuplicateRecipientGrantError` for a granting record duplicating an outstanding grant's `tool`, `account` and `destinations` — because on those two grounds the caller's recourse is not identical, which is the limb's own stated ground; every other refusal ground keeps the base class, the base class still catches all of them, and the rest of §1 stands entire, its exact store surface, its ceiling value and placement and its atomic count-with-append included) and ADR-0238 (§3's first clause in its fifth comparison, §4's first clause, and §6's eighth-check clause in its seventh limb — the limb requiring the decision's binding to carry `planned_with_external_content` as `False`, which now reads "`False`, or `closed_loop` `True`" — each for a closed-loop request as that decision's §5 defines one and for no other request: such a request may be covered by a grant, ruled `ALLOW` on route (b) of the egress-authorisation route enumeration and recorded, notwithstanding its binding carrying `planned_with_external_content`; §3's other four comparisons and its no-inference clause, §4's remaining clauses, §6's other seven checks, its `OriginUnrecordedBinding` arm, its ordering, revocation and digest rules, and §5's rule that a grant reaches the recipient and never the payload all stand entire, and that decision's §1 records the payload permission as a separate user act precisely so that §5 can be left standing) and ADR-0247 (§3's first clause in its application to a `WEB_SEARCH` request at the configured search provider, which reaches an `ALLOW` by a route of its own — the configuration being the owner's act for that one kind — rather than by a grant covering it, so that no `RecipientGrant` is established, read, extended or implied for such a request; and §6's pairing clause together with the scope of the eight-check invariant it states, in the limb reaching a non-resolving `ALLOW` whose `authorised_subject` is unset **and whose binding carries `closed_loop`**, such a decision being validated by a pointer check taken over the decision alone rather than against the grant store; the digest's presence becomes what tells the two standing routes apart over the whole history, `closed_loop` is the eligibility rather than the discriminator, and a digest-free standing `ALLOW` of any other kind is refused by the pairing clause exactly as it is today. Those two scopes, and nothing else in this ADR beyond what the two records above already recorded here: §1's store, §2's establishing act, §3's five comparisons wherever a grant is the route and its comparison-not-inference rule, §4, §5's rule that a grant reaches the recipient and never the payload, §6's other seven checks and its `OriginUnrecordedBinding` arm, §7's check point, §§8-9, §11's rendering bar and §§12-16 bind as those records left them) and ADR-0254 (§6's invariant in the scope of its route-(b) arm alone, which takes one further conjunct — **and whose `authorised_subject` is set and whose `authorised_goal` is unset** — so that a row resting on a goal-scoped authorization is not held to checks that resolve against this store; §6's eight checks themselves, its reservation clause, §3's five comparisons, §9's prospectivity and expiry rules and §11's three states all bind entire)
- Date: 2026-08-24
...
- **Partially superseded: 2026-09-11 by ADR-0247 — two scopes, and nothing else in this
  ADR beyond what ADR-0235 and ADR-0238 already recorded here.** The owner ruled that the
  configured web-search provider is the recipient they granted, for `WEB_SEARCH` and for
  nothing else, so such a request reaches an `ALLOW` without a grant at all.

  **§3's first clause, in its application.** A `WEB_SEARCH` request whose binding's account
  reference and canonical destination set are the deployment's configured search connection
  and origin takes **ADR-0148 §3's new route (c)** rather than needing a grant to cover it.
  No `RecipientGrant` is established, read, extended or implied for it, and the grant seam
  is consulted **zero** times. **The five comparisons are not narrowed, widened or
  reordered**, and they bind entire wherever a grant *is* the route: an email, a fetch, a
  tool call and a search bound anywhere else, in the very same conversation. **§5 is relied
  upon and not superseded** — a grant still reaches the recipient and never the payload —
  which is what ADR-0238 §1 kept it standing for.

  **§6's pairing clause, and the scope of its eight-check invariant, in one limb.** The
  invariant is stated over a non-resolving `ALLOW` carrying an `egress_binding` and an
  `authorised_by`, and every such row is validated against the grant store by these eight
  checks; the pairing clause separately refuses one that *"names standing authorisation …
  and fingerprints none"*. That scope is narrowed by one conjunct — **and whose
  `authorised_subject` is set** — and the pairing refusal becomes route (b)'s rather than
  every standing row's, because a route-(c) row names no grant for the store to hold and
  mints no digest. **A replacement invariant is stated beside it rather than a gap left**,
  and it has two conditions: a digest-free standing `ALLOW` is admitted only where its
  binding carries `closed_loop` **and** its `authorised_by` equals that binding's
  `BoundAccount.reference`, both decidable from the decision alone with no store read.
  Failing either, **the pairing clause refuses it exactly as it does today** — so an email,
  a fetch or a tool call cannot reach an `ALLOW` with no grant by omitting a digest, and the
  relaxation stays as wide as ADR-0148 §3's new route and no wider. The pointer half is
  `_check_authorisation`'s own reason — *"the pointer is a string a policy could invent"* —
  met one route over.
  ...
  **What is relied upon as written.** §1's store and its atomicity; §2's establishing act;
  ...
  arm, its ordering and its digest rule; §7's check point and its
  after-every-other-ground placement; §8; §9's prospectivity, which ADR-0247 §8(b) takes as
  the shape of a configuration change; §11's bar on what the audit surface renders, which
  binds on the new route as it binds on this one; and §§12-16. **This line already carries
  the leading token, so under ADR-0082 §2 no amendment qualifier is written on it.**
- **Partially superseded: 2026-09-12 by ADR-0254 — §6's invariant in the scope of its
  route-(b) arm alone. Nothing else in this ADR.** §6 states the invariant over *"a
  non-resolving `ALLOW` whose `egress_binding` is not `None`"*, which ADR-0247 §2 narrowed
  by *"and whose `authorised_subject` is set"*. ADR-0254 §7 adds a fourth route whose rows
  **also** carry a digest — the fingerprint of a goal-scoped `Authorization` rather than of
  a `RecipientGrant` — so a reader holding §6 as ADR-0247 left it would apply the eight
  checks to such a row and refuse it, because it resolves against the wrong store. The
  scope therefore takes one further conjunct, **and whose `authorised_goal` is unset**, and
  route (d) gets its own six-check invariant beside this one, resolved against an
  `AuthorizationResolution`. **The eight checks themselves bind entire on every route-(b)
  row**: the outstanding-grant read, both ends of liveness, tool equality, account
  equality, destination-set containment, the origin arm and the recomputed
  `subject_digest`, each still taken over the record the store returned rather than over
  the decision's account of it. **§6's reservation clause is discharged rather than
  superseded**: it requires a later ADR wanting a different standing source for egress to
  decide *"how the reference is told apart, whether by tagging it, by a second field, or by
  narrowing this scope"*, and ADR-0254 §7 chooses the second and the third together —
...
- **Note (2026-09-13): the `Status` line's scope text now names clauses without `ADR-NNNN`
  ...
  **The targets are unchanged — ADR-0235, ADR-0238, ADR-0247 and ADR-0254 — each still
  ...
  constraint in a note:** the closed-loop definition and the payload-permission record are
  **ADR-0238 §5's** and **ADR-0238 §1's**; the route enumeration is **ADR-0148 §3's**; and
  the earlier records on this line are **ADR-0235's** and **ADR-0238's**. ADR-0148
  supersedes no part of this ADR.
...
> §1's transcription rules, §3's split of `PermissionRuling` from
> `PermissionDecision`, its three `resolve` obligations, `authorised_by`'s two
> rules, §4's append-only trail and resolution invariant, and §5's floors.

> **Normative.** ADR-0021 §3's **precondition** on "the ADR that introduces
> standing grants" is satisfied, not superseded: §6 makes the other source
> resolvable and says where the records live. ADR-0021 §6's deferral of standing
> grants for **other actions** is likewise untouched and unnarrowed (§6's scoping
> clause).

> **Normative.** ADR-0021's `Status` line takes the leading `Partially superseded
> by ADR-0193 (<scope>)` token and the record lives in an appended dated note;
> under ADR-0082 §2 no amendment qualifier is written on that line, so the
> ADR-0148 §6 qualifier already there moves into a dated note of its own in this
> same change, carrying the same substance.

**Why this is a supersession and not the reading an earlier draft of §15 took.**
Rounds 9, 11, 15, 16, 17, 18, 19, 20, 21 and 22 raised it — architecture alone
until round 21, then both lenses on identical grounding. The reading those drafts
gave is not wrong about what ADR-0021 *says*: §3's `authorised_by` bullet does
carry its own condition, §5 does name the standing grant as the relief valve
"reachable **without amending this clause**", the named hazard is a policy
resolving an id, and the I/O sentence does sit inside a rejected alternative about
injecting a **write** seam. What it got wrong is which question ADR-0070 §1 asks.
The test is not whether the earlier text anticipated the change; it is whether a
...
- **ADR-0021 — partially superseded, in two sentences of §3 and one condition;
  ...
  no read-by-id member and §1 forbids adding one; and the rejection of injecting
  an `AuditTrail`, because the policy still does not write to the trail. §3's
  precondition on "the ADR
  ...
  **stacked additions** under ADR-0082 §1, because no sentence of ADR-0021 becomes
  false or wider on any of those axes. §1's transcription rules, §4's write-once,
  atomicity, detachment
...
not whether the earlier text anticipated the change. A reader meeting the question
again should read them for the scope, not as an argument that nothing moved.
...
- **The clause carries its own condition.** §3's first bullet is "**`decide` must
  ...
  *with* one is the case that sentence contemplates and exempts, so **that**
  bullet is satisfied rather than superseded — the shape ADR-0147 §11's precedent
  names the clause **working**, and the one §15's ADR-0148 bullet applies one ADR
  over. It is also what bounds §12's condition: the supersession reaches a sourced
  policy because ADR-0021 itself drew that line, and reaches no other.
