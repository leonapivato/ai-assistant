...
- Status: Partially superseded by ADR-0250 (two scopes. §7's `ProposedUnderstanding` field enumeration, in one member's element type alone: `questions` becomes a `tuple[ProposedQuestion, ...]`, so that a raised question names what it is about and a materiality test has a subject to run over. And §1's `Goal` model declaration, in the field count alone: `Goal` gains `last_engaged_in`, an `Identifier | None` naming the conversation of the goal's most recent engagement, so a reader holding only §1 authors a goal that never carries one and does not conform. Those two scopes, and nothing else in this ADR: §7's `Planner.plan` roster, its `PlannerOutput` enumeration, its retained-or-restated validator, its retention-copies-forward clause, its `ProposedElement` shapes, its ground-resolution rules and their refusals, its minted-record clause and its interpretation-is-the-model's asymmetry all bind entire; §1's `conversation_id`-is-provenance-and-not-a-fence clause binds entire and is the ground the new field is added on, its four-absences clause is extended and not weakened, and its `statement`-as-projection rule, its append-only interpretation rule and its `version` clause are untouched; §1's clause that no lane in that decision reads `last_engaged_at` is fulfilled rather than superseded; and §§2-6 and §§8-17 stand entire) and ADR-0251 (one scope. §7's `Planner.plan` parameter-preservation clause, in the `empty_reads` term alone: "keeps `context`, `memories`, `capabilities`, `files` and `empty_reads` **exactly as they stand**" stops being true of the fifth of those, which becomes `read_outcomes: Sequence[ReadOutcome] = ()` — carrying the same asks and, beside each, the typed outcome it earned, so that a refusal, a failure, an expiry, a truncation and a duplicate are as visible to the planner as an empty read is. The four other named parameters are kept exactly as they stand, and every remaining clause of §7 binds entire: the `GoalBrief` first positional parameter, the required `utterance` keyword, the `evidence` keyword defaulting to empty, the `PlannerOutput` return, the annotation clause, the applied keyword-parameters-not-a-bundle ruling, `PlannerOutput`'s two-field enumeration and its clause that `None` is neither an error nor an instruction to re-plan, together with §7's roster, its retained-or-restated validator, its retention-copies-forward clause, its `ProposedElement` shapes, its ground-resolution rules and refusals, its minted-record clause and its interpretation-is-the-model's asymmetry. §5's licences are exercised rather than superseded, and §§1-6 and §§8-17 are untouched) and ADR-0252 (three scopes. §1's `GoalElement` model declaration, in the field list and the validator's shape count alone: `GoalElement` gains `evidence_row_id`, an `Identifier | None`, and the validator admits a fourth shape, because a `FROM_EVIDENCE` element may name a `GoalEvidence` row of the same goal instead of a record of the labelled supply and carries exactly one of the two — which is the route §13 defers to A4 by name and §7 says is not through an id into `memory`. The same widening reaches `GoalInterpretation`'s outcome arguments, which §1 validates "as a `GoalElement`'s are". And §7's `FROM_EVIDENCE` resolution clause, in the label space alone: the clause resolving such a reference by the memory-labelling scheme, unchanged — *the label of the record at 1-based index n of the `memories` sequence passed on that call* — gains a second space, `E` followed by *n* over the `evidence` sequence §7 itself put on the seam, in the shape §9 already fixed for `constraints`, `criteria` and `conditions`. And §12's `GoalRevision` clause, in the command's field list and `record_interpretation`'s stated effect alone: the command gains `invalidates`, a possibly-empty `tuple[Identifier, ...]`, and the member marks each named evidence row `INAPPLICABLE` in the same indivisible step as the append, because an invalidation must be atomic with the revision that occasions it and a second store call cannot be. Those three scopes, and nothing else in this ADR: §1's append-only interpretation rule, its `Ground` closure at exactly three members, its four-absences clause, its `version` clause and its `GoalElement` correspondence argument all bind entire; §7's `Planner.plan` roster, its `PlannerOutput` enumeration, its retained-or-restated validator, its retention-copies-forward clause, its `ProposedElement` shapes, its refusal of a label naming a search-minted record, its silent drop of an element whose ground does not resolve, its no-identifier-crosses-the-seam clause and its interpretation-is-the-model's asymmetry all bind entire; §9's label scheme is extended by one sequence and is otherwise untouched; §10's six digest members, its no-identifier rule and its `supported`-is-never-derived rule are implemented rather than superseded; §11's "It does not gain evidence rows, because A4 mints them" is a statement about that decision's own change and is fulfilled rather than contradicted; §12's every other clause binds entire — `save_goal` as the opening write alone, the attempt members and `commit_attempt`, `commit_transition`'s claim condition, the append-only reference tuples, the compare-and-swap discipline and its error class, the commands-not-snapshots rule, the `delete_goal` cascade, the no-new-Protocol rule, the `PROTOCOL_VERSION` rule and the rule that `PlanExport` is not a second wire ground; and §§2-6, §8 and §§13-17 stand entire) and ADR-0253 (two scopes. §1's `GoalElement` model declaration, in the field list alone, as the record above already widened it: the model gains `id`, an `Identifier | None` minted by `orchestration` when an element is first recorded and copied unchanged by §7's retention, and `applicability`, an `EvidenceApplicability | None` stating what the element is about in the one form that record's §2 can compare — without the first, its §8 refresh test can never hold across two plans, because a re-reading necessarily interprets a new record and nothing else on an element survives a re-plan, and without the second its §9 invalidation predicate has no operands and its "`GoalRevision.invalidates` is empty on every revision" would be permanent. And §7's `ProposedElement` field enumeration and its four-shape validator, on the **new**-element shape alone: that shape gains the four axes an `EvidenceApplicability` is composed from, so a planner can say what a condition it proposes is about, and a **retaining** element still carries `retains` and nothing else. Those two scopes, and nothing else in this ADR: §1's append-only interpretation rule, its `Ground` closure at exactly three members, its four-absences clause, its `version` clause and its clause that the type expresses the correspondence rather than a rule to remember all bind entire; §7's `Planner.plan` roster, its `PlannerOutput` enumeration, its retained-or-restated validator, its retention-copies-forward clause, its ground-resolution rules and their refusals, its minted-record clause, its silent drop of an element whose ground does not resolve, its no-identifier-crosses-the-seam clause and its interpretation-is-the-model's asymmetry all bind entire; §8's two-field clause becomes a **four-field** clause in the field count alone — the fields any other component sets gain each `StepCondition.about` and each `PlanInterpretation.settles`, under §5's identical discipline, because an element's `id` is minted by `orchestration` after the planner returns and a plan conditioned on an element that same call proposed is otherwise inexpressible — while §8's `targets_revision`, its loop-sets-it clause, its stamp-after-recording ordering, its `save_plan` refusal (which gains one conjunct on this ADR's own §12 strengthening footing), its `commit_transition` claim condition and its not-driven rule are **relied on** and are what make a plan's conditions resolvable against one revision's elements; §9's `GoalBrief` and `BriefElement` enumerations gain **nothing** and its label scheme is used rather than extended; §10's digest is untouched; §12's wire, export and migration clauses bind entire and no store migration is owed; and §13's A5 entry — "`ActionPlan`'s step fields — `depends_on`, engine-resolved result references, the `when` vocabulary and `verifies`" — is **discharged** rather than superseded, together with the two deferrals of the planning-model decision it names. §§2-6 and §§11 and 14-17 stand entire) and ADR-0265 (three scopes, each stated in a count and none in a rule. §1's `Goal` model declaration: `Goal` gains `intended_actions`, a possibly-empty `tuple[IntendedAction, ...]`, append-only and oldest first, holding the acts the goal intends — so a reader holding only §1 authors a goal with nowhere to record which act a step is an attempt at, and an at-most-once effect claim keyed on the goal and the call's arguments alone dispatches once where the user asked twice. §7's `PlannerOutput` field enumeration: "carrying exactly two fields" becomes three — `plan`, `understanding` and `actions`, a possibly-empty `tuple[ProposedAction, ...]` defaulting to empty — because a planner that cannot propose an intended action cannot mint one and the record has no other author, and it rides on `PlannerOutput` rather than inside `ProposedUnderstanding` precisely because §7's own completeness rule makes omission removal, which is the one carrier a record that must survive every unmentioning revision may not have. And §9's `GoalBrief` field enumeration together with its label scheme's sequence count: the brief gains `actions`, a possibly-empty `tuple[BriefAction, ...]` carrying an intent and the live `C`/`S`/`D` labels of the elements it serves and no identifier at all, and the scheme "applied to three sequences" is applied to a fourth under the letter `A`. Those three scopes, and nothing else in this ADR: §1's append-only interpretation rule, its `statement`-as-projection rule, its round-trip clause, its four-absences clause and its `version` clause all bind entire; §7's `Planner.plan` roster, its `PlannerOutput` clause in every respect but the field count, its retained-or-restated validator, its retention-copies-forward clause, its `ProposedElement` shapes, its ground-resolution rules and their refusals, its minted-record clause, its silent drop of an element whose ground does not resolve, its no-identifier-crosses-the-seam clause and its interpretation-is-the-model's asymmetry all bind entire and are the grounds the new record's minting, resolution and refusals are reasoned from; §8's `targets_revision`, its loop-sets-it clause and its stamp-after-recording ordering are relied on and extended by nothing; §9's no-ground-reference rule, its no-identifier rule, its projected-from-the-current-interpretation-alone rule, its `goal_id` argument and its element-free-brief clause all bind entire; §10's digest is untouched; §12's widening is a precedent relied on and its member roster is not closed, so the one member added there is a stacked addition recorded in the later decision alone; and §§2-6, §11 and §§13-17 stand entire) and ADR-0261 (three scopes. First, a count and a division, both of §5. **§5's `AttemptOutcome` closure** — *"closed at exactly six members: `VERIFIED`, `CONDITION_PREVENTED`, `PARTIAL`, `FAILED`, `UNCERTAIN` and `ANSWERED`"* — becomes **seven**, gaining `CANCELLED`, *the attempt produced nothing and nothing of it is outstanding*, because §5's own validator compels an `AttemptOutcome` on every terminal state, `CANCELLED` is one of §5's two terminal states, and none of the six is true of an attempt a user ended before it produced anything — so a reader holding only §5 cannot construct the `GoalAttempt` that decision's act writes. **And §5's division of which member an attempt earns**, together with §13's *"which `AttemptOutcome` member an attempt earns. A10"*: the division becomes *every attempt but a cancelled one*, A10 keeping every member for every attempt it can reach and reaching no cancelled attempt, which §5's own *"no transition leaves a terminal member"* is what makes true. That one scope, and nothing else in this ADR: §5's *"The vocabulary is added to and never renamed"* is **exercised rather than superseded** and every one of the six keeps its name, its value and its meaning; §5's two-shape validator, its terminal-member closure at `CANCELLED` and `ENDED`, its no-transition-leaves-a-terminal-member rule, its `ANSWERED`-is-not-a-weaker-`VERIFIED` clause, its attempt-is-opened-only-by-a-user-act rule, its no-interpretation-on-an-attempt rule, its `AttemptEffort` clauses and its *paused* derivation all bind **entire** and are the grounds that decision reasons from; §8's `targets_revision` clauses and its `commit_transition` claim condition are **relied on** and extended by nothing; §4's *"An attempt reaching a terminal state does not move the goal's status"* binds entire and is obeyed in the direction it is stated; §12's append-only reference tuples, its commands-not-snapshots rule and its compare-and-swap discipline are untouched; and §§1-3, §§6-7, §§9-11 and §§13-17 stand entire. Second scope: **the class §8's revision conjunct refuses with**. §8's *"refused with the error class a stale `expected_version` already raises"* becomes `ClaimRefused`, a new `PlanningError` subclass raised by that conjunct and by ADR-0255 §3's attempt conjunct **in its *state* limb alone** — the attempt the claim names being terminal or paused — and by nothing else, so that a driver composing a report can tell a liveness refusal from an unknown execution or a malformed transition; and §8's *"The stale-revision refusal keeps `StaleExecutionError`"* becomes false of that conjunct alone. A lost `expected_version` on the same call keeps `StaleExecutionError` untouched, and the direction that class carried — re-read and retry — was never exercised, because the only `→ RUNNING` claimant is the driver and it does not retry a refused claim: a revision moves when the user corrects their understanding, whose answer is a later turn that plans again. §8's conjunct itself, its no-separate-read rule and its `targets_revision` clauses all bind entire. Third scope: **§12's sole-mutation-route sentence**, "After the first write, every change goes through this member", which carries no caller-or-Protocol qualifier and is made false in two places. First, that decision's `PlanStore.close_goal_abandoned` ends **every non-terminal attempt of a goal** as part of the one indivisible step that writes `GoalStatus.ABANDONED` and reports whether anything of the goal was outstanding at that instant — a mutation a *caller* makes, through the Protocol, and not through `commit_attempt`; it is there because the abandonment answer R78 owes is a fact about the instant the goal closed, and an act taking the attempt commits, the closure and the observation as separate calls loses that fact to an interleaving in every arrangement of them. Second, that decision's versioned plan-store upgrade step, which writes `AttemptState.CANCELLED` directly onto every non-terminal attempt of a goal already `ABANDONED` — completing the act that goal's own user performed, which the pre-change `abandon_goal` left half-written. That sentence alone, and those two writers alone: `commit_attempt` remains the route for every attempt mutation that decision does not name — every phase stamp, every effort counter, every `ENDED`, and every `CANCELLED` a caller other than that one act writes — and §12's `add_*` append discipline, its ignored-duplicate rule, its commands-not-snapshots rule, its append-only reference tuples and its compare-and-swap discipline each bind entire. §12's declaration of `commit_attempt` and `open_attempt` is not otherwise touched: the refusals that decision adds to both are strengthenings of existing members and owe no record, on ADR-0255 §3's own precedent) and ADR-0267 (one scope. §1's `Goal` model declaration, in the field count alone: `Goal` gains `quotes`, a possibly-empty `tuple[ActionQuote, ...]` that is append-only and oldest first, whose order **is** the total order a quote's governing rule is read under, and `quotes_elided`, an `int` `ge=0` counting what its history has dropped — so a reader holding only §1 authors a goal that can hold no quote, and every `MONEY` ceiling of that goal is proved against nothing. §1's append-only interpretation rule, its `statement`-as-projection rule, its round-trip clause, its four-absences clause and its `version` clause bind entire; §2's elision is relied on and is the shape the new bound reuses, its *"a count and never an identifier"* and never-decreases rules included; §12's member roster is not closed, so the one `PlanStore` member the later decision adds is a stacked addition recorded there alone; and §§3-17 stand entire)
- Date: 2026-09-12
...
- **Note (2026-09-13): the `Status` line's scope text now names clauses without `ADR-NNNN`
  ...
  superseded by` is a target"* — and this line carried 8 such tokens inside its scope
  parentheses. A consumer doing what §4 says a consumer does read ADR-0014, ADR-0211 and
  ADR-0226 as supersession targets of this ADR, which they are not.
  **The targets are unchanged — ADR-0250, ADR-0251, ADR-0252 and ADR-0253 — each still
  ...
  constraint in a note:** the applied keyword-parameters ruling is **ADR-0211 §2's**; the
  labelling scheme §7's `FROM_EVIDENCE` clause resolves by is **ADR-0226 §3's**, and
  ADR-0252's scope quoted that clause of §7 in full — *"resolved by ADR-0226 §3's
  labelling scheme, unchanged — the label of the record at 1-based index n of the
  `memories` sequence passed on that call"* — where the line now names it without the
  token and the quotation stands here; the record ADR-0253's scope layers on is
  **ADR-0252's**, whose §2, §8 and §9 the same scope
  names; the strengthening footing is this ADR's own **§12**; and the two deferrals
  discharged with §7's are **ADR-0014 §7's**. Of those, only ADR-0252 is a target of this
  line — and the token `ADR-0249` inside the scope was this ADR naming itself, which §4's
  rule read as this ADR superseding itself.
...
- **Partially superseded: 2026-09-13 by ADR-0265 — §1's `Goal` model declaration, §7's
  `PlannerOutput` field enumeration, and §9's `GoalBrief` field enumeration together with its
  label scheme's sequence count, each in a field or sequence count alone. Nothing else in this
  ADR.** The owner's correction of 2026-09-13 on #2255 requires a **stable identity for an
  intended action**, minted once and linked to the goal elements it serves, so that *"rewording
  or splitting an element must not accidentally permit a duplicate booking"*. ADR-0265 is the
  decision that mints it, and ADR-0255 §7 is what asked for it in terms — *"a goal records no
  completed effect that a driver could compare against"*, and a driver that compared capability
  and parameters *"would be inventing an identity nobody declared"*.

  **What moves in §1.** `Goal` gains `intended_actions`, a possibly-empty
  `tuple[IntendedAction, ...]`, append-only and oldest first, empty on every goal at the moment
  it is opened. A reader holding only §1 authors a goal with nowhere to record which act a step
  is an attempt at, so an at-most-once effect claim keyed on the goal and the call's arguments
  alone dispatches **once** where the user asked **twice** — ADR-0070 §1's test coming out on the
  supersession side, and **partial** in §3's sense. §1's append-only interpretation rule, its
  `statement`-as-projection rule, its round-trip-through-its-own-dump clause, its four-absences
  clause, its `conversation_id`-is-provenance clause and its `version` clause each bind entire;
  the new tuple is a **second** append-only sequence beside the revisions and takes nothing from
  the first.

  **What moves in §7, and it is the field count alone.** *"carrying exactly two fields"* becomes
  three: `plan`, `understanding`, and `actions`, a possibly-empty `tuple[ProposedAction, ...]`
  defaulting to empty, whose empty value means the planner proposes no new intended action and is
  read as no error, no degradation and no instruction to re-plan — §7's own clause about
  `understanding`, applied to the new member. It rides here and **not** inside
  `ProposedUnderstanding` for §7's own reason: that section makes a revision *"a complete
  statement of an understanding"* and an element it *"neither retains nor replaces is **not** in
  the new revision"*, so a record whose whole purpose is to survive a revision that does not
  mention it may not sit in the one carrier where omission is removal. Every other clause of §7
  binds entire — the `GoalBrief` first positional parameter, the `utterance` and `evidence`
  keywords, the `PlannerOutput` return, the applied keyword-parameters-not-a-bundle ruling, the
  retained-or-restated validator, the retention-copies-forward clause, the `ProposedElement`
  shapes, the ground-resolution rules and their refusals, the refusal of a label naming a
  minted record, the silent drop of an element whose ground does not resolve, the
  no-identifier-crosses-the-seam clause and the interpretation-is-the-model's asymmetry — and the
  last of those is the ground on which a model **proposes** an intended action and
  `orchestration` **records** it.

  **What moves in §9, and it is the field enumeration and one count.** The brief gains `actions`,
  a possibly-empty `tuple[BriefAction, ...]`, one entry per member of `Goal.intended_actions` in
  that tuple's order, each carrying an intent and the `C`/`S`/`D` **labels** of the elements it
  serves that are still in the current revision — and **no identifier of any kind**, which is
  §9's own containment argument applied rather than weakened. And the label scheme *"applied to
  three sequences"* is applied to a **fourth**, under the letter `A`, which collides with none of
  `M`, `F`, `C`, `S`, `D`, `E` or `G`. §9's no-ground-reference rule, its no-record-identifier
  rule, its projected-from-the-current-interpretation-alone rule, its `goal_id` argument, its
  `_render_request` clause and its element-free-brief clause each bind entire.

  **What is relied on and superseded in nothing.** §8's `targets_revision`, its loop-sets-it
  clause and its stamp-after-recording ordering are the ordering ADR-0265 §2 extends by one step
  and the operand its §6 names. §10's digest is untouched and no execution, effect or outcome is
  projected to the planner. §12's widening is a **precedent relied on**: its member roster is not
  closed, so the one `PlanStore` member ADR-0265 adds contradicts no sentence of it and is a
  stacked addition recorded in that decision alone (ADR-0082 §1); its compare-and-swap
  discipline, its commands-not-snapshots rule, its error class, its `delete_goal` cascade, its
  no-new-Protocol rule and its `PROTOCOL_VERSION` rule are each adopted whole. Appended dated
  note per ADR-0070 §1. Refs #2255.
- **Partially superseded: 2026-09-15 by ADR-0267 — §1's `Goal` model declaration, in the field
  count alone. Nothing else in this ADR.**
  `Goal` gains `quotes` and `quotes_elided`. The tuple is append-only and oldest first, and its
  **position** is the total order the governing-quote rule is read under — no instant is compared,
  no sequence number is minted and there is no tie-break to get wrong, which is exactly why §1's
  own `interpretation` is a tuple. The bound is an elision rather than a refusal because members
  are dropped from the front alone: within any one intended action the latest quote is the last to
  go, so an elision either leaves the governing quote where it was or leaves that action with none
  and the act asks. §2's elision, its count and its never-decreases rule are reused unchanged.
