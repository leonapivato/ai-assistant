...
- **Partially superseded: 2026-09-15 by ADR-0266 — §1's `Goal` model declaration, in the field
  list alone. Nothing else in this ADR.** ADR-0254 gives a goal-scoped authorization **coverage
  members** minted from the user's own recorded acts, and ADR-0266 decides what such a member
  records and what it is proved against. The owner's ruling of 2026-09-14 settles the second
  half: the tool takes what the action needs — a site, dates, a party size — and *"the price is
  usually a consequence of those choices, not an argument the assistant supplies"*, so a stated
  bound is proved against the **quote** the investigation recorded for the goal's intended
  action (ADR-0265 §1), over the very arguments the call will carry, and never against an
  argument of the call. That quote has to live somewhere durable and goal-scoped, and this
  section is the declaration that says what a `Goal` carries. So `Goal` gains **`quotes`**, a
  possibly-empty `tuple[ActionQuote, ...]`, oldest first and append-only, each member naming the
  intended action it was quoted for, its `BoundKind`, the quoted value in the encoding
  `ActionRequest.parameters_digest` is taken over, its currency where it states an amount, the
  digest of the user-facing arguments it was quoted over, and the step output it was read at;
  and **`quotes_elided`**, an `int` `ge=0`. A reader holding only §1 authors a goal with no place
  for a quote, so a `MONEY` member is proved against nothing and every act carrying one asks.

  **§2's bounded-history construction is reused rather than restated, and the choice between its
  two shapes is stated.** The quote tuple is bounded by **elision** — the oldest dropped, the
  count kept on the goal — where ADR-0265 §1 bounds `intended_actions` by a **refusal**. The
  difference is what losing one costs: a dropped quote fails nothing silently, because the
  comparison then finds none, the request is not covered and the user is asked, while a dropped
  identity would silently make a performed act's effect claim fresh. Where the failure of a bound
  is a question, the elision this section already uses is the right shape.

  **Every other clause of §1 binds entire**, and §§2-17 stand entire — §2's bound and its elided
  counts, §7's retention clause and its span check against the turn's own utterance, which is
  the check the coverage basis needs and is taken there and not again, and §9's containment
  argument, are each relied on rather than moved. Appended dated note per ADR-0070 §1. Refs
  #2373.
