...
- Status: Partially superseded by ADR-0251 (one scope. §5's per-turn sentence alone, in its first limb — the planning budget gates the start of each additional planner call, so a turn that declares one starts at most two planner calls and therefore at most two searches. That decision's §5 moves the planner-call count to the attempt and declares four for a conversational one, so the figure two stops being true of a turn inside such an attempt; the sentence's second limb, that an operation declaring none does not iterate at all, binds verbatim and is kept by that §5 unchanged, as is the PT20S planning gate the first limb names. §5's per-search clause, its per-money clause and above all its per-conversation clause — "**Per conversation: nothing bounds the number of searches**, and that is the decision rather than an omission" — all bind **verbatim**, and that decision's §11 answers §13's first deferral by keeping that sentence true rather than by reversing it. §5's removal clauses, its two-SQLite-columns clause, its dissolution of two deferrals of the search-supply decision and its #1908-sentence clause are untouched, and §§1-4 and §§6-16 are untouched) and ADR-0254 (three scopes, two in §2 and one in §3. **Its discriminator clause in the route-(b) limb alone**, which takes one further conjunct — a non-resolving egress `ALLOW` carrying a digest is route (b) where `authorised_goal` is unset and route (d) where it is set. And **its route-(c) outcome clause** — "where it holds **and the request is at the configured provider**, the ruling is an `ALLOW` on route (c) **without consulting the recipient-grant seam at all**" — in the one limb reaching a request carrying a `goal` for which the goal-authorization seam, asked for the live record of that goal and that request's declaration `id`, answers neither *no record* nor *a record every argument of the request is covered by*: the argument-authority bar refuses every standing route for such a request and the ruling is `CONFIRM`, because a recipient authority reaches the recipient and never the payload. Every other request at the configured provider — one carrying no goal, one whose goal holds no live record of that declaration `id`, and one whose record covers every argument it carries — reaches the same `ALLOW` on the same pointer with the recipient-grant seam consulted zero times. And **§3's final clause, in its reference to the lineage floor alone** — the sentence preserving, for *"An email, a fetch, a tool call and a search bound anywhere else"*, the lineage floor and four other clauses *"**exactly as written**"* — which becomes over-wide in that one limb, because the lineage floor is itself narrowed for an egress request every user-facing argument and every member of whose canonical destination set a live goal-scoped authorization record covers. **That clause's purpose is untouched and is what the scope preserves**: this ADR's own exception rides nothing but a `WEB_SEARCH` at the configured provider, and the goal-scoped route inherits none of it — the discharge is stated over full coverage and over no configured value. §3's two retirements, its both-limbs clause, its `SearchSupply` clause and its untouched-floors clause otherwise bind entire. §2's discriminator in its route-(c) limb, its one derived fact, route (c)'s **reachability** on the same five conditions, its ordering before the grant seam, the digest-free admission on `closed_loop` and pointer equality, the eligibility-versus-discriminator division, the trail's own invariant and the no-revalidation clause all bind entire, as do §§1 and 4-16) and ADR-0260 (§1's first clause and §8(a), in the limb enumerating which request kinds the configuration-based authority covers, and in no other limb: the population becomes a closed two-member set — a `WEB_SEARCH` at the configured search provider, or a forecast read at the configured forecast provider — each kind compared against its own separately configured connection reference and origin, with every conjunct kept as recorded-value equality and neither side inferred, folded, matched by domain or re-canonicalised. §1's remaining clauses, §2's route (c) and its trail check, §3's retirements as that ADR's §6 restates their reach over a generalised derived fact, §4's `closed_loop` — untouched, and still the search kind's alone — §5's removal of the call budget, §6, §7, §8's (a′) to (d′), §9's unchanged list including its fetch bar, and §§10-16 all bind entire) and ADR-0272 (§2's eligibility conjunct alone — the fact `AuditTrail.record` reads to admit a digest-free standing row, which §2 states three times in `closed_loop`'s own name: in the two-condition admission clause, in the eligibility-versus-discriminator clause and in the clause excluding a stored row predating ADR-0193's implementation. The conjunct becomes *the binding carries its kind's own configured-provider fact* — exactly one of `closed_loop` and `forecast_reach` carried, a binding carrying neither or both refused — so that the row ADR-0260 §6 mints for a forecast read at the configured forecast provider is recordable. Every other limb of §2 binds entire and is read over the substituted fact: the pointer half, the digest discriminator, the narrowing of ADR-0193 §6's pairing clause to route (b), route (c)'s reachability on the same five conditions, its ordering before the grant seam, the trail-holds-no-configuration clause, the no-revalidation clause and the rendering clause. §§1 and 3-16 bind entire, §4's `closed_loop` conspicuously — untouched in its field, its type, its default, its carriage and its comparison, and still the search kind's alone)
- Date: 2026-09-11
...
- **Partially superseded: 2026-09-12 by ADR-0254 — §2's discriminator clause in its
  route-(b) limb, and §2's route-(c) outcome clause in one limb. Nothing else in this
  ADR.**

  **The discriminator clause.** §2 rules that *"a non-resolving egress
  ...
  no-revalidation clause.

  **The route-(c) outcome clause, in one limb.** §2 also rules that where
  `_only_the_disclosure_floor` holds *"**and the request is at the configured provider**, the
  ruling is an `ALLOW` on route (c) **without consulting the recipient-grant seam at all**"*.
  ADR-0254 §6 states an **argument-authority bar** that is taken before any standing route.
  Where a request carries a `goal` and the policy holds the goal-authorization seam, that seam
  is asked for the live record of that goal and that request's declaration **`id`**, and a
  standing route is taken only on two answers: *no record*, or *a record every argument of the
  request is covered by*. On any other answer — including a seam the policy could not read, or
  two live rows where one was owed — no standing route is taken and the ruling is `CONFIRM`.
  **The bar reads no field of the declaration**, which is what keeps it monotone across a
  severity edit in either direction (ADR-0021 §5). A reader
  holding §2 would read the outcome as following from the eligibility and would author an
  `ALLOW` the policy owes a refusal for — ADR-0070 §1's test met. **The ground is the one
  ADR-0193 §5 already states**: a recipient authority reaches the recipient and never the
  payload, so neither a grant nor this deployment's own configuration discharges what the
  user's own act about that goal fixed, bounded, or did not authorise at all. **The scope is
  that limb and nothing else.** Every other request at the configured provider — one carrying
  no goal, one whose goal holds no live record of that declaration `id`, and one whose record
  covers every argument it carries — reaches the same `ALLOW`, on the same `account.reference`
  pointer, with the recipient-grant seam consulted zero times, exactly as §2 rules.
  **Route (c)'s *reachability* is untouched**:
  the bar refuses after `_only_the_disclosure_floor` holds rather than narrowing it, so §2's
  *"reachable exactly where route (b) is reachable, and on the same five conditions"* stays
  true word for word, and so does the ordering that puts route (c) before the grant seam.
  **§3's retirements are untouched**: the lineage floor and the coverage exception retire for a
  request at the configured provider exactly as this ADR rules, and the bar retires and relaxes
  nothing.

  **ADR-0254's route (d) inherits neither of this ADR's retirements**: §3's retirement of the
  lineage floor and the coverage exception is for a request at the configured provider and for
  nothing else, and ADR-0254 §6 says so in terms. Route (d) discharges the lineage floor on its
  own ground and under its own condition — the record covers the request **in full** — and it
  keeps `SpanCoverage` binding unrelaxed, so the two no longer retire together outside this
  ADR's own route. **ADR-0254 §12 states by name that this ADR's configured-provider authority
  takes no expiry**: it has no record, no instant and no revocation event, §8(b)'s prospectivity
  is its whole lifecycle, and the owner's decision 7 of 2026-09-12 excludes it in terms. Refs
  #2255.

  **The third scope: §3's final clause, in its reference to the lineage floor alone.** That
  clause reads *"An email, a fetch, a tool call and a search bound anywhere else each keep
  ADR-0181 §5's floor, ADR-0193 §3 and §4, ADR-0155 §3 and ADR-0233 §9 **exactly as
  written**"*. ADR-0254 §6 narrows ADR-0181 §5 **itself**, for an egress request every
  user-facing argument and every member of whose canonical destination set a live
  `Authorization` of that request's own goal covers — so a reader holding this clause would
  read that floor as preserved in full for such a request, which it no longer is. **The
  clause's purpose is untouched and is exactly what the scope preserves**: this ADR's
  exception rides nothing but a `WEB_SEARCH` at the configured provider, in the very same
  conversation and the very same turn, and route (d) takes none of it — its discharge is
  stated over full coverage and over no configured value. **Its references to ADR-0193 §3 and
  §4, to ADR-0155 §3 and to ADR-0233 §9 are unmoved**: no recipient grant covers a call
  carrying `planned_with_external_content`, and a binding carrying covered content reaches
  route (d) in no case. §3's two retirements, its both-limbs clause, its `SearchSupply` clause
  and its ADR-0155/ADR-0233 clause otherwise bind entire.
- **Note (2026-09-13): the `Status` line's scope text now names clauses without `ADR-NNNN`
  ...
  superseded by` is a target"* — and this line carried 11 such tokens inside its scope
  parentheses. A consumer doing what §4 says a consumer does read ADR-0016, ADR-0194,
  ADR-0228, ADR-0231, ADR-0238 and ADR-0241 as supersession targets of this ADR, which
  they are not.
  **The targets are unchanged — ADR-0251 and ADR-0254 — each still paired with the same
  ...
  in a note.** The per-turn sentence quoted in full: *"**Per turn**: ADR-0228 §4's
  planning budget gates the start of each additional planner call, so a turn that declares
  one starts at most two planner calls and therefore at most two searches, and an
  operation declaring none does not iterate at all."* It is **ADR-0251 §5** that moves
  **ADR-0228 §3's** count to the attempt, **ADR-0251 §5** that keeps the second limb
  ...
  **ADR-0194**, **ADR-0228**, **ADR-0231**, **ADR-0238** and **ADR-0241** supersede no
  ...
  other ADR moves, and no mark is added (ADR-0089 §5). Refs #2288, #2302.


- **Partially superseded: 2026-09-15 by ADR-0260 — §1's first clause and §8(a), in one
  limb each.** Both are written over a request *"whose kind is `WEB_SEARCH`"*, and §1's
  heading says so twice over — *"for `WEB_SEARCH` and for nothing else"*; §8(a) adds
  *"that account and that origin, and **no other**"*. [ADR-0260](0260-a-forecast-read-is-its-own-seam-at-a-configured-provider-and-what-it-mints-carries-the-extent-that-makes-it-evidence.md) §6 widens that population to
  a closed **two**-member set, a forecast read at the deployment's configured **forecast**
  provider taking route (c) beside a search at its own. **A reader holding only this ADR
  would refuse that ruling**, which is ADR-0070 §1's test coming out on the supersession
  side and ADR-0070 §3's partial form being the sanctioned tool.

  **What is kept is every conjunct, and that is the point of the narrow scope.** The
  comparison stays `DurableIdentifier` equality on the connection reference and
  `CanonicalDestination` equality on the destination set, over recorded values, neither
  side inferred, folded, matched by domain or re-canonicalised; **each kind is compared
  against its own configured pair**, so a request of one kind bound to the other's
  provider takes no route at all. §1's why-over-the-request-and-not-the-destination
  argument is inherited whole and is what makes the widening safe.

  **Not replaced — everything else, and §9 most of all.** §4's `closed_loop` is untouched
  in its field, its type, its default, its carriage and its comparison, and stays the
  **search** kind's alone: ADR-0260 §11 adds a sibling boolean rather than widening this
  one, precisely so that no stored row's assertion changes. §2's route (c), its derived
  fact and its trail check; §3's two retirements, whose limbs ADR-0260 §6 restates over a
  generalised fact without widening their reach; §5, §6, §7; §8's (a′), (b), (b′),
  (b″), (c) and (d), which bind on the new kind word for word with its own configured
  pair read for the search one; §10's surface; and §§11-16 all bind entire. **§9's second
  clause is read and is not this**: its subject is *"milestone 32's bounded fetch … at
  destinations nobody configured"*, and a provider the deployment configured is the one
  thing that is not that — ADR-0260 §14 restates the bar rather than relying on it.

  Appended dated note per ADR-0070 §1; no ratified text is rewritten and no mark is added
  (ADR-0089 §5). Refs #2255.
- **Partially superseded: 2026-09-16 by ADR-0272 — §2's eligibility conjunct alone, and
  nothing else in this ADR.** §2 admits a digest-free standing row *"**only** where its
  binding's `closed_loop` is `True` **and** its `authorised_by` equals that binding's
  `account.reference`"*. [ADR-0272](0272-the-trail-admits-a-digest-free-standing-row-on-the-kinds-own-configured-provider-fact.md) §1
  replaces the first of those two conditions with **the binding's own kind's
  configured-provider fact** — exactly one of `closed_loop` and `forecast_reach` carried,
  a binding carrying **neither** refused and one carrying **both** refused. **A reader
  holding only this ADR would refuse the `ALLOW` ADR-0260 §6 rules**, because a forecast
  binding never carries `closed_loop`; that is ADR-0070 §1's test coming out on the
  supersession side and ADR-0070 §3's partial form being the sanctioned tool. The defect is
  #2459, found by ADR-0260's L2 lane (PR #2458) and independently by that PR's round-3
  reviewer.

  **What is kept is the rest of §2 entire, read over the substituted fact.** The pointer
  half is unmoved — `authorised_by` still equals the binding's own `account.reference`, for
  `_check_authorisation`'s own reason — and so is the digest discriminator, which still
  says which route a row claims from the row alone; ADR-0272 §1 changes what makes a
  route-(c) row **eligible** and never which rows are route (c). The narrowing of ADR-0193
  §6's pairing clause to route (b), route (c)'s reachability on the same five conditions,
  its ordering before the grant seam, the clause forbidding the trail a `Settings` value,
  the no-revalidation clause and the rendering clause all bind entire. **§2's exclusion of
  ADR-0193 §11's reserved pointer survives with its own argument**: `forecast_reach` was
  added by ADR-0260 §11, later than the ADR-0238 field §2 argues from, so no row predating
  ADR-0193's implementation can carry either fact.
