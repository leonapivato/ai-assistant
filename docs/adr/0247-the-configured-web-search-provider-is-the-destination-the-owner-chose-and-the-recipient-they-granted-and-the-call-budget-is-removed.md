...
- **Partially superseded: 2026-09-12 by ADR-0254 — §2's discriminator clause in its
  ...
  **The route-(c) outcome clause, in one limb.** §2 also rules that where
  ...
  standing route is taken only on two answers: *no record*, or *a record covering the request's
  declaration by value and every one of its arguments*. On any other answer — including a seam
  the policy could not read — no standing route is taken and the ruling is `CONFIRM`. A reader
  ...
  covers the declaration by value and every argument — reaches the same `ALLOW`, on the same
  `account.reference` pointer, with the recipient-grant seam consulted zero times, exactly as
  §2 rules. **Route (c)'s *reachability* is untouched**:
