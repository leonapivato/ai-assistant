...
- **Partially superseded: 2026-09-12 by ADR-0254 — §2's discriminator clause in its
  ...
  **The route-(c) outcome clause, in one limb.** §2 also rules that where
  ...
  where a request carries a `goal` for which the goal-authorization store holds a **live**
  record of that request's declaration, and that record names an argument of the request in a
  member the argument fails, no standing route is taken and the ruling is `CONFIRM`. A reader
  ...
  payload, so neither a grant nor this deployment's own configuration discharges a value the
  user's own act about that goal fixed or bounded. **The scope is that limb and nothing else.**
  Every other request at the configured provider — one carrying no goal, one whose goal holds
  no live record of that declaration, and one whose record names no argument the request fails
  — reaches the same `ALLOW`, on the same `account.reference` pointer, with the recipient-grant
  seam consulted zero times, exactly as §2 rules. **Route (c)'s *reachability* is untouched**:
