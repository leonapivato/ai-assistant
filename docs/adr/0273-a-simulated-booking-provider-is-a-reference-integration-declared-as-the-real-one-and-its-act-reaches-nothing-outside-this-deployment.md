...
> refused at the read when it falls outside. **The enforcement point and the
> pruning mechanism are the lane's**, as the store's schema, format and path are:
> this decision fixes the obligation and not its mechanics. That the limb reaches
> a store outside `memory/` is **ADR-0004's own reading of itself**: its ADR-0268
> scope note says of the goal authorization store that *"§6's
...
> narrows it in no respect: it discharges the limb here rather than deferring
> it.** **ADR-0101 §7 is no authority to defer it**, because that section defers
> the two user-facing *subject-scoped* surfaces and says of ADR-0007 §1's four
> operations that **`purge_expired()`** *"is retention reclaim, not a data right,
> and is untouched"*. A bound over this store's own records is retention reclaim,
> so it neither waits on that deferral nor opens the deletion route it reserves.
...
>     uncertain effect**, and is not an implementation of this arm. **And
>     ADR-0259 §4's reconciliation pass is run over that `INDETERMINATE` step**,
>     asserting that it **invokes the provider not at all**, leaves the provider's
>     durable state — the record and the count alike — **exactly as the interrupted
>     commit left it**, and leaves the step standing **`INDETERMINATE`**: the
>     booking is `side_effecting` and its call is an egress call, so it satisfies
>     **neither** conjunct of ADR-0259 §3's reconcilability test, and §4's pass
>     *"reaches `ToolInvoker.invoke` through no act"*. That pass's act 3 moves the
>     **attempt** to `EFFECT_UNRESOLVED`, which is its own repair and **not** a
>     resolution of the effect, so this arm asserts the step's status and the
>     store rather than the attempt's state.
...
> 21. **One configuration answers identically, on every call and across a
