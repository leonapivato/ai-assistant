...
Stated because it is what makes the one-PR shape — this ADR and its
implementation together — legitimate, and it is checkable rather than asserted:
...
governs a Protocol or `core`-type change, which this is not, so it is not
triggered — and per ADR-0041 §5's precedent for a non-contract ADR, this one is
`Accepted` on merge and carried in the same change as its implementation rather
than split into a second PR.
