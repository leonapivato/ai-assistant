...
- Status: Partially superseded by ADR-0234 (the exclusion of a font's `/ToUnicode` CMap from what an extraction is bounded on, and the one-field shape that exclusion held up, in four clauses and no wider: §2's *"The decoded inputs an extraction reads **once and caches** … are **not bounded by this ADR**, and no implementation, lane or later ADR derives a refusal criterion on them from this field"*, whose `/ObjStm` half stands and whose `/ToUnicode` half is replaced — that CMap's decoded length is charged to `fetch_max_decoded_bytes`, once per parse, for every font in a parse's resource context whose `/ToUnicode` resolves to a stream; §2's *"`Settings` gains **one** field"*, which becomes two, because the CMap's cost is a function of its decoded bytes **and** of the mappings its parse builds and neither is a function of the other — 65,000 mappings arrive in 927,031 bytes of `bfchar` or in 178 of `bfrange` — so that decision adds `fetch_max_character_mappings` for the second, with its own named default and stated domain, refused at load in the same form; §3's *"Nothing else is counted"* clause, from whose enumeration of what is not charged the `/ToUnicode` CMap comes out while the `/ObjStm` stays, its separating test — the per-parse multiplier, never the cost per byte — unchanged and now applied the other way; §4's *"which of the **three** bounds refused"*, whose enumeration becomes any of four while its ruling is extended rather than replaced and binds the new field entire; and §8 arm 11's second arm, whose `/ObjStm` half is kept verbatim and whose CMap half is replaced by a refusal arm and a fetch arm at raised figures. §10's fifth deferral loses its `/ToUnicode` half, which the 2026-09-04 note below already recorded as fired, and its `/ObjStm` half is re-stated on a measurement: `PdfReader._get_object_from_stream` is entered once whatever the page count, so no per-parse multiplier acts on it. Everything else stands — §1's file bound, §2's counted-per-parse rule, its 1 MiB figure and its naming and independent-figure clauses, §3's walk entire and the standard it is held to, §4's single `TOO_LARGE` class, its every-member-is-reachable ground and the closed five-member enumeration, §5's refusal of a deadline, §6's evidence-about-a-version rule, §7's untouched audit, §9's footprint and the lane it charges, and §11)
- Date: 2026-09-03
...
- **Note (2026-09-13): the `Status` line's scope text now names clauses without `ADR-NNNN`
  ...
  **The targets are unchanged — ADR-0234 — each still paired with the same scope, whose
  ...
  field is added by **ADR-0234**, the one target of this line.
