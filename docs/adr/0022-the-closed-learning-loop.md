...
- Partially superseded: 2026-08-15 by ADR-0158 — **§3's Retrieval row no longer
  ...
  **The three pairs on the `Status` line name different scopes** — ADR-0084 §2's
  ...
  if that change does not land, neither does this. While ADR-0158 is still
  `Proposed`, the line names a supersession that is **drafted rather than
  ratified**, which is the form ADR-0075 established and `main` carries several
  times over: ADR-0076's header records ADR-0005 carrying `Partially superseded by
  ADR-0075` while ADR-0075 itself stood `Proposed`; ADR-0042 and ADR-0073 carry
  this sentence for ADR-0084, ADR-0045 for ADR-0080, and ADR-0074 for ADR-0076.
  ADR-0083 §15 rules the question outright — "**the existence condition is that
  the naming ADR ships in the same change, not that it has ratified**". The
  `Proposed` → `Accepted` flip is the ratifying edit at merge, and ADR-0070 §1
  keeps the repair path open besides — a marked supersession that never landed is
  restored by correcting the `Status` line, which changes no decision. Appended
  note per ADR-0070 §1; no ratified text below it is rewritten. Refs #791, #1163.
