...
**The policy rules on its own copy of the request, taken before `decide` is
reached.** ADR-0021 §3 gave `PermissionRuling` no field naming a tool, a payload
or a step so that a policy "cannot substitute the subject of the decision it is
answering about" — the absence is "the security property, not an economy", and
splitting the types "removes the capability rather than forbidding it". Handing
`decide` the very object that is then bound into the `PermissionDecision` and
executed would hand that capability straight back through the other argument:
`frozen=True` refuses `request.tool = ...` and does nothing about
`request.__dict__` (ADR-0018 §3), so a policy could rule `ALLOW` on a harmless
declaration and swap in another registered one before returning. Everything
downstream would agree with itself — the decision, the `ToolCall` and the invoker
all describe the substitute — and the tool the user's policy approved would never
have run. The timing carries the whole rule: a copy taken *after* `decide`
returns faithfully preserves a substitution already made.
