...
- **Partially superseded: 2026-08-28 by ADR-0205 — §3's count of the arguments
  ...
  **Not replaced — everything either section decides about what those arguments
  mean.** §3's declaration that this operation is the output channel and its
  audience unbounded, its refusal of any value by which a caller could assert,
  narrow or raise an audience, its reading of `plays` as a codec capability and not
  an audience, the threaded budget and its expiries, the local refusals, the format
  intersection, and the `PROTOCOL_VERSION` obligation all stand word for word — and
  ADR-0205 §1 states that `delivery` is likewise not an audience. §10's `timeout`
  clauses stand whole: `timeout` is still not browser-owned, no browser value
  reaches it, and a `timeout` a body carries is still never read. So do §10's one
