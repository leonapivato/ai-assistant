...
- **Partially superseded: 2026-09-12 by ADR-0255 — §5 in three scopes: its two per-turn
  ...
  before that walk's drive. **Two further clauses fall inside the same scope**: *"A superseded plan
  **drives nothing** … reaches no `StepRunner`"* binds of a plan superseded **before its walk
  began**, a first plan becoming superseded by the re-investigation's successor only after it has
  driven; and the **no-new-failure-mode** clause held because every `save_plan` preceded every
