...
- Status: Partially superseded by ADR-0240 (three scopes. §2's condition (e): a serviced structured read that was reached with budget remaining and whose completed store call returned no record at all satisfies (e), so a revision fires on it under the other six conditions unchanged; (a), (b), (c), (d), (f) and (g), §2's all-of-them rule, its closing clause and its prohibition on an implementation widening a request or substituting a read of its own all stand, and §2's other clauses are untouched; §12's no-signal clause and §1's nothing-else clause, in the single respect that the second planner call receives a defaulted parameter holding the asks whose read returned nothing, with every other signal those clauses forbid — an iteration index, a "last look" instruction, a count of the turn's calls, a budget or deadline signal — still forbidden and §1's enumeration of what is not re-run binding verbatim; §§3-11 and 13-15 are untouched) and ADR-0242 (§10's first clause, in its second sentence alone: "On every other turn it is given nothing, and the assembled prompt is byte-identical to what it is today" is false of a turn on which at least one search servicing did not reach a result, which §6 of the superseding ADR gives the composing stage a second carrier for; §10's first sentence binds entire, so a turn that stopped at the bound or the budget with a read_request standing is still given that fact and still composes an answer that says so, and §10's remaining four clauses bind entire and are the pattern §7 of the superseding ADR follows — the carrier travels inside ai_assistant.orchestration as data, adds no member to a Protocol, is never inferred at the render site, and reaches no step account; §§1-9 and §§11-15 are untouched) and ADR-0249 (§1's third clause in its second sentence alone: "The goal is minted once per turn from the user's unrewritten words and nothing about it changed" is false of a goal that carries an interpretation and false of a turn that associates to a goal an earlier turn opened. That one sentence, and nothing else in this ADR: §1's subject-stability rule, "The revision carries the same `goal_id` as the plan it replaces", binds verbatim, and the two calls of a turn plan for the same goal under that one `goal_id`; what §1 fixed is the subject and not the view of it, so where the first call revised the understanding the second receives the brief of the new revision, which is that decision's §8 stale-target rule working rather than a breach of §1; its reason, that "a second goal would make one turn look like two in every store that holds goals", binds entire; and §1's authored-at-the-seam clause, its context-assembled-once clause, its nothing-else-is-re-run clause and its capability-re-read clause are relied on unchanged; and §1's one-field clause together with §5's every-other-field clause, in the count alone, the fields any other component sets becoming `supersedes` and `targets_revision` under §5's identical discipline, with every remaining field of every plan still exactly as the planner returned it and §1's authored-at-the-seam enumeration untouched) and ADR-0251 (three scopes. §2's condition (e), together with the enumeration's completeness — its "if and only if **all** of the following hold" read as a claim that the list is closed — as the first record on this line left both: (e) is dissolved, so a servicing that completed admits a further planner call whatever its typed outcome, and three further conditions are added — the attempt's working allowance less its reserve, a bound on consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE` — while (f)'s counter and bound move under the §3 scope below rather than being added here. §2's all-of-them rule, its if-and-only-if form, its "a fact the turn already has in hand … none is a setting, and none is a judgement" framing, its closing clause and the prohibition inside it — that no implementation retries a failed servicing, widens a request, re-asks the planner on a different prompt, or substitutes a read of its own — and conditions (a), (b), (c), (d) and (g) all bind verbatim and are quoted in the superseding ADR's §4. §3's count and its subject, "A turn makes **at most two** calls to `Planner.plan`": a per-turn **total** becomes a per-**attempt admission threshold on iteration** — a further call within a turn is admitted only while the attempt's consumed calls are fewer than its kind's declared allowance, a conversational attempt declaring four — and every turn the owner starts makes its first call whatever the ledger holds, so the attempt's lifetime total is bounded by how many times the owner asks rather than by a figure. No total replaces the one that moves. §3's non-configurability clause is not superseded and is the clause the superseding decision rests on, its rule that the last plan's request is still serviced binds entire, its stopped-at-the-bound rule binds entire over the new figure, and its "the two figures differ by at most one" is unchanged in substance. And §9's closure of the stop vocabulary at five, in that count alone: the count becomes seven, gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`; every existing member keeps its name, its value, its meaning and its default, `BOUND_REACHED`'s subject moving with §3 and its name not; and §9's one-record rule, its extend-not-replace clause, its counts-and-no-copy rule, its per-turn fire-rate definition, its "not iterated" default clause and its stop-distribution clause all bind entire and are extended. §1, §4, §5, §6, §7, §8, §10 and §§11-15 are untouched: §4's PT20S stays the per-turn gate keyed on the operation, and §10's carrier gains no field) and ADR-0240 (three scopes. §2's condition (e): a serviced structured read that was reached with budget remaining and whose completed store call returned no record at all satisfies (e), so a revision fires on it under the other six conditions unchanged; (a), (b), (c), (d), (f) and (g), §2's all-of-them rule, its closing clause and its prohibition on an implementation widening a request or substituting a read of its own all stand, and §2's other clauses are untouched; §12's no-signal clause and §1's nothing-else clause, in the single respect that the second planner call receives a defaulted parameter holding the asks whose read returned nothing, with every other signal those clauses forbid — an iteration index, a "last look" instruction, a count of the turn's calls, a budget or deadline signal — still forbidden and §1's enumeration of what is not re-run binding verbatim; §§3-11 and 13-15 are untouched) and ADR-0242 (§10's first clause, in its second sentence alone: "On every other turn it is given nothing, and the assembled prompt is byte-identical to what it is today" is false of a turn on which at least one search servicing did not reach a result, which §6 of the superseding ADR gives the composing stage a second carrier for; §10's first sentence binds entire, so a turn that stopped at the bound or the budget with a read_request standing is still given that fact and still composes an answer that says so, and §10's remaining four clauses bind entire and are the pattern §7 of the superseding ADR follows — the carrier travels inside ai_assistant.orchestration as data, adds no member to a Protocol, is never inferred at the render site, and reaches no step account; §§1-9 and §§11-15 are untouched) and ADR-0249 (§1's third clause in its second sentence alone: "The goal is minted once per turn from the user's unrewritten words and nothing about it changed" is false of a goal that carries an interpretation and false of a turn that associates to a goal an earlier turn opened. That one sentence, and nothing else in this ADR: §1's subject-stability rule, "The revision carries the same `goal_id` as the plan it replaces", binds verbatim, and the two calls of a turn plan for the same goal under that one `goal_id`; what §1 fixed is the subject and not the view of it, so where the first call revised the understanding the second receives the brief of the new revision, which is ADR-0249 §8's stale-target rule working rather than a breach of §1; its reason, that "a second goal would make one turn look like two in every store that holds goals", binds entire; and §1's authored-at-the-seam clause, its context-assembled-once clause, its nothing-else-is-re-run clause and its capability-re-read clause are relied on unchanged; and §1's one-field clause together with §5's every-other-field clause, in the count alone, the fields any other component sets becoming `supersedes` and `targets_revision` under §5's identical discipline, with every remaining field of every plan still exactly as the planner returned it and §1's authored-at-the-seam enumeration untouched) and ADR-0251 (three scopes. §2's condition (e), together with the enumeration's completeness — its "if and only if **all** of the following hold" read as a claim that the list is closed — as ADR-0240 §6 left both: (e) is dissolved, so a servicing that completed admits a further planner call whatever its typed outcome, and three further conditions are added — the attempt's working allowance less its reserve, a bound on consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE` — while (f)'s counter and bound move under the §3 scope below rather than being added here. §2's all-of-them rule, its if-and-only-if form, its "a fact the turn already has in hand … none is a setting, and none is a judgement" framing, its closing clause and the prohibition inside it — that no implementation retries a failed servicing, widens a request, re-asks the planner on a different prompt, or substitutes a read of its own — and conditions (a), (b), (c), (d) and (g) all bind verbatim and are quoted in the superseding ADR's §4. §3's count and its subject, "A turn makes **at most two** calls to `Planner.plan`": a per-turn **total** becomes a per-**attempt admission threshold on iteration** — a further call within a turn is admitted only while the attempt's consumed calls are fewer than its kind's declared allowance, a conversational attempt declaring four — and every turn the owner starts makes its first call whatever the ledger holds, so the attempt's lifetime total is bounded by how many times the owner asks rather than by a figure. No total replaces the one that moves. §3's non-configurability clause is not superseded and is the clause the superseding decision rests on, its rule that the last plan's request is still serviced binds entire, its stopped-at-the-bound rule binds entire over the new figure, and its "the two figures differ by at most one" is unchanged in substance. And §9's closure of the stop vocabulary at five, in that count alone: the count becomes seven, gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`; every existing member keeps its name, its value, its meaning and its default, `BOUND_REACHED`'s subject moving with §3 and its name not; and §9's one-record rule, its extend-not-replace clause, its counts-and-no-copy rule, its per-turn fire-rate definition, its "not iterated" default clause and its stop-distribution clause all bind entire and are extended. §1, §4, §5, §6, §7, §8, §10 and §§11-15 are untouched: §4's PT20S stays the per-turn gate keyed on the operation, and §10's carrier gains no field) and ADR-0255 (one scope. §5's two per-turn clauses — "Every plan of the turn is persisted before anything is driven. The whole sequence of `save_plan` calls precedes `start_execution`" and "Exactly one plan of a turn is driven and it is the last" — become statements over a **walk**: the plans produced for a walk are all persisted before that walk's `start_execution`, and exactly one plan is driven per walk and it is the last produced for it. A turn makes at most two walks, its first and one after a licensed re-investigation, so it drives at most two plans. Without the scope a turn that investigates again after executing can neither persist nor drive the plan it produces, because that save necessarily follows the first walk's `start_execution`, and the re-investigation changes nothing. The reason for the ordering is kept rather than weakened and is what fixes the scope: it protects a turn whose second `save_plan` raises from having driven anything — "no execution is open, no capacity slot is spent on a step and no side effect has been reached" — which obtains before a turn's first walk and cannot obtain before its second, so the per-turn reading forbids the investigation and protects nothing. That one scope, and nothing else in this ADR: §5's superseded-plan-drives-nothing rule, its every-plan-is-persisted rule, its oldest-first persistence order, its one-persistence-site rule, its turn-that-ends-before-that-site-persists-nothing rule, its no-new-failure-mode clause, its `supersedes` field and loop-sets-it discipline, its export-closure clause and its no-plan-identifier-crosses-the-seam clause all bind verbatim; §14's plan-driving deferral is fired rather than superseded; and §§1-4, §§6-13 and §15 stand entire) and ADR-0240 (three scopes. §2's condition (e): a serviced structured read that was reached with budget remaining and whose completed store call returned no record at all satisfies (e), so a revision fires on it under the other six conditions unchanged; (a), (b), (c), (d), (f) and (g), §2's all-of-them rule, its closing clause and its prohibition on an implementation widening a request or substituting a read of its own all stand, and §2's other clauses are untouched; §12's no-signal clause and §1's nothing-else clause, in the single respect that the second planner call receives a defaulted parameter holding the asks whose read returned nothing, with every other signal those clauses forbid — an iteration index, a "last look" instruction, a count of the turn's calls, a budget or deadline signal — still forbidden and §1's enumeration of what is not re-run binding verbatim; §§3-11 and 13-15 are untouched) and ADR-0242 (§10's first clause, in its second sentence alone: "On every other turn it is given nothing, and the assembled prompt is byte-identical to what it is today" is false of a turn on which at least one search servicing did not reach a result, which §6 of the superseding ADR gives the composing stage a second carrier for; §10's first sentence binds entire, so a turn that stopped at the bound or the budget with a read_request standing is still given that fact and still composes an answer that says so, and §10's remaining four clauses bind entire and are the pattern §7 of the superseding ADR follows — the carrier travels inside ai_assistant.orchestration as data, adds no member to a Protocol, is never inferred at the render site, and reaches no step account; §§1-9 and §§11-15 are untouched) and ADR-0249 (§1's third clause in its second sentence alone: "The goal is minted once per turn from the user's unrewritten words and nothing about it changed" is false of a goal that carries an interpretation and false of a turn that associates to a goal an earlier turn opened. That one sentence, and nothing else in this ADR: §1's subject-stability rule, "The revision carries the same `goal_id` as the plan it replaces", binds verbatim, and the two calls of a turn plan for the same goal under that one `goal_id`; what §1 fixed is the subject and not the view of it, so where the first call revised the understanding the second receives the brief of the new revision, which is ADR-0249 §8's stale-target rule working rather than a breach of §1; its reason, that "a second goal would make one turn look like two in every store that holds goals", binds entire; and §1's authored-at-the-seam clause, its context-assembled-once clause, its nothing-else-is-re-run clause and its capability-re-read clause are relied on unchanged; and §1's one-field clause together with §5's every-other-field clause, in the count alone, the fields any other component sets becoming `supersedes` and `targets_revision` under §5's identical discipline, with every remaining field of every plan still exactly as the planner returned it and §1's authored-at-the-seam enumeration untouched) and ADR-0251 (three scopes. §2's condition (e), together with the enumeration's completeness — its "if and only if **all** of the following hold" read as a claim that the list is closed — as ADR-0240 §6 left both: (e) is dissolved, so a servicing that completed admits a further planner call whatever its typed outcome, and three further conditions are added — the attempt's working allowance less its reserve, a bound on consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE` — while (f)'s counter and bound move under the §3 scope below rather than being added here. §2's all-of-them rule, its if-and-only-if form, its "a fact the turn already has in hand … none is a setting, and none is a judgement" framing, its closing clause and the prohibition inside it — that no implementation retries a failed servicing, widens a request, re-asks the planner on a different prompt, or substitutes a read of its own — and conditions (a), (b), (c), (d) and (g) all bind verbatim and are quoted in the superseding ADR's §4. §3's count and its subject, "A turn makes **at most two** calls to `Planner.plan`": a per-turn **total** becomes a per-**attempt admission threshold on iteration** — a further call within a turn is admitted only while the attempt's consumed calls are fewer than its kind's declared allowance, a conversational attempt declaring four — and every turn the owner starts makes its first call whatever the ledger holds, so the attempt's lifetime total is bounded by how many times the owner asks rather than by a figure. No total replaces the one that moves. §3's non-configurability clause is not superseded and is the clause the superseding decision rests on, its rule that the last plan's request is still serviced binds entire, its stopped-at-the-bound rule binds entire over the new figure, and its "the two figures differ by at most one" is unchanged in substance. And §9's closure of the stop vocabulary at five, in that count alone: the count becomes seven, gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`; every existing member keeps its name, its value, its meaning and its default, `BOUND_REACHED`'s subject moving with §3 and its name not; and §9's one-record rule, its extend-not-replace clause, its counts-and-no-copy rule, its per-turn fire-rate definition, its "not iterated" default clause and its stop-distribution clause all bind entire and are extended. §1, §4, §5, §6, §7, §8, §10 and §§11-15 are untouched: §4's PT20S stays the per-turn gate keyed on the operation, and §10's carrier gains no field) and ADR-0255 (one scope. §5's two per-turn clauses — "Every plan of the turn is persisted before anything is driven. The whole sequence of `save_plan` calls precedes `start_execution`" and "Exactly one plan of a turn is driven and it is the last" — become statements over a **walk**: the plans produced for a walk are all persisted before that walk's `start_execution`, and exactly one plan is driven per walk and it is the last produced for it. A turn makes at most two walks, its first and one after a licensed re-investigation, so it drives at most two plans. Without the scope a turn that investigates again after executing can neither persist nor drive the plan it produces, because that save necessarily follows the first walk's `start_execution`, and the re-investigation changes nothing. The reason for the ordering is kept rather than weakened and is what fixes the scope: it protects a turn whose second `save_plan` raises from having driven anything — "no execution is open, no capacity slot is spent on a step and no side effect has been reached" — which obtains before a turn's first walk and cannot obtain before its second, so the per-turn reading forbids the investigation and protects nothing. **Two further clauses of §5 fall inside the same scope**: "A superseded plan drives nothing … reaches no `StepRunner`" binds of a plan superseded **before its walk began** and does not retrospectively forbid a walk that had already run when the supersession was recorded, because a first plan becomes superseded by the re-investigation's successor only after it has driven; and the **no-new-failure-mode** clause held because every `save_plan` preceded every drive, which a re-investigation's save no longer does, so the superseding decision states what a second walk's persistence failure leaves — the first walk's execution and effects stand, no second walk begins, and nothing is undone or re-dispatched. Those three scopes, and nothing else in this ADR: §5's every-plan-is-persisted rule, its oldest-first persistence order, its one-persistence-site rule, its turn-that-ends-before-that-site-persists-nothing rule, its no-new-failure-mode clause, its `supersedes` field and loop-sets-it discipline, its export-closure clause and its no-plan-identifier-crosses-the-seam clause all bind verbatim; §14's plan-driving deferral is fired rather than superseded; and §§1-4, §§6-13 and §15 stand entire) and ADR-0240 (three scopes. §2's condition (e): a serviced structured read that was reached with budget remaining and whose completed store call returned no record at all satisfies (e), so a revision fires on it under the other six conditions unchanged; (a), (b), (c), (d), (f) and (g), §2's all-of-them rule, its closing clause and its prohibition on an implementation widening a request or substituting a read of its own all stand, and §2's other clauses are untouched; §12's no-signal clause and §1's nothing-else clause, in the single respect that the second planner call receives a defaulted parameter holding the asks whose read returned nothing, with every other signal those clauses forbid — an iteration index, a "last look" instruction, a count of the turn's calls, a budget or deadline signal — still forbidden and §1's enumeration of what is not re-run binding verbatim; §§3-11 and 13-15 are untouched) and ADR-0242 (§10's first clause, in its second sentence alone: "On every other turn it is given nothing, and the assembled prompt is byte-identical to what it is today" is false of a turn on which at least one search servicing did not reach a result, which §6 of the superseding ADR gives the composing stage a second carrier for; §10's first sentence binds entire, so a turn that stopped at the bound or the budget with a read_request standing is still given that fact and still composes an answer that says so, and §10's remaining four clauses bind entire and are the pattern §7 of the superseding ADR follows — the carrier travels inside ai_assistant.orchestration as data, adds no member to a Protocol, is never inferred at the render site, and reaches no step account; §§1-9 and §§11-15 are untouched) and ADR-0249 (§1's third clause in its second sentence alone: "The goal is minted once per turn from the user's unrewritten words and nothing about it changed" is false of a goal that carries an interpretation and false of a turn that associates to a goal an earlier turn opened. That one sentence, and nothing else in this ADR: §1's subject-stability rule, "The revision carries the same `goal_id` as the plan it replaces", binds verbatim, and the two calls of a turn plan for the same goal under that one `goal_id`; what §1 fixed is the subject and not the view of it, so where the first call revised the understanding the second receives the brief of the new revision, which is ADR-0249 §8's stale-target rule working rather than a breach of §1; its reason, that "a second goal would make one turn look like two in every store that holds goals", binds entire; and §1's authored-at-the-seam clause, its context-assembled-once clause, its nothing-else-is-re-run clause and its capability-re-read clause are relied on unchanged; and §1's one-field clause together with §5's every-other-field clause, in the count alone, the fields any other component sets becoming `supersedes` and `targets_revision` under §5's identical discipline, with every remaining field of every plan still exactly as the planner returned it and §1's authored-at-the-seam enumeration untouched) and ADR-0251 (three scopes. §2's condition (e), together with the enumeration's completeness — its "if and only if **all** of the following hold" read as a claim that the list is closed — as ADR-0240 §6 left both: (e) is dissolved, so a servicing that completed admits a further planner call whatever its typed outcome, and three further conditions are added — the attempt's working allowance less its reserve, a bound on consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE` — while (f)'s counter and bound move under the §3 scope below rather than being added here. §2's all-of-them rule, its if-and-only-if form, its "a fact the turn already has in hand … none is a setting, and none is a judgement" framing, its closing clause and the prohibition inside it — that no implementation retries a failed servicing, widens a request, re-asks the planner on a different prompt, or substitutes a read of its own — and conditions (a), (b), (c), (d) and (g) all bind verbatim and are quoted in the superseding ADR's §4. §3's count and its subject, "A turn makes **at most two** calls to `Planner.plan`": a per-turn **total** becomes a per-**attempt admission threshold on iteration** — a further call within a turn is admitted only while the attempt's consumed calls are fewer than its kind's declared allowance, a conversational attempt declaring four — and every turn the owner starts makes its first call whatever the ledger holds, so the attempt's lifetime total is bounded by how many times the owner asks rather than by a figure. No total replaces the one that moves. §3's non-configurability clause is not superseded and is the clause the superseding decision rests on, its rule that the last plan's request is still serviced binds entire, its stopped-at-the-bound rule binds entire over the new figure, and its "the two figures differ by at most one" is unchanged in substance. And §9's closure of the stop vocabulary at five, in that count alone: the count becomes seven, gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`; every existing member keeps its name, its value, its meaning and its default, `BOUND_REACHED`'s subject moving with §3 and its name not; and §9's one-record rule, its extend-not-replace clause, its counts-and-no-copy rule, its per-turn fire-rate definition, its "not iterated" default clause and its stop-distribution clause all bind entire and are extended. §1, §4, §5, §6, §7, §8, §10 and §§11-15 are untouched: §4's PT20S stays the per-turn gate keyed on the operation, and §10's carrier gains no field) and ADR-0255 (three scopes. §5's two per-turn clauses — "Every plan of the turn is persisted before anything is driven. The whole sequence of `save_plan` calls precedes `start_execution`" and "Exactly one plan of a turn is driven and it is the last" — become statements over a **walk**: the plans produced for a walk are all persisted before that walk's `start_execution`, and exactly one plan is driven per walk and it is the last produced for it. A turn makes at most two walks, its first and one after a licensed re-investigation, so it drives at most two plans. Without the scope a turn that investigates again after executing can neither persist nor drive the plan it produces, because that save necessarily follows the first walk's `start_execution`, and the re-investigation changes nothing. The reason for the ordering is kept rather than weakened and is what fixes the scope: it protects a turn whose second `save_plan` raises from having driven anything — "no execution is open, no capacity slot is spent on a step and no side effect has been reached" — which obtains before a turn's first walk and cannot obtain before its second, so the per-turn reading forbids the investigation and protects nothing. **Two further clauses of §5 fall inside the same scope**: "A superseded plan drives nothing … reaches no `StepRunner`" binds of a plan superseded **before its walk began** and does not retrospectively forbid a walk that had already run when the supersession was recorded, because a first plan becomes superseded by the re-investigation's successor only after it has driven; and the **no-new-failure-mode** clause held because every `save_plan` preceded every drive, which a re-investigation's save no longer does, so the superseding decision states what a second walk's persistence failure leaves — the first walk's execution and effects stand, no second walk begins, and nothing is undone or re-dispatched. Those three scopes, and nothing else in this ADR: §5's remaining clauses bind verbatim — its every-plan-is-persisted rule, its oldest-first persistence order, its one-persistence-site rule, its turn-that-ends-before-that-site-persists-nothing rule, its `supersedes` field and loop-sets-it discipline, its export-closure clause and its no-plan-identifier-crosses-the-seam clause all bind verbatim; §14's plan-driving deferral is fired rather than superseded; and §§1-4, §§6-13 and §15 stand entire) and ADR-0240 (three scopes. §2's condition (e): a serviced structured read that was reached with budget remaining and whose completed store call returned no record at all satisfies (e), so a revision fires on it under the other six conditions unchanged; (a), (b), (c), (d), (f) and (g), §2's all-of-them rule, its closing clause and its prohibition on an implementation widening a request or substituting a read of its own all stand, and §2's other clauses are untouched; §12's no-signal clause and §1's nothing-else clause, in the single respect that the second planner call receives a defaulted parameter holding the asks whose read returned nothing, with every other signal those clauses forbid — an iteration index, a "last look" instruction, a count of the turn's calls, a budget or deadline signal — still forbidden and §1's enumeration of what is not re-run binding verbatim; §§3-11 and 13-15 are untouched) and ADR-0242 (§10's first clause, in its second sentence alone: "On every other turn it is given nothing, and the assembled prompt is byte-identical to what it is today" is false of a turn on which at least one search servicing did not reach a result, which §6 of the superseding ADR gives the composing stage a second carrier for; §10's first sentence binds entire, so a turn that stopped at the bound or the budget with a read_request standing is still given that fact and still composes an answer that says so, and §10's remaining four clauses bind entire and are the pattern §7 of the superseding ADR follows — the carrier travels inside ai_assistant.orchestration as data, adds no member to a Protocol, is never inferred at the render site, and reaches no step account; §§1-9 and §§11-15 are untouched) and ADR-0249 (§1's third clause in its second sentence alone: "The goal is minted once per turn from the user's unrewritten words and nothing about it changed" is false of a goal that carries an interpretation and false of a turn that associates to a goal an earlier turn opened. That one sentence, and nothing else in this ADR: §1's subject-stability rule, "The revision carries the same `goal_id` as the plan it replaces", binds verbatim, and the two calls of a turn plan for the same goal under that one `goal_id`; what §1 fixed is the subject and not the view of it, so where the first call revised the understanding the second receives the brief of the new revision, which is ADR-0249 §8's stale-target rule working rather than a breach of §1; its reason, that "a second goal would make one turn look like two in every store that holds goals", binds entire; and §1's authored-at-the-seam clause, its context-assembled-once clause, its nothing-else-is-re-run clause and its capability-re-read clause are relied on unchanged; and §1's one-field clause together with §5's every-other-field clause, in the count alone, the fields any other component sets becoming `supersedes` and `targets_revision` under §5's identical discipline, with every remaining field of every plan still exactly as the planner returned it and §1's authored-at-the-seam enumeration untouched) and ADR-0251 (three scopes. §2's condition (e), together with the enumeration's completeness — its "if and only if **all** of the following hold" read as a claim that the list is closed — as ADR-0240 §6 left both: (e) is dissolved, so a servicing that completed admits a further planner call whatever its typed outcome, and three further conditions are added — the attempt's working allowance less its reserve, a bound on consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE` — while (f)'s counter and bound move under the §3 scope below rather than being added here. §2's all-of-them rule, its if-and-only-if form, its "a fact the turn already has in hand … none is a setting, and none is a judgement" framing, its closing clause and the prohibition inside it — that no implementation retries a failed servicing, widens a request, re-asks the planner on a different prompt, or substitutes a read of its own — and conditions (a), (b), (c), (d) and (g) all bind verbatim and are quoted in the superseding ADR's §4. §3's count and its subject, "A turn makes **at most two** calls to `Planner.plan`": a per-turn **total** becomes a per-**attempt admission threshold on iteration** — a further call within a turn is admitted only while the attempt's consumed calls are fewer than its kind's declared allowance, a conversational attempt declaring four — and every turn the owner starts makes its first call whatever the ledger holds, so the attempt's lifetime total is bounded by how many times the owner asks rather than by a figure. No total replaces the one that moves. §3's non-configurability clause is not superseded and is the clause the superseding decision rests on, its rule that the last plan's request is still serviced binds entire, its stopped-at-the-bound rule binds entire over the new figure, and its "the two figures differ by at most one" is unchanged in substance. And §9's closure of the stop vocabulary at five, in that count alone: the count becomes seven, gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`; every existing member keeps its name, its value, its meaning and its default, `BOUND_REACHED`'s subject moving with §3 and its name not; and §9's one-record rule, its extend-not-replace clause, its counts-and-no-copy rule, its per-turn fire-rate definition, its "not iterated" default clause and its stop-distribution clause all bind entire and are extended. §1, §4, §5, §6, §7, §8, §10 and §§11-15 are untouched: §4's PT20S stays the per-turn gate keyed on the operation, and §10's carrier gains no field) and ADR-0255 (three scopes, every one of them §5's. The first is its two per-turn clauses, in their subject — "Every plan of the turn is persisted before anything is driven. The whole sequence of `save_plan` calls precedes `start_execution`" and "Exactly one plan of a turn is driven and it is the last" — which become statements over a **walk**: the plans produced for a walk are all persisted before that walk's `start_execution`, and exactly one plan is driven per walk and it is the last produced for it. A turn makes at most two walks, its first and one after a licensed re-investigation, so it drives at most two plans. Without the scope a turn that investigates again after executing can neither persist nor drive the plan it produces, because that save necessarily follows the first walk's `start_execution`, and the re-investigation changes nothing. The reason for the ordering is kept rather than weakened and is what fixes the scope: it protects a turn whose second `save_plan` raises from having driven anything — "no execution is open, no capacity slot is spent on a step and no side effect has been reached" — which obtains before a turn's first walk and cannot obtain before its second, so the per-turn reading forbids the investigation and protects nothing. **The second and third scopes are two further clauses of §5**, named rather than left inside the first. The second is its superseded-plan-drives-nothing rule, in the moment it binds at: "A superseded plan drives nothing … reaches no `StepRunner`" binds of a plan superseded **before its walk began** and does not retrospectively forbid a walk that had already run when the supersession was recorded, because a first plan becomes superseded by the re-investigation's successor only after it has driven. The third is its **no-new-failure-mode** clause, which held because every `save_plan` preceded every drive, which a re-investigation's save no longer does, so the superseding decision states what a second walk's persistence failure leaves — the first walk's execution and effects stand, no second walk begins, and nothing is undone or re-dispatched. Those three scopes, and nothing else in this ADR: §5's remaining clauses bind verbatim — its every-plan-is-persisted rule, its oldest-first persistence order, its one-persistence-site rule, its turn-that-ends-before-that-site-persists-nothing rule, its `supersedes` field and loop-sets-it discipline, its export-closure clause and its no-plan-identifier-crosses-the-seam clause all bind verbatim; §14's plan-driving deferral is fired rather than superseded; and §§1-4, §§6-13 and §15 stand entire)
- Date: 2026-09-03
- **Partially superseded: 2026-09-12 by ADR-0249 — §1's third clause in its second
  ...
  **What §1 was actually deciding is kept verbatim.** The subject-stability rule — *"The
  revision carries the **same `goal_id`** as the plan it replaces"* — binds word for word, and
  its reason binds entire: ADR-0249 adds no second goal within a turn and takes nothing from
  that argument. Both calls of a turn plan for one goal under one `goal_id`.

  **What is not claimed is that both calls see the same `GoalBrief`.** §1 fixed the *subject* of
  a turn's two calls, not the view of it, and ADR-0249 §8 requires the second call to receive the
  **current** revision: where the first call returned an understanding the loop recorded, the
  second call's brief is the new revision's, and the plan that call returns targets it. A reader
  who took this note to mean the second call must be re-handed the first call's brief would have
  it plan against an understanding already superseded, which is what the owner's third ruling of
  2026-09-12 on #2255 forbids in terms.

  **A second, narrower scope: §1's one-field clause and §5's every-other-field clause, in the
  count alone.** §1 closes *"The **one** field any other component ever sets is `supersedes`,
  which §5 gives to the loop and which is not a decision the planner is in a position to
  make"*, and §5 closes *"Every other field is **exactly as the planner returned it**."*
  ADR-0249 §8 adds `ActionPlan.targets_revision`, the interpretation revision a plan was planned
  against, and gives it to the loop for §5's own reason: the loop holds the number, the planner
  is handed no revision to copy, and a number a model wrote into a durable audit chain is
  unprovenanced. So the count becomes two and nothing else about either clause moves — the loop
  still takes the field **once**, immediately on return, before any other component observes
  it, still discards a supplied value **silently**, and every remaining field of every plan is
  still exactly as the planner returned it. A reader holding only this ADR would refuse the
  second field, which is ADR-0070 §1's test coming out on the supersession side.

  **§1's authored-at-the-seam enumeration is untouched**, and that is what keeps the scope
  narrow: it names `id`, `goal_id`, `steps`, `rationale` and `read_request`, and
  `targets_revision` is none of them. No implementation authors or edits any of those five
  anywhere but at the `Planner.plan` seam.

  **Nothing else in this ADR moves.** §1's remaining clauses — a plan's decision content
  ...
  relied on by ADR-0249 exactly as written; §5's discard-silently clause, its no-plan-identifier
...
- **Partially superseded: 2026-09-12 by ADR-0251 — §2's condition (e) and the
  ...
  **What §2 loses.** (e) reads *"The servicing returned **at least one record the supply
  ...
  planner-call allowance, its working allowance less its reserve, a stop at two
  consecutive unproductive rounds, and the attempt standing in `AttemptPhase.INVESTIGATE`. A reader holding only this ADR would refuse a second
  ...
  **What §3 loses, and what it keeps.** *"A turn makes **at most two** calls to
  `Planner.plan`"* stops being a **total** and becomes an **admission threshold on
  iteration**: a further call within a turn is admitted only while
  `AttemptEffort.planner_calls` is below the attempt kind's declared allowance, and a turn
  the owner starts makes its first call whatever the ledger holds. **No new total replaces
  the old one** — what bounds an attempt's lifetime count is the number of turns the owner
  takes, which is ADR-0247 §5's own ground. A reader holding only this ADR would stop at
  two. **§3's non-configurability is not superseded**
  ...
  **What §9 loses, and what it keeps.** *"a closed vocabulary of **five**"* becomes seven,
  gaining `WORKING_ALLOWANCE_REACHED` and `UNPRODUCTIVE`. **The count alone moves.** Every existing member keeps its name, its value and its meaning;
  ...
  **§§1, 5, 6, 7, 8 and 11-15 are untouched.** §1's assemble-once, nothing-else-is-re-run,
  ...
  wider sequence and are load-bearing in ADR-0251 §7's progress fold, which counts the records
  an ask admitted after that deduplication; §8's
...
- **Note (2026-09-13): the `Status` line's scope text now names clauses without `ADR-NNNN`
  ...
  **The targets are unchanged — ADR-0240, ADR-0242, ADR-0249 and ADR-0251 — each still
  ...
  constraint in a note:** the stale-target rule is **ADR-0249 §8's**, and it is **ADR-0240
  §6** that left §2's condition (e) and the enumeration's completeness as ADR-0251's scope
  finds them. Both are targets of this line by pairs of their own.
...
- **Partially superseded: 2026-09-12 by ADR-0255 — §5 in three scopes: its two per-turn
  ...
  persisted before anything is driven"* and *"Exactly one plan of a turn is driven and it is the last"* become statements
  over a **walk** rather than a turn, because a turn that investigates again after executing
  produces a plan whose `save_plan` necessarily follows the first walk's `start_execution`. A turn
  makes at most two walks and drives at most two plans, and each walk's plans are all persisted
  before that walk's drive. **The second and third scopes are two further clauses of §5**, named
  rather than left inside the first. **The second is its superseded-plan-drives-nothing rule, in
  the moment it binds at**: *"A superseded plan **drives nothing** … reaches no `StepRunner`"*
  binds of a plan superseded **before its walk began**, a first plan becoming superseded by the
  re-investigation's successor only after it has driven. **The third is its
  no-new-failure-mode clause**, which held because every `save_plan` preceded every
  drive, which a re-investigation's save no longer does — so that decision states what a second
  walk's persistence failure leaves: the first walk's execution and effects stand, no second walk
  begins, and nothing is undone or re-dispatched. **§5's reason fixes the scopes rather than being
  set aside by them**: the ordering protects a turn that has driven nothing, which obtains before
  a turn's first walk and cannot obtain before its second. **§5's remaining clauses bind
  verbatim** — the every-plan-is-persisted rule, the oldest-first persistence order, the
  one-persistence-site rule and the turn-that-ends-before-that-site-persists-nothing rule — and
  §14's plan-driving deferral is **fired** by that decision rather than superseded.
