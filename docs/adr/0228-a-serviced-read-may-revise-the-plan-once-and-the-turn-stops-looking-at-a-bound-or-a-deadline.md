...
- **Partially superseded: 2026-09-12 by ADR-0255 — §5's two per-turn clauses, in their
  ...
  **drives nothing** … reaches no `StepRunner`"* binds of a plan superseded **before its walk
  ...
  drive, which a re-investigation's save no longer does. **§5's reason fixes the scope rather than being set aside by it**: the
  ordering protects a turn that has driven nothing, which obtains before a turn's first walk and
  cannot obtain before its second. Every other clause of §5 binds verbatim — the
  superseded-plan-drives-nothing rule, the every-plan-is-persisted rule, the oldest-first order,
  the one-persistence-site rule and the no-new-failure-mode clause — and §14's plan-driving
  deferral is **fired** by that decision rather than superseded.
