...
- **Partially superseded: 2026-09-13 by ADR-0259 — §4's transition table, in the moves it
  ...
  **§4's table.** It gains one row, `INDETERMINATE → SUCCEEDED`, triggered by a reconciliation
  that established the effect and also setting `output` and `finished_at`. §4 states that *"every
  legal move is enumerated in §4 and enforced by `PlanExecution`"*, so a reader holding only it builds a tracker that raises
  `PlanningError` on the one move by which an uncertain effect is ever resolved. **Every other
