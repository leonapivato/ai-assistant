...
- **Partially superseded: 2026-09-13 by ADR-0259, in three scopes — §4's transition table, in
  ...
  **`satisfied_by_execution`** and **`satisfied_by_step`**, and a `SUCCEEDED` step carrying them requires **no**
  `approval_ref`, `bound_tool`, `started_at` or non-zero `attempts` — nothing ran under it, and those two fields name
  the step whose act it was, which is the correlation ADR-0004 §7 asks for carried to the act that happened. A reader
