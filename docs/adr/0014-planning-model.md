...
- **Partially superseded: 2026-09-13 by ADR-0259 — §4's transition table, in the moves it
  ...
  *"`FAILED` is terminal unless retried"* rule and its `FAILED → RUNNING` row, its
  ...
  §§1-3 and §§5-7 are untouched. **§7's deferral of *"Idempotency keys and `INDETERMINATE`
  resolution"* is fired rather than replaced**, which is a stacked addition and earns no record
