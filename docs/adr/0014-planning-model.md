...
- **Partially superseded: 2026-09-13 by ADR-0259 — §4's transition table, in the moves it
  ...
  **§5's enumeration, export shape and cascade.** The roster gains `claim_effect`, which claims a
  goal-scoped effect key in one indivisible write and answers whether that goal has already
  claimed it. `PlanExport` gains **`effects`**, a possibly-empty tuple of effect records, and its
  `schema_version` moves **from 11 to 12** because the document's shape changed and an older
