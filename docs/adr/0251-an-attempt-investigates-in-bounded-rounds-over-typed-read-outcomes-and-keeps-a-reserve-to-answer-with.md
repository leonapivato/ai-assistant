...
- Status: Partially superseded by ADR-0258 (the name of the model §3 mints, in that term alone and wherever this ADR writes it — the model is named `ReadAskOutcome` and `Planner.plan`'s replaced parameter is annotated `read_outcomes: Sequence[ReadAskOutcome] = ()`, because `core/types.py` has held `ReadOutcome` since the permission trail's own record of how a gated source read ended. Nine occurrences, three in §3 and six outside it: §3's minting clause, its parameter clause and their heading; **§12's writer clause**, "each round's `ReadOutcomeKind` and the `ReadOutcome` carrying it"; **§16's L1 clause**, "`core/types.py` gains `ReadOutcomeKind`, `ReadOutcome` and `AttemptKind`"; §15's working for the parameter declaration it supersedes, and the header bullet stating that same scope; §17's arm 2; and the Alternatives entry. At every one of them the name is the only thing replaced. Every other clause of §3 binds **verbatim** — the two fields `ask` and `outcome` and their types, the `extra="forbid"`, the carries-nothing-else clause, the parameter's name, its default, its keyword position and its additive-in-shape character, the breaking-change flag, the one-entry-per-serviced-ask-in-servicing-order clause and its `()` reading, the nothing-the-source-said clause, the ask-carried-back-unaltered clause, the carrier-and-audit-governed-separately clause, the mints-nothing-durable clause and the planner-is-not-told-which-round clause — and §§1-2 and §§4-18 keep every obligation they carry, the two marked clauses among them) and ADR-0255 (two scopes. §4's condition **(j)** — "The attempt's `phase` is `AttemptPhase.INVESTIGATE`" — together with the **read-trigger** conditions **(b)**, **(c)** and **(d)** it sits beside: all four are satisfied, in place of a serviced read, by a **re-investigation licence** — a typed outcome of a completed plan-driving walk that skipped at least one step because the world did not meet what the plan declared. Without it an attempt that executed and learned something may make its turn's one ungated planner call and may not iterate over what it learned, while ADR-0249 §5 and ADR-0250 §12 forbid opening a fresh attempt to escape the bar, so investigating an alternative is unreachable inside that attempt. And §1's occupancy clause, in one term alone: "Every round of one attempt's investigation sits inside one occupancy of `AttemptPhase.INVESTIGATE`" becomes "inside one occupancy of `AttemptPhase.INVESTIGATE` **or** under a re-investigation licence", because a round placed by the licence is placed by the attempt's current activity and not by its phase stamp. Those two scopes, and nothing else in this ADR: §1's clause that **no round moves the phase, forward or backward** binds **verbatim**, and so does its clause that **there is no re-entry into `INVESTIGATE` from a later phase** — a licensed round re-enters no phase, which is why the licence and not the phase places it — together with §1's round definition, its turn-assembled-once rule, its monotone-supply rule, its no-supply-inherited-across-turns rule and its brief-as-it-stands rule. §4's every **guard** binds verbatim and is checked unchanged — (a), (g), **(f′)'s planner-call allowance**, **(h)'s working allowance less the reserve** and **(i)'s unproductive-rounds test** — so a licensed round is charged from the same ledger and stops on the same three guards; a licence lifts no budget, admits exactly **one** round, and is spent when it is used, after which (b), (c) and (d) govern as written. §4's never-gated-first-call clause, its (e)-is-dissolved clause and (j)'s own `NOT_ITERATED` record are untouched, and §4's own reservation — "How a later turn's planning relates to an attempt that is authorizing, executing or verifying is A7's and A9's" — is the warrant for the **executing** limb alone, the other two being left to A9. §5's figures, §6's reserve and its ungated-composing clause, §7's stopping rules, §8, §§9-13 — including §13's "A replan never resets an allowance", which is what makes a licensed round charge rather than refresh — and §§14-18 all stand entire) and ADR-0255 (two scopes. §4's condition **(j)** — "The attempt's `phase` is `AttemptPhase.INVESTIGATE`" — together with the **read-trigger** conditions **(b)**, **(c)** and **(d)** it sits beside: all four are satisfied, in place of a serviced read, by a **re-investigation licence** — a typed outcome of a completed plan-driving walk that skipped at least one step because the world did not meet what the plan declared. Without it an attempt that executed and learned something may make its turn's one ungated planner call and may not iterate over what it learned, while the decisions fixing which acts open an attempt forbid opening a fresh one to escape the bar, so investigating an alternative is unreachable inside that attempt. And §1's occupancy clause, in one term alone: "Every round of one attempt's investigation sits inside one occupancy of `AttemptPhase.INVESTIGATE`" becomes "inside one occupancy of `AttemptPhase.INVESTIGATE` **or** under a re-investigation licence", because a round placed by the licence is placed by the attempt's current activity and not by its phase stamp. Those two scopes, and nothing else in this ADR: §1's clause that **no round moves the phase, forward or backward** binds **verbatim**, and so does its clause that **there is no re-entry into `INVESTIGATE` from a later phase** — a licensed round re-enters no phase, which is why the licence and not the phase places it — together with §1's round definition, its turn-assembled-once rule, its monotone-supply rule, its no-supply-inherited-across-turns rule and its brief-as-it-stands rule. §4's every **guard** binds verbatim and is checked unchanged — (a), (g), **(f′)'s planner-call allowance**, **(h)'s working allowance less the reserve** and **(i)'s unproductive-rounds test** — so a licensed round is charged from the same ledger and stops on the same three guards; a licence lifts no budget, admits exactly **one** round, and is spent when it is used, after which (b), (c) and (d) govern as written. §4's never-gated-first-call clause, its (e)-is-dissolved clause and (j)'s own `NOT_ITERATED` record are untouched, and §4's own reservation — "How a later turn's planning relates to an attempt that is authorizing, executing or verifying is A7's and A9's" — is the warrant for the **executing** limb alone, the other two being left to A9. §5's figures, §6's reserve and its ungated-composing clause, §7's stopping rules, §8, §§9-13 — including §13's "A replan never resets an allowance", which is what makes a licensed round charge rather than refresh — and §§14-18 all stand entire) and ADR-0255 (two scopes. §4's condition **(j)** — "The attempt's `phase` is `AttemptPhase.INVESTIGATE`" — together with the **read-trigger** conditions **(b)**, **(c)** and **(d)** it sits beside: all four are satisfied, in place of a serviced read, by a **re-investigation licence** — a typed outcome of a completed plan-driving walk that skipped at least one step because the world did not meet what the plan declared. Without it an attempt that executed and learned something may make its turn's one ungated planner call and may not iterate over what it learned, while the decisions fixing which acts open an attempt forbid opening a fresh one to escape the bar, so investigating an alternative is unreachable inside that attempt. And §1's occupancy clause, in one term alone: "Every round of one attempt's investigation sits inside one occupancy of `AttemptPhase.INVESTIGATE`" becomes "inside one occupancy of `AttemptPhase.INVESTIGATE` **or** under a re-investigation licence", because a round placed by the licence is placed by the attempt's current activity and not by its phase stamp. Those two scopes, and nothing else in this ADR: §1's clause that **no round moves the phase, forward or backward** binds **verbatim**, and so does its clause that **there is no re-entry into `INVESTIGATE` from a later phase** — a licensed round re-enters no phase, which is why the licence and not the phase places it — together with §1's round definition, its turn-assembled-once rule, its monotone-supply rule, its no-supply-inherited-across-turns rule and its brief-as-it-stands rule. §4's every **guard** binds verbatim and is checked unchanged — (a), (g), **(f′)'s planner-call allowance**, **(h)'s working allowance less the reserve** and **(i)'s unproductive-rounds test** — so a licensed round is charged from the same ledger and stops on the same three guards; a licence lifts no budget. **It places that turn's investigation and is not spent by a round**: it satisfies (j) and the occupancy clause for every round of that turn, while (b), (c) and (d) decide whether there is a further round, exactly as written. A turn holds at most **one** licence however many walks it makes. §4's never-gated-first-call clause, its (e)-is-dissolved clause and (j)'s own `NOT_ITERATED` record are untouched, and §4's own reservation — "How a later turn's planning relates to an attempt that is authorizing, executing or verifying is A7's and A9's" — is the warrant for the **executing** limb alone, the other two being left to A9. §5's figures, §6's reserve and its ungated-composing clause, §7's stopping rules, §8, §§9-13 — including §13's "A replan never resets an allowance", which is what makes a licensed round charge rather than refresh — and §§14-18 all stand entire) and ADR-0255 (two scopes. §4's condition **(j)** — "The attempt's `phase` is `AttemptPhase.INVESTIGATE`" — together with the **read-trigger** conditions **(b)**, **(c)** and **(d)** it sits beside: all four are satisfied, in place of a serviced read, by a **re-investigation licence** — a typed outcome of a completed plan-driving walk that skipped at least one step because the world did not meet what the plan declared **and that dispatched no sibling branch the plan had declared for the other reading of that same basis**, so a conditional plan that declared both readings and took one carries no licence at all (ADR-0253 §9). Without it an attempt that executed and learned something may make its turn's one ungated planner call and may not iterate over what it learned, while the decisions fixing which acts open an attempt forbid opening a fresh one to escape the bar, so investigating an alternative is unreachable inside that attempt. And §1's occupancy clause, in one term alone: "Every round of one attempt's investigation sits inside one occupancy of `AttemptPhase.INVESTIGATE`" becomes "inside one occupancy of `AttemptPhase.INVESTIGATE` **or** under a re-investigation licence", because a round placed by the licence is placed by the attempt's current activity and not by its phase stamp. Those two scopes, and nothing else in this ADR: §1's clause that **no round moves the phase, forward or backward** binds **verbatim**, and so does its clause that **there is no re-entry into `INVESTIGATE` from a later phase** — a licensed round re-enters no phase, which is why the licence and not the phase places it — together with §1's round definition, its turn-assembled-once rule, its monotone-supply rule, its no-supply-inherited-across-turns rule and its brief-as-it-stands rule. §4's every **guard** binds verbatim and is checked unchanged — (a), (g), **(f′)'s planner-call allowance**, **(h)'s working allowance less the reserve** and **(i)'s unproductive-rounds test** — so a licensed round is charged from the same ledger and stops on the same three guards; a licence lifts no budget. **It places that turn's investigation and is not spent by a round**: it satisfies (j) and the occupancy clause for every round of that turn, while (b), (c) and (d) decide whether there is a further round, exactly as written. A turn holds at most **one** licence however many walks it makes. §4's never-gated-first-call clause, its (e)-is-dissolved clause and (j)'s own `NOT_ITERATED` record are untouched, and §4's own reservation — "How a later turn's planning relates to an attempt that is authorizing, executing or verifying is A7's and A9's" — is the warrant for the **executing** limb alone, the other two being left to A9. §5's figures, §6's reserve and its ungated-composing clause, §7's stopping rules, §8, §§9-13 — including §13's "A replan never resets an allowance", which is what makes a licensed round charge rather than refresh — and §§14-18 all stand entire)
...
- Partially superseded: 2026-09-13 by ADR-0258 — **§3's name for the model it mints, and
  ...
  *"**`read_outcomes: Sequence[ReadOutcome] = ()`**"*. That name was already taken:
  ...
  untouched, and §§1-2 and §§4-18 keep every obligation they carry.
  **The name is replaced wherever this ADR writes it for that model, not in §3 alone**, because
  two of the other sites are marked clauses and an ADR instructing an implementation two ways at
  once would be worse than the divergence this closes. The nine, three in §3 and six outside it:
  §3's minting clause, its parameter clause and their heading; **§12's writer clause**, *"each round's `ReadOutcomeKind`
  and the `ReadOutcome` carrying it"*; **§16's L1 clause**, *"`core/types.py` gains
  `ReadOutcomeKind`, `ReadOutcome` and `AttemptKind`"*; §15's working for ADR-0240 §7 and the
  header bullet stating that same scope; §17's arm 2; and the Alternatives entry. At every one of
  them the name is the only thing replaced, and ADR-0258 §1 states the rule and lists the sites.
...
- **Partially superseded: 2026-09-12 by ADR-0255 — §4's trigger group (conditions (b),
  ...
  its plan, skipped at least one step because the world did not meet what the plan declared, and
  **dispatched no sibling branch the plan had declared for the other reading of that same basis**
  (ADR-0253 §9's two branches), so that a conditional plan which declared both readings and took
  one carries none —
  ...
  **Every guard of §4 binds verbatim and is checked** — (a), (g), (f′)'s planner-call allowance,
  ...
  round is charged from this decision's own ledger and stops on this decision's own guards. **The
  licence places that turn's investigation rather than admitting a single round**: it satisfies (j)
  and §1's occupancy for every round of that turn, and (b), (c) and (d) decide how far the loop
  goes, unchanged. A turn holds at most one licence. §13's
