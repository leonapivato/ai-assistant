...
# The ``Status`` field reader — the bullet's content column computed from the
# marker as written, and the following lines folded in only while they reach it
# — moved whole to `scripts/adr_status.py` so `scripts/check_citations.py` can
# read the same one. It had been *copied* there instead, and the copy kept this
# defect after this file's was fixed (#519 → #2017); issue #751 records what a
# second spelling of one of this repository's rules costs. The rule's own
# reasoning travelled with it and is stated there.
