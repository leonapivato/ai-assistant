...
class MutatingProvider:
    """Fails once, mutating the caller's own conversation from inside that attempt.

    The mutation is performed after this provider's own ``await``, so it lands
    while ``RetryingProvider.complete`` is itself suspended — the only window
    ADR-0065's input-observation clause is about. A mutation applied after
    ``complete`` returns cannot tell a wrapper that snapshots from one that
    ...
    What each attempt saw is recorded as plain strings, not as ``Message``
    objects: a recorded object would be re-read at assertion time and would show
    the *final* state of a turn mutated in place, hiding the very tear the case
    is looking for.
    """
