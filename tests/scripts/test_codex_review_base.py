...
An earlier version defaulted to the literal string "master" — the local
branch ref, which nothing in this workflow keeps current (worktrees branch
from origin/master directly; see claim-workspace.sh) and can sit stale
indefinitely. Reviewing against it can silently diff a completely different,
larger range than CI's merge-relative one. Fixed to prefer origin/master when
...
def _init_repo(repo: Path, *, with_origin_master: bool) -> None:
    """A repo whose 'master' branch sits one commit behind HEAD.

    HEAD's extra commit lands on a separate 'feature' branch, checked out
    from 'master' — the same topology a real PR branch has relative to a
    (possibly stale) local master ref, so a fallback to local master has a
    ...
    if with_origin_master:
        # A remote-tracking ref, faked directly — the script only asks git
        # whether refs/remotes/origin/master resolves, so a real configured
...
def _run_review(tmp_path: Path, *, with_origin_master: bool, base_arg: str | None) -> str:
    """Run the review script; return the resolved base read off its stderr."""
    assert _BASH is not None
    repo = tmp_path / "repo"
    _init_repo(repo, with_origin_master=with_origin_master)
...
def test_omitted_base_prefers_origin_master_when_known(tmp_path: Path) -> None:
    resolved = _run_review(tmp_path, with_origin_master=True, base_arg=None)
    assert resolved == "origin/master"


def test_omitted_base_falls_back_to_local_master_without_origin(tmp_path: Path) -> None:
    resolved = _run_review(tmp_path, with_origin_master=False, base_arg=None)
    assert resolved == "master"


def test_explicit_base_is_respected_even_with_origin_master_present(tmp_path: Path) -> None:
    resolved = _run_review(tmp_path, with_origin_master=True, base_arg="HEAD~1")
