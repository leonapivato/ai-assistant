...
surprise. The ``store`` bindings also carry the **fault** clause, which the fake can
only satisfy by construction: §5's *"a fault is not an absence"* is about a durable
read failing, and the durable store is where that actually happens.
"""
...
import pytest
from goal_quotes_contract import ACTION, GOAL, OTHER_ACTION, GoalQuotesContract, quote
...
from goal_quotes_contract import READ_AT as _WHEN


def _goal() -> Goal:
    """A goal holding the two acts the suite keys on, and no quote."""
    return Goal(
        id=GOAL,
        interpretation=(
            GoalInterpretation(
...
async def _seeded(store: PlanStore, quotes: Sequence[ActionQuote]) -> None:
...
class TestFakeGoalQuotesContract(GoalQuotesContract):
    ...
    async def holding(self, quotes: Sequence[ActionQuote]) -> GoalQuotes:
        """The fake, seeded in the order given."""
        return FakeGoalQuotes(quotes, goal=GOAL)
...
class TestInMemoryPlanStoreGoalQuotesContract(GoalQuotesContract):
    ...
    **The annotation is half the assertion**: :meth:`holding` returns the store where a
    ...
    async def holding(self, quotes: Sequence[ActionQuote]) -> GoalQuotes:
        """The in-memory store, holding the goal, its two acts and ``quotes``."""
        store = InMemoryPlanStore()
        await _seeded(store, quotes)
        return store
...
class TestSqlitePlanStoreGoalQuotesContract(GoalQuotesContract):
    ...
    def _directory(self, tmp_path: Path) -> None:
        """Capture this case's data directory, which :meth:`holding` takes no argument for."""
        self._path = tmp_path

    async def holding(self, quotes: Sequence[ActionQuote]) -> GoalQuotes:
        """The durable store, holding the goal, its two acts and ``quotes``."""
        store = SqlitePlanStore(path=self._path / "plans.db")
        await _seeded(store, quotes)
        return store


async def test_a_fault_behind_the_fake_is_raised_and_never_an_empty_tuple(
    tmp_path: Path,
) -> None:
    ...
    """
    del tmp_path
    seam = FakeGoalQuotes([quote()], goal=GOAL)
