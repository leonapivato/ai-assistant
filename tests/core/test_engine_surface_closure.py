...
def test_the_promoted_surface_and_the_protocol_version_are_both_pinned() -> None:
    ...
    **48 is ADR-0259 §9's L1, and it is under the second limb alone, on three grounds.**
    ``Disposition`` gains ``EFFECT_ALREADY_CLAIMED`` and ``EFFECT_UNSCOPED`` — a
    ``StrEnum`` value a client at 47 refuses against its own closed enumeration, which
    ...
    two numbers part company for a third consecutive entry. §9 fixes no numeral either,
    and a lane that lands after this one re-bumps rather than reusing it.
