...
def test_the_open_recorder_sees_the_client_an_accepted_listing_builds(
    ...
    wrong wiring, or a ``reads`` that reached its client by some route the recorder
    does not sit on, would leave the refusal below asserting an emptiness that could
    never have been anything else.
    """
    opened = _wire_recording_opens(monkeypatch, _ScriptedReadEngine(_record("r-1")))
