...
async def test_a_binding_refusal_is_committed_failed_and_not_re_driven() -> None:
    ...
    seam = FakeToolInvoker(
        [(tool("other"), implementation)], ledger=_AdmittingLedger(), gate=FakeAuditTrail()
    )
...
async def test_a_success_is_committed_with_its_output() -> None:
    ...
    state = await a_claimed_execution(store)
    seam = FakeToolInvoker(
        [(tool(), Spy({"message_id": "m-1"}))], ledger=_AdmittingLedger(), gate=FakeAuditTrail()
    )
...
async def test_a_read_only_deadline_expiry_reaches_failed() -> None:
    ...
    seam = FakeToolInvoker(
        [(read_only(), Blocking())], ledger=_AdmittingLedger(), gate=FakeAuditTrail()
    )
...
async def test_a_cancelled_call_is_committed_and_the_cancellation_re_raised(
    ...
    implementation = Blocking()
    seam = FakeToolInvoker(
        [(definition, implementation)], ledger=_AdmittingLedger(), gate=FakeAuditTrail()
    )
    task = asyncio.create_task(
...
async def test_a_call_that_does_not_survive_revalidation_claims_nothing() -> None:
    ...
    implementation = Spy()
    seam = FakeToolInvoker(
        [(read_only(), implementation)], ledger=_AdmittingLedger(), gate=FakeAuditTrail()
    )
...
async def test_a_call_substituted_while_the_claim_is_in_flight_does_not_run() -> None:
    ...
    seam = FakeToolInvoker(
        [(read_only(), innocuous), (tool(), dangerous)],
...
class ReturningInvoker(ScriptedInvoker):
    ...
    def __init__(self, definition: ToolDefinition, returned: object) -> None:
        """Answer every ``invoke`` with ``returned``."""
        super().__init__(definition, [])
        self._returned = returned

    async def invoke(self, call: ToolCall, *, timeout: timedelta) -> ToolResult:  # noqa: ASYNC109 — the seam's signature (ADR-0029 §4)
        """Return the non-conforming value, deliberately violating the annotation."""
        del call, timeout
        self.calls += 1
        return self._returned  # type: ignore[return-value]  # the point is a bad return


class ForgedDump(ToolResult):
    """A ``ToolResult`` subclass whose ``model_dump`` lies with *valid* data.

    The instance is genuinely ``INDETERMINATE``, but its overridden ``model_dump``
    returns a well-formed ``SUCCEEDED`` dict. Nothing raises — a revalidation that
    trusted this overridable serializer would mint a real ``SUCCEEDED`` result and
    the executor would commit a forged success. Rejecting a non-exact type *before*
    any overridable method runs is what closes that.
    """

    def model_dump(self, **kwargs: object) -> dict[str, object]:
        """Return a valid-but-false dict decoupled from the real fields."""
        del kwargs
...
async def test_a_seam_that_returns_a_non_result_is_closed_not_stranded() -> None:
    """A return that is not a ``ToolResult`` at all fails the exact-type gate.

    A non-conforming seam that hands back a bare object is not the contract type,
    so it is unusable before any method is called on it — closed rather than
    stranding the step (ADR-0051 §1).
    """
    await _assert_unusable_indeterminate(tool(), ReturningInvoker(tool(), object()))


async def test_a_subclass_that_forges_a_valid_dump_is_rejected_not_trusted() -> None:
    """A subclass cannot forge a ``SUCCEEDED`` commit through an overridden dump.

    ``ForgedDump`` is genuinely ``INDETERMINATE`` but its ``model_dump`` returns a
    valid ``SUCCEEDED`` dict — nothing raises, so only the exact-type gate catches
    it. Without that gate the executor would commit a forged success; with it the
    step closes ``INDETERMINATE`` (ADR-0051 §1). The critical assertion is the
    status: it must not be ``SUCCEEDED``.
    """
    forged = ForgedDump(
...
async def test_a_cancellation_requested_before_invoke_lands_on_the_claim_append() -> None:
    """The window ADR-0192 §1 opened, and the answer it rules for it.

    This case used to assert the opposite, and the reversal is the ADR's rather
    than a regression. It read: "there is no gap between the claim and the
    callable for one to land in" — true while ``invoke`` awaited the callable
    directly and suspended nowhere before it, so a cancellation requested by the
    injected clock was delivered at the **tool's** first suspension and the
    callable had been entered.

    Since ADR-0192 §1 the seam appends a claim through its ledger immediately
    before the callable, and that append **is** a suspension point. So the pending
    cancellation is delivered there. §1 rules the outcome in terms: the append is
    driven as a retained, shielded await, its outcome is observed before the
    cancellation is propagated, and "where the claim landed and the call is then
    cancelled before the callable is entered, ``invoke`` appends the completion
    carrying the outcome ADR-0029 §4 computes for that cancellation, and
    re-raises". The callable is therefore **never entered**, and the step is
    committed ``interrupted_outcome`` — ``INDETERMINATE`` for this side-effecting,
    non-``NATURAL`` tool, which ADR-0034 §1 assigns and #234 still owns.

    **What is new is that the record tells the two apart**, which is the durable
    reachability fact #234 asked for and §1 gives: the trail holds one claim,
    completed, for an act that provably did not run — where a cancellation
    *before* the claim would have left no invocation row at all.
    """
    store = FakePlanStore()
    state = await a_claimed_execution(store)
    implementation = Blocking()
    ledger = _AdmittingLedger()
    seam = FakeToolInvoker([(tool(), implementation)], ledger=ledger, gate=FakeAuditTrail())
    ...
    assert completion.failure_kind is None, "no kind is synthesised from a cancellation"
    assert await ledger.trail.open_invocations(decision_id="d-1") == []
...
async def test_a_cancellation_absorbed_while_closing_unstarted_outranks_the_cause() -> None:
    ...
    store = HoldingPlanStore()
    state = await a_claimed_execution(store)
    seam = FakeToolInvoker([(tool(), Spy())], ledger=_AdmittingLedger(), gate=FakeAuditTrail())
