...
class SqliteDeferralStore:
    ...
    def _setup(self) -> sqlite3.Connection:
        ...
        try:
            ...
            # escaping past the ``DeferralStoreError`` boundary this constructor
            # documents. Doubly so here: the constructor already documents
            # ``ValueError`` for a tuning figure it refuses, so an untranslated one
            # from the path reads as that instead (#1933).
