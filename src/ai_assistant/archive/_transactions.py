...
The transcript archive owns a ``sqlite3`` connection and needs the same four
...
arm can skip, an early ``return`` included; a ``ROLLBACK`` on the way out of *any*
...
**Why this is a sixth copy rather than an import from ``memory``, ``planning``,
``permissions``, ``tools`` or ``evaluation``.** All six own SQLite stores with
the same need, and golden rule 1 forbids one package importing another's module —
``lint-imports`` fails the gate on it, and this package is a leaf besides
(ADR-0225 §10), so it is not a convention that could be bent. A single home would
...
(#563, and #506 for the same question about ``_restrict_permissions``). Six copies
of one function is what that boundary costs, and the copy is verbatim so a fix to
one is a mechanical fix to all.
"""
