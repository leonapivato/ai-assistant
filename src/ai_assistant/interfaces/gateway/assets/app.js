...
async function relay(half, path, payload, panelId, stopping, noticed) {
  ...
  //
  // **And it is withheld rather than written quietly into the panel's slot**, which is
  // the one question in this sweep two rounds of review answered in opposite directions
  // and neither ADR settles. Round 3: a page whose ceremony outlived its session is
  // *looking at* the panel it pressed the control in, and telling it nothing leaves the
  // owner believing a destruction happened. Round 4: a panel's fault slot is one slot,
  // so a condition written there after a newer session's listing has re-filled that panel
  // sits above rows it is not about. Both are true; a slot that is per-panel cannot be
  // both. What ships is the rule #2404 asked for, uniform and with no race in it — a
  // resumed continuation whose session has ended puts nothing on the screen — and #2451
  // carries the question with both readings written out.
  if (!sameSession(half, era)) {
    return null;
  }
...
function offerGoalActs(item, goal) {
  ...
  authorities.addEventListener("click", () => listAuthorizations(goal));
...
//
// **Without it one goal's rows are drawn under another's name**: a slow request for A
// landing after a fast one for B leaves B's statement at the head of the panel and A's
...
function readCoverageView(view) {
  ...
  // member, so the page would be rendering a figure nothing denominates. That is a
  // check about the *kind*, which is this page's own discipline — `readBound`
  // branches on the kind the same way.
  //
  // **What is deliberately NOT here is whether a fixed period names a real day.**
  // ADR-0266 §3 puts that on `CoverageMember` and `CoverageView`, and golden rule 3
  // keeps business logic out of an interface adapter. A round-4 version of this
  // function parsed RFC 3339 and the calendar here, and by the next round it had
  // drifted from `core.types.period_reading_form` in **both** directions — admitting
  // a `+24:00` offset the contract refuses, and refusing every year below 0100 that
  // the contract admits, because `Date.UTC` remaps them into 1901-1999. That is the
  // hazard, observed rather than argued. The projection this page renders is a
  // validated `CoverageView`, so a conforming gateway cannot send an unreal date, and
  // a response carrying one is a rewritten response — against which a second parser
  // here proves nothing, since the same rewrite can carry a date that is real and a
  // lie. Architecture review, round 5, `blocker`.
...
function renderOpenedAuthorizations(body, opened) {
  // **Only an empty array is absence.** `null`, an object or a string are shapes no
  // conforming hub sends, and reading any of them as "this turn opened none" would
  // silently omit authorities that may have come into being — with their revocation
  // handles. A malformed **non-empty** array is already reported; a malformed value of
  // any other kind is reported on the same ground. Adversarial review, round 4,
  // `major`.
  if (Array.isArray(opened) && opened.length === 0) {
    return;
  }
  if (!Array.isArray(opened) || !opened.every(readAuthorizationView)) {
  ...
  try {
    ...
    if (body.authorizations.length === 0) {
      ...
      body.authorizations.forEach((one) =>
        renderAuthorization(list, one, false)
      );
      line(
        list,
