...
The one store here that owns a ``sqlite3`` connection needs the same four
...
The seam's error class is the only thing a caller varies, so it is the only
parameter. ``SqlitePlanStore`` keeps a thin ``_transaction`` method that binds
