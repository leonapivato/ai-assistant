...
#: branched at 47 and was written **48**; ADR-0267's Q1 landed 48 first, so it re-bumped
#: to **49** — that entry's own instruction one paragraph above, *"a lane that lands
#: after this one re-bumps rather than reusing the figure"*, and the reason §10 refuses
#: to write an integer into an ADR at all. ADR-0259's L1 also moves it and re-bumps in
#: its turn.
#:
#: **Two stored-record versions do move, and neither is a second wire ground.**
#: ``PlanExport.schema_version`` moves 14 → **15**, because the document carries
...
#: **value** rather than to a shape. The plan store's ``_SCHEMA_VERSION`` moves 6 → **7**
...
#: **downgrade** direction. **Both figures are one above what Q1 left**, for the reason
#: the version above gives: a lane landing second re-bumps rather than reusing, and a
#: version-5 file now passes through Q1's quotes migration *and* this one to reach 7.
#: Neither crosses a frame and neither is emitted by a peer.
