...
On findings, `docs/review/guide.md` is the reference: treat each one as a **hypothesis to verify against the actual text**, never a fact to comply with. Park anything out-of-scope, pre-existing, or nit-level as a **GitHub issue** — do NOT grow the PR to absorb findings. **Or acknowledge it with no action warranted** (ADR-0243 §7): where you have verified the finding is true, can state its concrete consequence, and judge that consequence too small to justify any future work, write the verdict and that one-line consequence in the PR and file nothing. It is never available at `blocker` or `major` — there the choice stays fix-or-waive — and never for a finding you have not checked against the text it describes.
