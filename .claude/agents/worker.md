...
- **Do not wait on CI before you ship and flip ready** (ADR-0180 §2). Once the closing anchor has passed on the tree you pushed and the required reviews are terminal, run `just ship`, run `just ready`, and report. The `gate` check is the merger's to watch — `gh pr checks --watch --fail-fast` runs immediately before every merge, and branch protection refuses a red one — and flipping ready starts a fresh `gate` run over the same content anyway, so waiting buys a result the flip recomputes. What guarantees the shipping tree is your own closing anchor, not the remote run.
