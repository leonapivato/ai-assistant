...
- **The review is the other, and a round is *started* and then *polled*.** Never `run_in_background`, and **never end a turn with a round in flight**.
  ...
  `wait` exits **0** with the artifact path and verdict on stdout — read the artifact and triage. It exits **3** `still running`, which means the round is alive and nothing is lost: **call `wait` again**, as many times as it takes. It exits **4** when no round is in flight for HEAD's tree — never started, running on a *different* tree because you committed after starting it, or died having recorded nothing (it prints the log). A 4 is never a reason to call `wait` again, and it can no longer be reported about a round that succeeded: the artifact directory is read *after* the round is observed gone, so a record for HEAD's tree comes back as a 0 whether or not anything is still running (#1629, #1630). Read what a 4 says and act on it. Never relaunch on a 3: the round is alive, a second one is refused, and relaunching is how a paid round gets thrown away.
...
- **"Terminal" is ADR-0138 §3's test, not the verdict word.** A lens is terminal only when its latest recorded run leaves **no finding the triage rule obliges this PR to fix** (`CONTRIBUTING.md` → "Triage every finding — do not let the PR grow to absorb them") — so a round returning a `blocker`/`major` about code in your diff is **not** terminal whatever verdict line it prints, and the two thresholds below keep counting through it. A run whose remaining findings are all deferred to issues, waived with a rationale recorded in the PR, or disposed of as *acknowledged, no action warranted* (ADR-0243 §7), *is* terminal. Read the findings, not the verdict line: every one of PR #1652's twelve adversarial rounds printed `APPROVE WITH NITS`, and eleven of them raised a `major` about code in the diff — eighteen in all, at churn 1.8×. Read as verdicts, every round looked terminal and neither arm below could fire; read as §3 asks, only the twelfth was terminal and the handoff was owed no later than the seventh (#1655).
...
- **At the fourth reviewed tree under you, diagnose the loop before you buy a fifth round.** ADR-0243 §§1–6, and it fires long before the handoff below. Once **four substantively reviewed trees** are recorded under you — persona-agnostic, so any lens's recorded artifact counts its tree, which is what `codex-review.sh` itself counts; less the figure the handoff comment you took the lane under recorded; a byte-identical rebase, a squash, a second lens on one tree, the ADR-0165 ratify flip and a round that recorded nothing each count no tree — write a **comment on the PR** that does two things. It **classifies** every open finding, and every finding you fixed since your last diagnosis, as exactly one of: a **new independent defect**, a **regression** from an earlier fix in this loop, a **specification dispute** (two readings of one clause, or a finding that contradicts a sentence of a ratified ADR), or **scope expansion**. And it **names one next activity**: an executable probe, a smaller decision (split the PR, cut its scope), an **adjudication request to the coordinator** written on the PR, a grounded waiver, or continued review with the reason stated. **A specification dispute does not get another round** — take the adjudication request (quote the clause, give both readings) or the grounded waiver. #1684 is why: three rounds, three correct findings, each forbidden by the previous round's fix, because the question was never about the code. The checkpoint authorises no merge, forbids no round, caps nothing, and is **not** a second handoff trigger — the arms below are untouched, and a handoff is never delayed or excused by a missing diagnosis (where your count never reached four, say so in the handoff comment). It is the cheap intervention that ordinarily comes before the expensive one, and PR #2136 is what it is for: four handoffs, five holders, shipped at round 26 and churn ≥2.4×, and not one holder was ever obliged to say why the loop was expensive. When you do hand off, carry your most recent diagnosis and the count of trees actually recorded — never a failed launch's printed figure — into the handoff comment (§6).
- **Past a counted threshold that stops being a judgement, and you hand the loop over.** ADR-0138 §1: hand off once a required lens has recorded **seven rounds under you** and is not terminal, or once the churn ratio `codex-review.sh` prints **first reaches 1.5 during your tenure** with a required lens still open — whichever fires first. The counts are per lens and per holder (§2: the printed round is persona-agnostic and an upper bound; subtract what the handoff comment you took the lane under recorded). It caps nothing — the eighth round is run by a **different agent** on this branch in this clone, which is the entire point. A handoff is a **comment on the PR** as well as your report — standing findings with your assessment, which you treat as settled and which you contest, the exact next action you would have taken, the per-lens counts and churn at that moment (§4), and your most recent diagnosis plus the count of substantively reviewed trees recorded on the branch (ADR-0243 §6) — and then you stop: you do not pick the successor, do not flip the PR ready on a lens that is not terminal, and do not resume the loop yourself.
...
**Terminal means both lenses terminal on the same tree** — each in ADR-0138 §3's
sense above, not the verdict line it printed. That is ADR-0020 §2 as
...
**Read what the report declines to claim, not only its verdict.** It prints its inputs — `HEAD` and its tree, the fetched base tip, the merge base, the patch identity — then, per recorded base, the base move's file set with floor paths marked `[FLOOR]`, and the §2(b) verdict. The set is printed whole or not at all — where it does not fit ADR-0027 §4's budget the drill omits it and says so, because a truncated one is the failure it exists to prevent. Where it is printed, each marked path carries on the line under it the test that bound it or that every test cleared it — ADR-0209 §6's disclosure, so that "why did this cost a round" is never something you reconstruct by hand:
...
On findings, `docs/review/guide.md` is the reference: treat each one as a **hypothesis to verify against the actual text**, never a fact to comply with. Park anything out-of-scope, pre-existing, or nit-level as a **GitHub issue** — do NOT grow the PR to absorb findings. **Or acknowledge it with no action warranted** (ADR-0243 §7): where you have verified the finding is true, can state its concrete consequence, and judge that consequence too small to justify any future work, write the verdict and that one-line consequence in the PR and file nothing. It is never available at `blocker` or `major` — there the dispositions stay the ones above, fix now for code in your diff, an issue for code you merely sit next to, or a waiver with its rationale — and never for a finding you have not checked against the text it describes.
